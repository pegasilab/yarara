...
if stage == -1:
    ...
    print(
        ...
        % (
            star,
            [instrument[0:5], "ESPRESSO"][drs_version != "old"],
...
if stage == 6:
    ...
    sts.yarara_telluric(
        sub_dico="matching_pca", continuum="linear", reference="norm", ratio=True
    )
...
if stage == 8:
    ...
    sts.yarara_produce_mask_frog(
        frog_file=root + "/Python/Material/Ghost_" + ins.split("_")[0] + ".p"
    )
...
if stage == 9:
    # CORRECT FROG
    sts.yarara_correct_stitching(
        sub_dico="matching_ghost_a", smooth_box=3, reference=ref
    )
...
if stage == 12:
    ...
    sts.yarara_correct_smooth(
        sub_dico=sub_dico, continuum="linear", reference=ref, window_ang=5
    )
...
if stage == 18:
    ...
    sts.yarara_activity_index(
        sub_dico="matching_pca",
        continuum="linear",
        substract_map=["ghost_a", "ghost_b", "thar", "mad", "stitching", "smooth"],
    )
    ...
    sts.yarara_wb_ca2(
        ...
        window_core=None,
        substract_map=["ghost_a", "ghost_b", "thar", "stitching", "smooth"],
    )
...
if stage == 30:
    ...
    if nb_comp_slice:
        ...
        sts.yarara_ccf_fit(
            "matching_slice", sub_dico=sub_dico1, proxies=base, time_detrending=0
        )
        sts.lbl_fit_vec(
            "matching_slice",
            sub_dico=sub_dico1,
...
if stage == 36:
    ...
    v2_dico = "matching_empca"
    mask_lines_clean = np.array(sts.lbl_iter[v2_dico]["catalog"]["qc_telluric"]).astype(
        "bool"
    )
    ...
    vec_clean = sts.lbl_subselection(
        sub_dico=v2_dico, kw_dico="lbl_iter", mask=mask_lines_clean, valid_lines=False
    )
...
if stage == 51:  # form the database for the slice master production
    ...
    vec_color = sts.import_ccf_timeseries(
        "LBL_ITER_kitcat_mask_" + sts.starname, sub_dico1, "rv"
    )
    ...
    for i in np.arange(nb_comp_color):
        ...
        p = np.hstack(
            [sts.matrix_corr_lbl["pixels_l1"], sts.matrix_corr_lbl["pixels_l2"]]
        )
    ...
    plt.subplots_adjust(
        left=0.05, right=0.95, top=0.95, bottom=0.07, hspace=0.4, wspace=0.45
    )
    plt.savefig(
        "/Users/cretignier/Documents/Yarara/database/%s/PCA_%s_%s.png"
        % (sts.instrument, sub_dico2, sts.starname)
    )
...
if stage == 59:
    ...
    if (np.max(sts.table.jdb) - np.min(sts.table.jdb)) > (2 * 365.25):
        ...
        if not os.path.exists(file + "1year_" + sub_dico + ".csv"):
            ...
            loc = np.where(
                (table2["star"] == sts.starname)
                & (table2["instrument"] == sts.instrument)
            )[0]
            if len(loc):
...
if False:
    ...
    kernel = sts.yarara_create_kernel(
        sub_dico="matching_activity",
        wave_min=3900,
        power_snr=2,
        save=True,
    ...
    test = sts.yarara_kernel_integral(
        "matching_pca",
        "Kernel_CaII_HD128621.txt",
        p_noise=1 / np.inf,
        doppler_free=False,
        noise_kernel="else",
        power_snr=1,
    )  # best power ~ 1 according to simulations
    test2 = sts.yarara_kernel_integral(
        "matching_smooth",
        "Kernel_WB_HD128621.txt",
        p_noise=1 / np.inf,
        doppler_free=False,
        noise_kernel="else",
        power_snr=2,
    ...
    plt.figure()
    sts.yarara_snr_contam(analysis="snr_limit", sub_dico="matching_activity")
    plt.figure()
    sts.yarara_snr_contam(
        analysis="snr_limit",
        sub_dico="matching_activity",
        slope_vec=sts.slope_corr * sts.std_norm_corr,
    )
    ...
    sts.keplerian_add_model(
        "WASP-47",
        list_par_kep=[
            [4.1590593, 142.247, 0.02614, 289.965, 245.600, 55500],
        ],
    )
    ...
    sts.keplerian_add_model(
        "Gl581",
        list_par_kep=[
            [5.3686128, 12.649, 0.01282, 275.00, 237.894, 55500],
    ...
    sts.keplerian_add_model(
        "HD45364",
        list_par_kep=[
            [344.0747, 18.838, 0.02235, 14.77, 203.326, 55500],
    ...
    sts.keplerian_add_model(
        "Gl876",
        list_par_kep=[
            [2.0661743, 3.845, 0.1435, 197.33, 57.735, 55500],
    ...
    vec_lbl2 = sts.import_ccf_timeseries(
        "LBL_ITER_kitcat_mask_" + sts.starname, v2_dico, "rv"
    )
    vec_lbl = sts.import_ccf_timeseries(
        "LBL_ITER_kitcat_mask_" + sts.starname, v1_dico, "rv"
    )

    sts.copy_dace_in_summary(
        "ESPRESSO18", path_table="ALL_OBSERVATIONS/", bin_length=1, db=0
    )  # do it is the star was not reduced by an instrument to put DRS values
    vec_ref = sts.import_dace_sts()
    ...
    vec_perso.yerr[sts.table.ins == "ESPRESSO18"] = vec_lbl.yerr[
        sts.table.ins == "ESPRESSO18"
    ]
    vec_perso.y[sts.table.ins == "ESPRESSO18"] = vec_lbl.y[
        sts.table.ins == "ESPRESSO18"
    ]
    vec_perso.yerr[sts.table.ins == "ESPRESSO19"] = vec_lbl.yerr[
        sts.table.ins == "ESPRESSO19"
    ]
    vec_perso.y[sts.table.ins == "ESPRESSO19"] = vec_lbl.y[
        sts.table.ins == "ESPRESSO19"
    ]
    ...
    vec_perso.yerr[sts.table.ins == "CORALIE14"] = np.sqrt(
        vec_perso.yerr[sts.table.ins == "CORALIE14"] ** 2 + 2.5**2
    )
    vec_perso.yerr[sts.table.ins == "HARPS03"] = np.sqrt(
        vec_perso.yerr[sts.table.ins == "HARPS03"] ** 2 + 0.5**2
    )
    vec_perso.yerr[sts.table.ins == "HARPS15"] = np.sqrt(
        vec_perso.yerr[sts.table.ins == "HARPS15"] ** 2 + 0.5**2
    )
    vec_perso.yerr[sts.table.ins == "ESPRESSO18"] = np.sqrt(
        vec_perso.yerr[sts.table.ins == "ESPRESSO18"] ** 2 + 0.3**2
    )
    vec_perso.yerr[sts.table.ins == "ESPRESSO19"] = np.sqrt(
        vec_perso.yerr[sts.table.ins == "ESPRESSO19"] ** 2 + 0.3**2
    )

    ins_rejected = [".", "HARPS03", "HARPS15", "CORALIE14", "ESPRESSO18", "ESPRESSO19"][
        0
    ]
    ...
    for name, v in zip(
        ...
        sts.periodogram_kep(
            ...
            fit_ecc=False,
            species=sts.table["ins"],
            known_planet=[4.31365, 6.58881],
            periodic=0.25,
            transits={"4,31": [1685.4751 + 57000], "6.5881": [1690.38814 + 57000]},
        )
