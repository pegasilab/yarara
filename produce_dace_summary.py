...
def fileroot_format(vec,code):
    if code=='0':
        return vec
    if code=='1':
        vec = np.array([string.split('T')[0]+'T'+string.split('T')[1].replace('-',':') for string in vec])
        return vec
...
if DRS_version=='new':
    dico = {'HARPS03':['HARPS','2.3.1','/projects/astro/HARPSNEWDRS/DRS-2.3.1/reduced','r.','_'+import_raw.upper()+'_A.fits','0'],
            ...
            'CORALIE07':['CORALIE','3.4','/hpcstorage/cretigni/CORALIE/reduced','r.','_'+import_raw.upper()+'_A.fits','0'],
            'CORALIE14':['CORALIE','3.8','/hpcstorage/cretigni/CORALIE/reduced','r.','_'+import_raw.upper()+'_A.fits','0']
            }
    use_drift = 0

elif DRS_version!='new':
    dico = {'HARPS03':['HARPS','3.5','/projects/astro/HARPSDRS/DRS-3.5/reduced','','_'+import_raw+'_A.fits','0'],
            'HARPS15':['HARPS','3.5','/projects/astro/HARPSDRS/DRS-3.5/reduced','','_'+import_raw+'_A.fits','0'],
            'HARPN':['HARPN','3.7','/projects/astro/HARPNDRS/DRS-3.7/reduced','','_'+import_raw+'_A.fits','0'],
            'CORALIE98':['CORALIE','3.3','/hpcstorage/cretigni/CORALIE/reduced','','_'+import_raw+'_A.fits','0'],
            'CORALIE07':['CORALIE','3.4','/hpcstorage/cretigni/CORALIE/reduced','','_'+import_raw+'_A.fits','0'],
            'CORALIE14':['CORALIE','3.8','/hpcstorage/cretigni/CORALIE/reduced','','_'+import_raw+'_A.fits','0']
            }
    use_drift = 1
...
if raw_directory:
    ...
    raw_directory = root+'/Yarara/'+star+'/data/'+import_raw+'/spectroDownload'
...
if len(data_extract):
    ...
    if instrument=='HARPN':
        time_raw = np.array([i.split('T')[0]+'T'+i.split('T')[1].replace('-',':') for i in time_raw])
        data_extract['berv'] = np.array(data_extract['berv'])/1000 

        
    data_extract['night'] = pd.to_datetime(time_raw) - pd.Timedelta(24*3600*1e9*0.5)
    data_extract['night'] = np.array([str(i)[:10] for i in data_extract['night']])
    data_extract = data_extract.loc[~data_extract.duplicated('dace_data')].reset_index(drop=True)

    if instrument == 'HARPS03':
        data_extract = data_extract.loc[data_extract['rjd']>53500].reset_index(drop=True)
    ...
    def index_selection(col='rv',Plot=False):
        ...
        if len(vec.stacked.y)>5:
            dust,dust,sup,inf = myf.rm_outliers(vec.stacked.y,kind='inter',m=5,return_borders=True)
        ...
        if (sum(abs(vec.stacked.y))!=0)&(len(vec.stacked.y)>1):
            if instrument!='HARPN':
                ...
                vec.stacked.substract_polymorceau([56738], degree=3, replace=False, Draw=False)            
        ...
        if sum(abs(vec.y)):
            for j in range(2): #two iteration
                ...
                if instrument!='HARPN':
                    ...
                    vec.substract_polymorceau([56738], degree=3, replace=False, Draw=False)
                model = myc.tableXY(vec.x,vec.model)
                vec.y-=model.y
                model.interpolate(new_grid=np.array(data_extract['rjd']),method='linear')
                trend+=model.y
        ...
        if Plot:
            ...
            if not iq:
                iq=1
            plt.ylim(np.nanpercentile(vec.y,25)-5*iq, np.nanpercentile(vec.y,25)+5*iq)
            plt.ylabel(col)
            plt.xlabel('Time')
        return index2
    ...
    if sigma_clipping:
        ...
        plt.subplot(5,2,1) ; index_rv = index_selection('rv',Plot=Plot)
        plt.subplot(5,2,2) ; index_rv_std = index_selection('rv_err',Plot=Plot)
        plt.subplot(5,2,3) ; index_fwhm = index_selection('fwhm',Plot=Plot)
        plt.subplot(5,2,4) ; index_fwhm_std = index_selection('fwhm_err',Plot=Plot)
        plt.subplot(5,2,5) ; index_contrast = index_selection('contrast',Plot=Plot)
        plt.subplot(5,2,6) ; index_contrast_std = index_selection('contrast_err',Plot=Plot)
        plt.subplot(5,2,7) ; index_sindex = index_selection('sindex',Plot=Plot)
        plt.subplot(5,2,8) ; index_sindex_std = index_selection('sindex_err',Plot=Plot)     
        ...
        #time clipping
        time = myc.tableXY(data_extract['rjd'].astype('float'),data_extract['rjd'].astype('float'))
        time.rm_outliers(m=200,kind='inter',bin_length=1,replace=False)
        plt.subplot(5,2,9) ; time.plot(color='gray') ; time.plot(color='k',mask=time.mask) ; plt.ylabel('Time')
    ...
    data_extract.to_csv(directory_to_yarara+'/ALL_OBSERVATIONS/All_observations_%s_%s_DRS-%s.rdb'%(star,instrument,drs_version.replace('.','-')),sep='\t',index=False, float_format='%.6f')
    ...
    data_extract_selected = data_extract_selected.loc[data_extract['spectroFluxSn50']>np.nanpercentile(data_extract['spectroFluxSn50'],1)]
    
    print(' [INFO] %.0f spectrum selected as valid on the pool of %.0f spectra'%(len(index_kept),len(index_spectrum)))
    
    rv = myc.tableXY(data_extract['rjd'],data_extract['rv'],data_extract['rv_err'])
    plt.subplot(5,2,10)
    rv.plot(color='gray',label='all(%.0f-%.0fn)'%(len(data_extract),len(np.unique(data_extract['night']))),capsize=0)
    rv = myc.tableXY(data_extract_selected['rjd'],data_extract_selected['rv'],data_extract_selected['rv_err'])
    rv.plot(color='k',label='kept(%.0f-%.0fn)'%(len(data_extract_selected),len(np.unique(data_extract_selected['night']))),capsize=0)
    iq = myf.IQ(rv.y)
    plt.ylim(np.nanpercentile(rv.y,25)-5*iq, np.nanpercentile(rv.y,25)+5*iq)
    plt.subplots_adjust(left=0.08,right=0.95,top=0.95,bottom=0.09,hspace=0.3,wspace=0.3)
    plt.legend()
    ...
    if (nb_files<treshold)&(not force_reduction):
        ...
        contrast = myc.tableXY(data_extract_selected['rjd'],data_extract_selected['contrast'].astype('float'),data_extract_selected['contrast_err'].astype('float'))
        rhk = myc.tableXY(data_extract_selected['rjd'],data_extract_selected['rhk'].astype('float'),data_extract_selected['rhk_err'].astype('float'))
        acti = r'$\log R\prime_{HK}$'
        
        if not np.nansum(abs(rhk.y)):
            acti = 'S-index'
            rhk = myc.tableXY(data_extract_selected['rjd'],data_extract_selected['sindex'].astype('float'),data_extract_selected['sindex_err'].astype('float'))
        
        rv.supress_nan() ; rv.not_null() 
        fwhm.supress_nan() ; fwhm.not_null() 
        contrast.supress_nan() ; contrast.not_null()
        rhk.supress_nan() ; rhk.not_null()
        ...
        plt.subplots_adjust(left=0.06,right=0.93,top=0.93,bottom=0.08,hspace=0.3,wspace=0.3)
        plt.savefig(directory_to_yarara+'/Summary_observations_CCF_%s.pdf'%(instrument))
        ...
        if berv_span<3:
            print(' [WARNING] BERV span unsufficiently covered for the star : %s, current value of %.1f km/s is below the treshold of 3 km/s'%(star,berv_span))
        else:
            print(' [INFO] BERV span of %.1f km/s for the star : %s'%(berv_span,star))
    
        if rv_span>50:
            print(' [WARNING] RV span too large (binary?,large amplitude planet?, transit?) for the star : %s, current value of %.1f m/s'%(star,rv_span))
        
        data_extract_selected = data_extract_selected.rename(columns={'rv':'vrad',
                                                                      'rv_err':'svrad',
                                                                      'fwhm_err':'sig_fwhm',
        ...
        for i in data_extract_selected.keys():
            try:
                if sum(np.isnan(data_extract_selected[i].astype('float'))):
                    ...
                    data_extract_selected[i] = np.round(data_extract_selected[i].astype('float'),6)
            except:
                pass
        ...
        for number in np.arange(len(complete_fileroot)):
            if os.path.exists(complete_fileroot[number].replace(import_raw.upper(),import_raw.upper()+'_SKYSUB')):
                complete_fileroot[number] = complete_fileroot[number].replace(import_raw.upper(),import_raw.upper()+'_SKYSUB')
        ...
        for f,old_night,index in zip(np.array(data_extract_selected['fileroot']),np.array(data_extract_selected['night']),np.array(data_extract_selected.index)):
            ...
            if os.path.exists(f):
                ...
                if instrument[0]=='E': #for espresso copy the data on the machine
                    ...
                    error = os.system('scp espresso_drs@espressodrs:/data/ESPRESSODRS/DRS-2.2.8/reduced/'+old_night+'/'+f.split('/')[-1]+' '+raw_directory+'/'+old_night+'/')
                    if not error:
                    ...
                    new_night = str(pd.to_datetime(old_night)+pd.Timedelta(24*3600*1e9))[0:10]
        ...
        file_missing = np.array(data_extract_selected['fileroot'])[np.array(~file_exist)]
        
        print(' [INFO] %.0f file(s) not found on the pool of %.0f files'%(sum(~file_exist),len(file_exist)))
        if len(file_missing)>0:
            np.savetxt(directory_to_yarara+'/Files_not_founded.txt',file_missing,fmt='%s') 
            print(' [INFO] file(s) not found saved in %s'%(directory_to_yarara+'/Files_not_founded_%s.txt'%(instrument)))
        data_extract_selected = data_extract_selected.loc[file_exist].reset_index(drop=True)
        print(' [INFO] %.0f file(s) can be reduced in the pipeline'%(len(data_extract_selected)))
