...
telluric_killed = 0.20  # maximum telluric contamination allowed in relative contamination
...
if (Teff<4500)|(Teff>6500):
    print(Fore.RED + "\n[WARNING] Stellar effective temperature out of the VALD Teff range [4500,6500]"+Fore.WHITE)

print(Fore.YELLOW + "\n[INFO] Importation of the high SNR spectrum : LOADING \n" + Fore.WHITE)

spectreI, gridi, spectre_tI, spectre_rejectedI = Spectre['flux'], Spectre['wave'], Spectre['flux_telluric'], Spectre['rejected']
...
if feedback:
    ...
    radio = RadioButtons(rax, ('rectangular', 'gaussian', 'savgol'), active = [2,1][int(fwhm_ccf>15)])
...
Interpol = interp1d(gridi, spectrei, kind='cubic', bounds_error=False, fill_value='extrapolate')
spectre = Interpol(grid)

Interpol_conti = interp1d(gridi, Spectre['continuum'], kind='cubic', bounds_error=False, fill_value='extrapolate')
conti = Interpol_conti(grid)

Interpol2 = interp1d(gridi, spectre_tI, kind='cubic', bounds_error=False, fill_value='extrapolate')
spectre_t = Interpol2(grid)

if np.sum(spectre_rejectedI):
    Interpol_rejection = interp1d(gridi, spectre_rejectedI, kind='linear', bounds_error=False, fill_value='extrapolate')
...
plt.scatter(grid[matrix_index[:,0]],spectre[matrix_index[:,0]],s=10,color='r',zorder=100)
plt.scatter(grid[matrix_index[:,1]],spectre[matrix_index[:,1]],s=10,color='g',zorder=101)
plt.scatter(grid[matrix_index[:,2]],spectre[matrix_index[:,2]],s=10,color='g',zorder=102)
if feedback:
    plt.show()
if not feedback:
    plt.show(block=False)
...
HW = np.min(np.vstack([abs(matrix_wave[:,0]-matrix_wave[:,1]), abs(matrix_wave[:,0]-matrix_wave[:,2])]),axis=0)
HW_sign = np.diag(np.vstack([matrix_wave[:,0]-matrix_wave[:,1], matrix_wave[:,0]-matrix_wave[:,2]])[np.argmin(np.vstack([abs(matrix_wave[:,0]-matrix_wave[:,1]),abs(matrix_wave[:,0]-matrix_wave[:,2])]),axis=0),:])
HD = np.min(np.vstack([abs(matrix_flux[:,0]-matrix_flux[:,1]),abs(matrix_flux[:,0]-matrix_flux[:,2])]),axis=0)
Width = matrix_wave[:,2] - matrix_wave[:,1]
Depth = abs(matrix_flux[:,0] - np.mean([matrix_flux[:,1], matrix_flux[:,2]],axis=0))

def graphique_clean2(par,par2,par_name='',par_name2='', mask=False, outliers_clipping=False):
...
if feedback:
    ...
    l1, = plt.plot(np.linspace(xlim[0],xlim[1],100),np.ones(100)*0.5*(ylim[0]+ylim[1]),color='r',lw=2.5) 
    l2, = plt.plot(np.ones(100)*0.5*(xlim[0]+xlim[1]),np.linspace(ylim[0],ylim[1],100),color='r',lw=2.5) 
    l = plt.text(xlim[0]+0.2*(xlim[1]-xlim[0]),ylim[0]+0.9*(ylim[1]-ylim[0]),'Number of lines kept %.0f over %.0f'%(np.sum((t.x>0.5*(xlim[0]+xlim[1]))&(t.y>0.5*(ylim[0]+ylim[1]))),len(matrix_index)))
    ...
    class Index():   
        ...
        def update_data(self,newx,newy):
            ...
            l.set_text('Number of lines kept %.0f over %.0f'%(np.sum((t.x>self.x)&(t.y>self.y)),len(matrix_index)))
...
Dico = {'wave':matrix_wave[:,0],'wave_left':matrix_wave[:,1],'wave_right':matrix_wave[:,2], 
        'idx':matrix_index[:,0],'idx_left':matrix_index[:,1],'idx_right':matrix_index[:,2],
        'flux':matrix_flux[:,0],'line_depth':1-matrix_flux[:,0],'flux_left':matrix_flux[:,1],'flux_right':matrix_flux[:,2], 'dist_continuum':(1-np.min([matrix_flux[:,1],matrix_flux[:,2]],axis=0))/(1 - matrix_flux[:,0]),
        'width':Width,'depth_rel':Depth,'min_width':HW,'min_width_signed':HW_sign,'min_depth':HD, 'valid':mask, 'line_nb':np.arange(len(mask)),
        }
...
Dico['min_width'] = np.min(np.vstack([abs(Dico['wave']-Dico['wave_left']),abs(Dico['wave']-Dico['wave_right'])]),axis=0)
Dico['min_width_signed'] = np.diag(np.vstack([Dico['wave']-Dico['wave_left'],Dico['wave']-Dico['wave_right']])[np.argmin(np.vstack([abs(Dico['wave']-Dico['wave_left']),abs(Dico['wave']-Dico['wave_right'])]),axis=0),:])
Dico['min_depth'] = np.min(np.vstack([abs(Dico['flux']-Dico['flux_left']),abs(Dico['flux']-Dico['flux_right'])]),axis=0)
Dico['width'] = Dico['wave_right']-Dico['wave_left']
Dico['depth_rel'] = abs(Dico['flux']-np.mean([Dico['flux_left'],Dico['flux_right']],axis=0))
...
for k in range(len(i1)):
    EW.append(dwave*np.sum(abs(0.5*(spectre[i1[k]:i2[k]]+spectre[i1[k]+1:i2[k]+1])-1)))
...
i1_bis = i0-np.round(fwhm_ccf*Dico['wave']/(dwave*3e5),0).astype('int') #maximum 5kms window for weight computation 
i2_bis = i0+np.round(fwhm_ccf*Dico['wave']/(dwave*3e5),0).astype('int') #maximum 5kms window for weight computation 
...
Dico['mean_grad'] = 0.5*(abs(Dico['flux']-Dico['flux_left'])/(Dico['wave']-Dico['wave_left'])+abs(Dico['flux']-Dico['flux_right'])/(Dico['wave_right']-Dico['wave']))
...
def fit_spectral_line_core(idx_center, grid, flux, continuum, width_rv = 5, Plot=False):
    ...
    for j in tqdm(idx_center):
        ...
        std_depth = np.sqrt(abs(spectrum_line[mini1]*(1+(spectrum_line[mini1]/continuum_line[mini1])**2)))/abs(continuum_line[mini1])
        try:
        ...
        if Plot:
            ...
            plt.plot(newx+line.xmean, np.polyval(line.poly_coefficient,newx) + line.ymean,color='r')
            line.decenter()
            line.plot()
    return coordinates

coordinates  = fit_spectral_line_core(np.array(Dico['idx']), grid, spectre,np.ones(len(spectre)),width_rv=1,Plot=False) #get a visual feedback on the fit
...
for limites in excluded_region:
    Dico = Dico.loc[(Dico['freq_mask0']<limites[0])|(Dico['freq_mask0']>limites[1])]
...
Dico['tri_flux'] = (Dico['flux_left']-Dico['flux_right'])*(Dico['wave']-Dico['wave_left'])/(Dico['wave_left']-Dico['wave_right'])+Dico['flux_left'] #flux level intersection at the line center between the line linking left and right maxima  
Dico['max_depth'] = np.max(np.vstack([abs(Dico['flux']-Dico['flux_left']),abs(Dico['flux']-Dico['flux_right'])]),axis=0) #maximum difference of flux between the line center and one of the maxima
Dico['max_width'] = np.max(np.vstack([abs(Dico['wave']-Dico['wave_left']),abs(Dico['wave']-Dico['wave_right'])]),axis=0) #maximum difference of wave between the line center and one of the maxima
Dico['diff_continuum'] = abs(Dico['flux_left']-Dico['flux_right'])/Dico['max_depth'] # absolute difference of flux between the two maxima normalised by the previous parameter
Dico['diff_continuum_signed'] = (Dico['flux_left']-Dico['flux_right'])/Dico['max_depth'] # difference of flux between the two maxima normalised by the previous parameter
Dico['diff_continuum_rel'] = Dico['diff_continuum']/Dico['depth_rel']
Dico['tri_param'] = abs(Dico['tri_flux']-Dico['flux'])/Dico['max_depth'] #triangular parameter (for a line such that HW_max =HW : 1 = symetric, 0.5 = asymmetric)
...
tri_left = tri_area(Dico['wave_left'],Dico['flux_left'],Dico['wave'],Dico['flux'],Dico['wave'],Dico['tri_flux']) #triangular area formed by left maxima, tri flux, line center
tri_right = tri_area(Dico['wave_right'],Dico['flux_right'],Dico['wave'],Dico['flux'],Dico['wave'],Dico['tri_flux'])  #triangular area formed by right maxima, tri flux, line center
Dico['tri_sum'] = tri_left + tri_right # total area triangular
Dico['asym_area_tri'] = (tri_left-tri_right)/Dico['tri_sum'] # area difference normalised by the total area

# computation of the EW

Dico['win_xav'] = 0.82*8/3e5*Dico['wave']  # initial windows of 8 pixel used by Xavier
area_left = [] ; area_right = []
for j in tqdm(Dico.index):
    area_left.append(np.mean(spectre[Dico.loc[j,'idx_left']:Dico.loc[j,'idx']+1]))
    area_right.append(np.mean(spectre[Dico.loc[j,'idx']:Dico.loc[j,'idx_right']+1]))
    
area_left, area_right = np.array(area_left), np.array(area_right) # mean flux using the windows of Xavier (but the the good line center)
Dico['mean_flux'] = (area_left+area_right)/2
Dico['asym_mean_flux'] = (area_left-area_right)/Dico['mean_flux'] # difference of flux level

area_left2 = [] ; area_right2 = []
for j in tqdm(Dico.index):
    windows = np.min([Dico.loc[j,'idx_right']-Dico.loc[j,'idx'],Dico.loc[j,'idx']-Dico.loc[j,'idx_left']])
    area_left2.append(np.mean(spectre[Dico.loc[j,'idx']-windows:Dico.loc[j,'idx']+1]))
    area_right2.append(np.mean(spectre[Dico.loc[j,'idx']:Dico.loc[j,'idx']+windows+1]))
    
area_left2, area_right2 = np.array(area_left2), np.array(area_right2) # mean flux using the windows of Xavier (but the the good line center)
Dico['mean_flux2'] = (area_left2+area_right2)/2
Dico['asym_mean_flux2'] = (area_left2-area_right2)/Dico['mean_flux2'] # difference of flux level
...
if feedback:
    ...
    l1, = plt.plot(grid, myf.smooth(diff_spectrum,int(kernel_length_first_deri),shape = 'rectangular'), color='k')
    axcolor = 'whitesmoke'
    axsmoothing = plt.axes([0.2, 0.1, 0.40, 0.03], facecolor = axcolor)
    ssmoothing = Slider(axsmoothing, 'Kernel length', 1, 40, valinit = int(kernel_length_first_deri), valstep=1)
...
diff_spectrum = myf.smooth(diff_spectrum, kernel_length_first_deri, shape = kernel_shape_first_deri)

#end of the smoothing

diff_min = [] ; diff_max = [] ; contrast_deri = [] ; contrast_deri_norm = []
cm = [] ; cm_norm = [] ; cm_left = [] ; cm_right = []
for j in tqdm(Dico.index):
    ...
    contrast_deri.append((mini+maxi)/2) # mean of previous parameter (symmetric = 0)
    contrast_deri_norm.append((mini+maxi)/(2*abs(mini-maxi))) # previous parameter normalised by the absolute derivative
    mini_wave = diff_spectrum[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)].argmin()
    maxi_wave = diff_spectrum[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)].argmax()
    cm_left.append(grid[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)][mini_wave] - grid[int(Dico.loc[j,'idx'])]) # distance from line center of HDL
    cm_right.append(grid[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)][maxi_wave] - grid[int(Dico.loc[j,'idx'])]) # distance from line center of HDR   
    center_of_mass = np.mean([grid[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)][mini_wave],grid[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)][maxi_wave]])-grid[int(Dico.loc[j,'idx'])] # mean of previous parameters
    center_of_mass_norm = center_of_mass/abs(grid[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)][mini_wave]-grid[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right']+1)][maxi_wave]) # previous parameter normalised
    cm.append(center_of_mass)
    cm_norm.append(center_of_mass_norm)
    

diff_min, diff_max, contrast_deri, contrast_deri_norm = np.array(diff_min), np.array(diff_max), np.array(contrast_deri), np.array(contrast_deri_norm)
cm = np.array(cm) ; cm_norm = np.array(cm_norm) ; cm_left = np.array(cm_left) ; cm_right = np.array(cm_right)
...
if feedback:
    ...
    l1, = plt.plot(grid, myf.smooth(diff_diff_spectrum,int(kernel_length_second_deri),shape = 'rectangular'), color='k')
    axcolor = 'whitesmoke'
    axsmoothing = plt.axes([0.2, 0.1, 0.40, 0.03], facecolor = axcolor)
    ssmoothing = Slider(axsmoothing, 'Kernel length', 1, 40, valinit = int(kernel_length_second_deri), valstep=1)
    ...
    class Index():
        ...
        def update(self,val):
            smoothing = ssmoothing.val
            l1.set_ydata(myf.smooth(diff_diff_spectrum,int(smoothing),shape=self.shape))
            fig.canvas.draw_idle()            
...
diff_diff_spectrum = myf.smooth(diff_diff_spectrum, kernel_length_second_deri, shape=kernel_shape_second_deri)

#end of the smoothing

dd_min = np.where((np.in1d(np.arange(len(grid)),argrelextrema(diff_diff_spectrum,np.less)[0])&(diff_diff_spectrum<0))==True)[0]  # find all dd minima with dd<0 (clue to define the windows later)
dd_max = np.where((np.in1d(np.arange(len(grid)),argrelextrema(diff_diff_spectrum,np.greater)[0])&(diff_diff_spectrum>0))==True)[0] # find all dd maxima with dd>0 (clue to identify blends later)
...
num_dd_max = np.sum((save_diff*save_diff2)<0,axis=1) # find the number of maxima with dd>0 between the two dd minima (blend clue)
num_dd_max2 = np.sum(((dd_max-np.array(Dico['idx_right'])[:,np.newaxis])*(dd_max-np.array(Dico['idx_left'])[:,np.newaxis]))<0,axis=1)  # same as before with the two flux maxima (blend clue)
...
HW_dd = np.min(np.vstack([abs(Dico['wave']-ddwave_left),abs(Dico['wave']-ddwave_right)]),axis=0)
HW_dd_max = np.max(np.vstack([abs(Dico['wave']-ddwave_left),abs(Dico['wave']-ddwave_right)]),axis=0)
HW_dd_sign = np.diag(np.vstack([Dico['wave']-ddwave_left,Dico['wave']-ddwave_right])[np.argmin(np.vstack([abs(Dico['wave']-ddwave_left),abs(Dico['wave']-ddwave_right)]),axis=0),:])

  
nb_turnover = np.nan*np.ones(len(Dico.index)) ; mean_bis = np.nan*np.ones(len(Dico.index)) ; bis =np.nan*np.ones(len(Dico.index)) 
bis_contrast = np.nan*np.ones(len(Dico.index)) ;  mean_abs_bis = np.nan*np.ones(len(Dico.index))
slope_bis_w = np.nan*np.ones(len(Dico.index)) ; intercept_bis_w = np.nan*np.ones(len(Dico.index))
med_bis = np.nan*np.ones(len(Dico.index)) ; med_abs_bis = np.nan*np.ones(len(Dico.index))
for j in tqdm(Dico.index):
    try:
        line = myc.tableXY(grid[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right'])+1],spectre[int(Dico.loc[j,'idx_left']):int(Dico.loc[j,'idx_right'])+1])
        line.my_bisector(oversampling=5)
        bisec = myc.tableXY(line.bisector[1:,1],line.bisector[1:,0]-line.bisector[0,0],line.bisector[1:,2])
        bisec.fit_line(perm=100)
        slope_bis_w[j] = bisec.lin_slope_w # linear weighted slope of the bisector 
        intercept_bis_w[j] = bisec.lin_intercept_w # linear weighted intersection of the bisector 
        smoothed = myf.smooth(line.bisector[:,0],10) # presmoothing to compute the local maxima
        ...
        nb_turnover[j] = sum(abs(np.diff(smoothed[mask2][np.hstack([id1,id2])]))>0.002) # only keeping significant turnover
        mean_bis[j] = np.sum(line.bisector[1:,0]*line.bisector[1:,2]**-2)/np.sum(line.bisector[1:,2]**-2)-line.bisector[0,0] # weighted average of the bisector
        mean_abs_bis[j] = np.sum(abs(line.bisector[1:,0]-line.bisector[0,0])*line.bisector[1:,2]**-2)/np.sum(line.bisector[1:,2]**-2)   # weighted average of the absolute bisector   
        bis[j] = line.bisector[np.argmax(abs(line.bisector[:,0]-line.bisector[0,0])),0]-line.bisector[0,0] # bisspan
        bis_contrast[j] = np.max(-line.bisector[:,0]+line.bisector[0,0])-np.min(-line.bisector[:,0]+line.bisector[0,0]) # abs bisspan
...
Dico_new = {'min_deri':diff_min,'max_deri':diff_max,'contrast_deri':contrast_deri,'contrast_deri_norm':contrast_deri_norm,
            'cm_deri':cm,'cm_deri_norm':cm_norm,'cm_left':cm_left,'cm_right':cm_right,'nb_turn_bis':nb_turnover,
            'mean_bis':mean_bis,'bis':bis,'bis_contrast':bis_contrast,
            'mean_abs_bis':mean_abs_bis,'slope_bis_w':slope_bis_w,
            'intercept_bis_w':intercept_bis_w,'num_dd_max':num_dd_max,'num_dd_max2':num_dd_max2,'ddwave_left':ddwave_left,
            'ddwave_right':ddwave_right,'asym_ddflux':asym_ddflux,'asym_ddflux_norm':asym_ddflux_norm,'min_width_dd':HW_dd,
            'min_width_dd_signed':HW_dd_sign,'max_width_dd':HW_dd_max}
...
Dico['win_asym_left'] = np.nan ; Dico['win_asym_right'] = np.nan

w = np.array(Dico['wave']) ; win = np.array(Dico['win_mic'])
left_i = np.array(Dico['idx_left']) ; right_i = np.array(Dico['idx_right'])
...
indext = indext[mask] ; wavet = wavet[mask] ; fluxt = fluxt[mask] 

wave_tel = [] ; depth_tel = [] ; contam = []
...
wave_tel = np.array(wave_tel) ; depth_tel = np.array(depth_tel) ; contam = np.array(contam)

Dico['telluric'] = contam ; Dico['telluric_depth'] = depth_tel ; Dico['telluric_wave'] = wave_tel
...
Dico2['parameters'] = {'rv_sys':rv_sys,'berv_mean':berv,'berv_min':berv_min,'berv_max':berv_max, 
     'flux_max_telluric':flux_max_telluric,'ratio_telluric':ratio_tel,'oversampling':oversampling,
     'kernel_length_f0':kernel_smoothing_length,'kernel_length_f1':kernel_length_first_deri,'kernel_length_f2':kernel_length_second_deri,
     'kernel_f0':kernel_smoothing_shape,'kernel_f1':kernel_shape_first_deri,'kernel_f2':kernel_shape_second_deri,
     'vicinity':vicinity,'weights':weights}

Dico_backup =  Dico.copy()

myf.pickle_dump(Dico2,open(directory+'kitcat_mask_'+star+ext+'.p','wb'))
print(Fore.YELLOW + "\n[INFO] The table has been saved under : %s \n"%(directory+'kitcat_mask_'+star+ext+'.p') + Fore.WHITE)
...
loc_temp = myf.find_nearest(np.array([T[0:4] for T in all_vald_table.keys()]).astype('int'),Teff)[0][0]
vald_table = all_vald_table[list(all_vald_table.keys())[loc_temp]]
vald_table = vald_table.loc[(vald_table['wave_vald']>np.min(Dico['freq_mask0']))&(vald_table['wave_vald']<np.max(Dico['freq_mask0']))]
...
Dico['line_depth_std'] = np.sqrt(1+Dico['continuum']**2)*np.sqrt(abs(Dico['line_depth']*Dico['continuum']))/Dico['continuum']**2
...
if len(match)!=len(Dico_unblended):
    print(Fore.RED + "\n[WARNING] Problem during ther matching of VALD and KitCat, unable to correct the abundances"+Fore.WHITE)
else:
    ...
    typ_shift = [np.median(Dico_unblended['diff_vald_wave'])-1.5*myf.IQ(Dico_unblended['diff_vald_wave']),
                 np.median(Dico_unblended['diff_vald_wave'])+1.5*myf.IQ(Dico_unblended['diff_vald_wave'])]

    Dico_unblended = Dico_unblended.loc[(Dico_unblended['diff_vald_wave']<0.04)&(Dico_unblended['vald_depth']>0.02)]
    Dico_unblended = Dico_unblended.reset_index(drop=True)
    count_element = Dico_unblended['element'].value_counts()    
    element_kept = count_element.loc[count_element>=20].keys()
    size = np.where((np.array([x**2 for x in range(8)])>len(element_kept))==1)[0][0]
    if len(element_kept):
        plt.figure(figsize=(12,15))
        for idx_plot,elem in enumerate(element_kept):
            plt.subplot(size,size,idx_plot+1)
            plt.title(elem)
            vald_depth = Dico_unblended.loc[Dico_unblended['element']==elem,'vald_depth']
            my_depth = Dico_unblended.loc[Dico_unblended['element']==elem,'diff_vald_depth']
            my_depth_std = Dico_unblended.loc[Dico_unblended['element']==elem,'line_depth_std']
            ...
            plt.scatter(vald_depth,my_depth,c=Dico_unblended.loc[Dico_unblended['element']==elem,'wave'],cmap='jet',zorder=2)
            line.fit_poly(Draw=True,d=3,color='k')   
            
            vald_table.loc[vald_table['element']==elem,'depth_vald_corrected'] = vald_table.loc[vald_table['element']==elem,'depth_vald'] + np.polyval(line.poly_coefficient,vald_table.loc[vald_table['element']==elem,'depth_vald'])
            plt.scatter(vald_table.loc[vald_table['element']==elem,'depth_vald'], np.polyval(line.poly_coefficient,vald_table.loc[vald_table['element']==elem,'depth_vald']),color='k',alpha=0.2)
            plt.xlabel('VALD depth')
            plt.ylabel(r'$\Delta$ Line depth')
            plt.xlim(0,1)
            plt.ylim(-1,1)
            plt.plot([0,1],[0,0],color='gray',alpha=0.5)
        plt.subplots_adjust(top=0.95,left=0.08,right=0.96,bottom=0.10,wspace=0.45,hspace=0.55)
...
kw = ['wave_vald','depth_vald','depth_vald_corrected','element','atomic_number','log_gf','E_low','E_up','J_low','J_up','lande_low','lande_up','lande_mean','damp_rad','damp_stark','damp_waals','zeeman','ionisation_energy','mass_ion']
for keyword in kw:
    Dico[keyword] = np.nan
...
Dico['blend_crit'] = ((Dico['contam_blend']<ratio_blend)|(Dico['contam_blend']==-99.9)).astype('int')
...
myf.pickle_dump(Dico2,open(directory+'kitcat_mask_'+star+ext+'.p','wb'))
myf.pickle_dump(Dico2,open(directory+'intermediate_kitcat_mask_'+star+ext+'.p','wb'))
print(Fore.YELLOW + "\n[INFO] The table has been saved under : %s and copied in %s \n"%(directory+'kitcat_mask_'+star+ext+'.p',directory+'intermediate_kitcat_mask_'+star+ext+'.p') + Fore.WHITE)
...
if feedback:
    ...
    l1, = plt.plot(np.linspace(xlim[0],xlim[1],100),np.ones(100)*0.5*(ylim[0]+ylim[1]),color='r',lw=2.5) 
    l2, = plt.plot(np.ones(100)*0.5*(xlim[0]+xlim[1]),np.linspace(ylim[0],ylim[1],100),color='r',lw=2.5) 
    l = plt.text(xlim[0]+0.2*(xlim[1]-xlim[0]),ylim[0]+0.9*(ylim[1]-ylim[0]),'Number of lines kept %.0f over %.0f'%(np.sum((t.x<0.5*(xlim[0]+xlim[1]))&(t.y<0.5*(ylim[0]+ylim[1]))),len(Dico)))
    ...
    class Index():   
        ...
        def update_data(self,newx,newy):
            ...
            l1.set_ydata(np.ones(100)*self.y)
            l2.set_xdata(np.ones(100)*self.x)
            l.set_text('Number of lines kept %.0f over %.0f'%(np.sum((t.x<self.x)&(t.y<self.y)),len(Dico)))
            plt.gcf().canvas.draw_idle()
...
if feedback:
    ...
    l1, = plt.plot(np.linspace(xlim[0],xlim[1],100),np.ones(100)*0.5*(ylim[0]+ylim[1]),color='r',lw=2.5) 
    l2, = plt.plot(np.ones(100)*0.2*(xlim[0]+xlim[1]),np.linspace(ylim[0],ylim[1],100),color='r',lw=2.5) 
    l = plt.text(xlim[0]+0.2*(xlim[1]-xlim[0]),ylim[0]+0.9*(ylim[1]-ylim[0]),'Number of lines kept %.0f over %.0f'%(np.sum((t.x>0.2*(xlim[0]+xlim[1]))&(t.y<0.5*(ylim[0]+ylim[1]))),len(Dico)))
...
Dico_backup.loc[index_backup.astype('int'),'morpho_crit'] = np.array(mask).astype('int')
...
Dico2['parameters'] = {'rv_sys':rv_sys,'berv_mean':berv,'berv_min':berv_min,'berv_max':berv_max, 
     'flux_max_telluric':flux_max_telluric,'ratio_telluric':ratio_tel,'oversampling':oversampling,
     'kernel_length_f0':kernel_smoothing_length,'kernel_length_f1':kernel_length_first_deri,'kernel_length_f2':kernel_length_second_deri,
     'kernel_f0':kernel_smoothing_shape,'kernel_f1':kernel_shape_first_deri,'kernel_f2':kernel_shape_second_deri,'vicinity':vicinity,
     'tresh_rel_depth':rel_depth_tresh, 'tresh_max_width':max_width_tresh,
     'tresh_asym_ddflux':asym_ddflux_tresh,'tresh_diff_continuum':diff_continuum_tresh,'weights':weights}
...
if loop=='y':
    ...
    myf.pickle_dump(Dico2,open(directory+'kitcat_cleaned_mask_'+star+ext+'.p','wb'))
    ...
    np.savetxt(root+'/Python/MASK_CCF/kitcat_cleaned_'+star+ext+'.txt',mask_star)
    
    print(Fore.YELLOW + "\n[INFO] Your table is ready !\n" + Fore.WHITE)
    print(Fore.YELLOW + "\n[INFO] The table has been saved under : %s \n"%(directory+'kitcat_cleaned_mask_'+star+ext+'.p') + Fore.WHITE)
    print(Fore.YELLOW + "\n[INFO] The mask has been saved under : %s \n"%(root+'/Python/MASK_CCF/kitcat_cleaned_'+star+ext+'.txt') + Fore.WHITE)
...
if False:
    ...
    for j in np.arange(len(Dico)):
        ...
        plt.axvspan(xmin=Dico['wave_fitted'][j]-Dico['win_mic'][j],xmax=Dico['wave_fitted'][j]+Dico['win_mic'][j],color='k',alpha=0.3)
