    ...
    if len(neg) == 0:
        ...
        if (len(where) == 0) & (np.mean(neg) < len(array) / 2):
            ...
            for j in range(len(where) - 1):
                if (
                    np.mean(array[neg[where[j]] : neg[where[j + 1]] + 1])
                    < len(array) / 2
                ):
                    array[neg[where[j]] : neg[where[j + 1]] + 1] = array[
                        neg[where[j + 1]] + 1
                    ]
                else:
                    array[neg[where[j]] : neg[where[j + 1]] + 1] = array[
...
def intersect_all_continuum_sphinx(
    ...
    if nthreads >= multicpu.cpu_count():
        ...
        if (
            os.getcwd() == "/Users/cretignier/Documents/Python"
        ):  # to restablish Rassine on my computer only
            product = [
    ...
    if master_spectrum is not None:
        if copies_master == 0:
            ...
            print(
                Fore.YELLOW
                + "[WARNING] Default value of master copies fixed at %.0f."
                % (copies_master)
                + Fore.WHITE
            )
    ...
    strat = np.linspace(int(sum_mask_vert.min()), int(sum_mask_vert.max()), 10).astype(
        "int"
    )
    ...
    for j in range(len(strat) - 1)[::-1]:
        sum_mask_vert[
            (sum_mask_vert >= strat[j]) & (sum_mask_vert < strat[j + 1])
        ] = strat[j]
    ...
    for j in range(len(null) - 1):
        area.append(
            np.sum(
                border[null[j] + 1 : null[j + 1], 2]
                * border[null[j] + 1 : null[j + 1], 3]
            )
        )
        peak = np.where(
            border[null[j] + 1 : null[j + 1], 3]
            == (border[null[j] + 1 : null[j + 1], 3].max())
        )[0]
        peak = peak[border[null[j] + 1 : null[j + 1], 2][peak].argmax()]
        small_length.append(border[null[j] + 1 : null[j + 1], 2][peak])
        small_center.append(
            border[null[j] + 1 : null[j + 1], 0][peak] + small_length[-1] / 2
        )
        center2.append(
            np.median(
                all_idx[
                    (all_idx > border[null[j] + 1, 1])
                    & (all_idx < border[null[j + 1], 0])
                ]
            )
        )
    ...
    for j in range(len(stack_vert3) - 2):
        j += 1
        if (stack_vert3[j] != stack_vert3[j - 1]) | (
            stack_vert3[j] != stack_vert3[j + 1]
        ):
            liste.append(j + 1)
    ...
    class Index:
        ...
        def update(self, val):
            ...
            for j in range(len(center)):
                if center[j] != np.nan:
                    stack_vert3[
                        center[j] - windows[j] : center[j] + windows[j] + 1
                    ] = height[j]
    ...
    while type(callback.nb_clust) == str:
        print(
            "[WARNING] You have to move the sliders at least once before validating clusters"
        )
        make_sound("Warning")
        sphinx("Press ENTER to save the clusters locations (black curve)")
...
def intersect_all_continuum(names, add_new=True):
    ...
    for i, j in enumerate(names):
        ...
        if valid:
            ...
            if add_new:
                index_max, flux_max = local_max(
                    spectre, file["parameters"]["vicinity_local_max"]
                )
            ...
            outputs_interpolation_saved = file["parameters"][
...
def matching_diff_continuum_sphinx(
    ...
    (l1,) = plt.plot(
        ...
        correction,
        color="k",
        label="smoothed flux difference (flux correction)",
    )
    plt.xlabel(r"Wavelength [$\AA$]", fontsize=14)
    ...
    ssmoothing = Slider(
        axsmoothing, "Kernel length", 1, 500, valinit=savgol_window, valstep=1
    )
...
def matching_diff_continuum(
    ...
    for i, j in enumerate(names):
        ...
        try:
            valid = (
                file["matching_diff"]["parameters"]["reference_continuum"]
                != master_name
            )
        except:
            pass
...
def make_continuum(
    wave, flux, flux_denoised, grid, spectrei, continuum_to_produce=["all", "all"]
):
...
def preprocess_fits(
    ...
    if (instrument == "HARPS") | (instrument == "CORALIE") | (instrument == "HARPN"):
        ...
        for spectrum_name in files_to_process:
                ...
                if time_it > 1:
                    print(
                        ...
                        % (
                            ...
                            str(
                                int(((number_of_files - counter) * time_it) % 60)
                            ).zfill(2),
                        )
                    )
                else:
            ...
            spectre = fits.getdata(spectrum_name).astype(
                "float64"
...
def preprocess_prematch_stellar_frame(files_to_process, rv=0, dlambda=None):
    ...
    if np.max(abs(rv)) > 300:
        ...
        rep = sphinx(
            "Are you sure your RV are in km/s ? Purchase with these RV ? (y/n)",
            rep=["y", "n"],
        )
        if rep == "n":
            emergency = 0
    ...
    if emergency:
        ...
        if len(hole_left) != 0:
            hole_left_k = (
                np.min(hole_left) - 0.5
            )  # increase of 0.5 the gap limit by security
            hole_right_k = (
                np.max(hole_right) + 0.5
            )  # increase of 0.5 the gap limit by security
            make_sound("Warning")
            print(
                Fore.YELLOW
        ...
        if dlambda is None:
            ...
            if len(value) == 1:
                ...
                dlambda = sphinx(
                    "Which dlambda value are you selecting for the wavelength grid ?"
                )
...
def preprocess_match_stellar_frame(
    ...
    if args is None:
        args = preprocess_prematch_stellar_frame(
            files_to_process, rv=rv, dlambda=dlambda
        )
    ...
    wave_ref = int(
        find_nearest(np.arange(wave_min_k, wave_max_k + dlambda, dlambda), 5500)[0]
    )
    ...
    for name in files_to_process:
        ...
        mask = (wave >= (wave_min_k - dlambda / 2.0)) & (
            wave <= (wave_max_k + dlambda / 2.0)
        )
        mask2 = (wave >= (hole_left_k - dlambda / 2.0)) & (
            wave <= (hole_right_k + dlambda / 2.0)
        )
        nflux[mask2] = 0
        new_flux = nflux[mask]
        nflux_err[mask2] = 1
...
def preprocess_stack(files_to_process, bin_length=1, dbin=0, make_master=True):
    ...
    if make_master:
        ...
        master_name = "Master_spectrum_%s.p" % (
...
def postprocess_tofits(path_input, path_rassine, anchor_mode, continuum_mode):
    ...
    for i in range(len(files_input)):
        ...
        col1 = fits.Column(
            name="stellar_continuum", format="1D", array=cont_interp_Rassine
        )
        col2 = fits.Column(
            name="flux_norm",
            format="1D",
            array=data_input[1].data["flux"] / cont_interp_Rassine,
        )
        ...
        thdulist.writeto(
            files_input[i][: files_input[i].index(".fits")] + "_rassine.fits"
        )
...
def produce_line(grid, spectre, box=5, shape="savgol", vic=7):
    ...
    index = np.hstack(
        [index[:, np.newaxis], index2[0:-1, np.newaxis], index2[1:, np.newaxis]]
    )
    flux = np.hstack(
        [flux[:, np.newaxis], flux2[0:-1, np.newaxis], flux2[1:, np.newaxis]]
    )
    wave = np.hstack(
        [wave[:, np.newaxis], wave2[0:-1, np.newaxis], flux2[1:, np.newaxis]]
    )
...
def supress_ccd_gap(files_to_process, continuum="linear"):
    ...
    for i, j in enumerate(files_to_process):
        print(
            "Modification of file (%.0f/%.0f) : %s" % (i + 1, len(files_to_process), j)
        )
        file = pd.read_pickle(j)
