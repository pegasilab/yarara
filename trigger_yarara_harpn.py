...
if stage==777: #to rerun the pipeline automatically from the last step in case of a crash (however pickle may be corrupted...)
    ...
    if 'reference_spectrum_backup' in sts.material.keys():
        ...
        reference=None
        stage_break = 13
    vec = {'matching_diff':1,'matching_cosmics':2,'matching_telluric':3,'matching_oxygen':4,'matching_pca':5,'matching_activity':6,
           'matching_ghost_a':7,'matching_ghost_b':8,'matching_thar':9,'matching_fourier':10,'matching_smooth':11,'matching_mad':12,
           'matching_brute':13,'matching_morpho':24,'matching_shell':26,'matching_color':26,'matching_empca':27}
...
if stage==1:
    ...
    if sts.starname=='Sun':
        sts.yarara_correct_smooth(sub_dico='matching_diff',
                           continuum='linear',
                           reference='median',
                           window_ang = 1)
...
if stage==2:
    ...
    sts.yarara_correct_cosmics(sub_dico = sub_dico,
                               continuum = 'linear',
                               k_sigma = 5)
...
if stage==3:
    ...
    sts.yarara_correct_telluric_proxy(sub_dico = 'matching_cosmics',
...
if stage==5:
    ...
    if nb_spec:
        ...
        sts.yarara_correct_frog(correction = 'contam',
                                sub_dico = 'matching_oxygen', 
                                reference='master',
                                wave_min = 5710 , wave_max = 5840,
                                berv_shift='berv', wave_max_train=7000, pca_comp_kept=3, algo_pca='pca',
                                treshold_contam=0.5, equal_weight=True, complete_analysis=False, rcorr_min=0)
...
if stage==6:
    ...
    if reference is None:
        sts.yarara_median_master(sub_dico = None, 
                                 continuum = 'linear',
                                 method = 'mean' #if smt else than max, the classical weighted average of v1.0
                                 )
    
    sts.yarara_correct_telluric_gradient(sub_dico_detection = 'matching_cosmics',
...
if stage==7:
    ...
    sts.yarara_correct_activity(sub_dico = 'matching_pca', proxy_corr = [proxy,'ccf_fwhm','ccf_contrast'], smooth_corr=1, reference = ref)
...
if stage==8:
    ...
    sts.yarara_correct_frog(correction = 'ghost_a',
                            ...
                            reference = ref,
                            berv_shift='berv', treshold_contam=1, equal_weight=False, pca_comp_kept=3, complete_analysis=False)
...
if stage==9:
    ...
    sts.yarara_correct_frog(correction = 'ghost_b',
                            ...
                            reference = ref,
                            berv_shift='berv', wave_max_train=4100, pca_comp_kept=2, treshold_contam=1, equal_weight=True, complete_analysis=False)
...
if stage==10:
    ...
    #MEDIAN MASTER
    if reference is None:        
        sts.yarara_median_master(sub_dico = 'matching_ghost_b', 
                                 continuum = 'linear',
                                 method = 'mean', #if smt else than max, the classical weighted average of v1.0
                                 bin_berv = 10, 
                                 bin_snr = 1000,
    ...
    sts.yarara_correct_pattern(sub_dico = 'matching_ghost_b', 
                               continuum = 'linear', 
                               reference = 'master', 
                               correct_blue = True, 
                               correct_red = True,
                               width_range=[2.5,3.5],
                               jdb_range=[0,100000]) #all the time a pattern on HARPN
...
if stage==11:
    ...
    sts.yarara_correct_smooth(sub_dico='matching_fourier',
                           continuum='linear',
                           reference=ref,
                           window_ang = 5)
...
if stage==15:
    ...
    command_line = sts.yarara_preprocess_mask(sub_dico_ref='matching_cosmics', sub_dico='matching_mad', method='stack', 
...
if stage==17:
    #WB ACTIVITY
    sts.yarara_activity_index(sub_dico='matching_pca', continuum = 'linear', substract_map=['ghost_a','ghost_b','smooth','mad'])
    
    sts.yarara_wb_ca2(sub_dico = 'matching_pca', substract_proxy='CaII', optimize='both', window_extraction = None, window_core = None,  substract_map=['ghost_a','ghost_b','smooth','mad'])
    ...
    ccf_output = sts.yarara_ccf(mask = 'kitcat_cleaned_mask_'+sts.starname+'.p', plot=True, sub_dico='matching_mad', save=False, rv_range = None,  ccf_oversampling=3) 
...
if stage==20:
    ...
    sts.kitcat_statistic_lbl(sub_dico='matching_diff', clean=False, telluric_tresh=1, produce_mask=False)  
...
if stage==29:
    ...
    if nb_comp_slice:
        ...
        if DBD:
            sts.lbl_fit_vec('matching_slice', sub_dico = 'matching_mad', kw_dico = 'dbd', base_vec = base, time_detrending = 0, add_step = 1, save_database=False, col=0)
...
if stage==51: #form the database for the slice master production
    ...
    fap = 0.1
    kw_dico = 'lbl_iter'
    sub_dico1, sub_dico2 = [('matching_shell','matching_color'),('matching_color','matching_empca')][0]
    alpha_crit = 1 #1% of significance alpha
    z_crit = 0.25
    
    if sub_dico2=='matching_color':
        sts.lbl_pca(reduction='pca_scikit', sub_dico=sub_dico1, kw_dico=kw_dico, col=0, nb_comp_kept=nb_comp_color, ordering='var_lbl',
                    ext='_color', color_residues='k', contam_training=True, recenter=True, standardize=True, wave_bins=4, depth_bins=0,
                    snr_min=0.5)    
    else:
...
if stage==52:
    ...
    sts.yarara_stellar_atmos(sub_dico='matching_diff', reference='master', continuum='linear')
    sts.yarara_correct_continuum_absorption(model=None, T=None, g=None)
    sts.yarara_activity_index(sub_dico='matching_pca', continuum = 'linear', substract_map=['ghost_a','ghost_b','mad','fourier'])
...
if stage==42:
    sts.yarara_merge_instrument(instruments=['HARPS03','HARPS15','HARPN','ESPRESSO'],main_ins=None)
...
if False:     
    ...
    sts.lbl_pca(reduction='pca', sub_dico='matching_shell', kw_dico='lbl_iter', col=0, nb_comp_kept=5, nb_comp=5, ordering='var_lbl',
            ext='_color', color_residues='k', contam_training=True, recenter=True, standardize=True, wave_bins=0, depth_bins=0, 
            kernel_num=1, kernel_file='manual',nb_kernel_split=50)
    
    sts.lbl_pca(reduction='pca', sub_dico='matching_shell', kw_dico='lbl_iter', col=0, nb_comp_kept=5, ordering='var_lbl',
            ext='_color', color_residues='k', contam_training=True, recenter=True, standardize=True, wave_bins=4, depth_bins=0, kernel_num=0)

    sts.lbl_pca(reduction='pca', sub_dico='matching_shell', kw_dico='lbl_iter', col=0, nb_comp_kept=5, ordering='var_lbl',
            ext='_color', color_residues='k', contam_training=True, recenter=True, standardize=True, wave_bins=0, depth_bins=0, kernel_num=0)
    ...
    sts.yarara_lbl(kitcat=sts.dir_root+'KITCAT/kitcat_mask_'+sts.starname+'.p', calib_std=1e-3, all_dico=['matching_pca'], substract_map=['ghost','stitching','contam','mad'])
    sts.yarara_dbd(kitcat=sts.dir_root+'KITCAT/kitcat_mask_'+sts.starname+'.p', calib_std=1e-3, all_dico=['matching_pca'], substract_map=['ghost','stitching','contam','mad'] )
    sts.yarara_aba(kitcat=sts.dir_root+'KITCAT/kitcat_mask_'+sts.starname+'.p', calib_std=1e-3, all_dico=['matching_pca'], substract_map=['ghost','stitching','contam','mad'] )
    sts.yarara_wbw(kitcat=sts.dir_root+'KITCAT/kitcat_mask_'+sts.starname+'.p', calib_std=1e-3, all_dico=['matching_pca'], substract_map=['ghost','stitching','contam','mad'] )
    ...
    sts.lbl_fit_sinus_film(period = myf.my_ruler(6,400,0.05,0.1), frequency = np.linspace(1/400,1/10,2000), 
                       sub_dico=['matching_pca','matching_1y'], seasons=[3], good_morpho=False, kw_dico='lbl', col=0,
                       color_axis='K', radial_axis='r_corr', cmax=None,
                       fit_lbl=True, loop=0, delay=10, planet = [0,26,np.pi],circle=[0.0,'r','-',2], hist=72, kde=True)
