...
if liste[0] == "database":
    ...
    for ins in instrument:
        star = np.array(
            pd.read_csv(root + "/Python/Dace_summary_%s.csv" % (ins), index_col=0)[
                "name"
            ]
        )
...
if qc_check:
    ...
    if os.path.exists(root + "/Yarara/Summary_database_reduction.csv"):
        ...
        star_not_passed = np.array(
            table_qc.loc[table_qc["qc_" + qc_check] == 0, "star"]
        )
        ...
        for ins in instrument:
            print(
                " [INFO] Number of stars for %s : %.0f"
                % (ins, sum(short_liste[:, 1] == ins))
            )
...
if type_reduction == "rassine":
    for star in stars:
        print(
            "sbatch run_rassine.s %s %s %.0f %.0f %.0f %s %.0f"
            % (
                star,
                ins,
                bin_length,
    ...
    for star in stars:
        print(
            "sbatch run_yarara.s %s %s %.0f %s %.0f"
            % (star, ins, bin_length, drs_version, planet_simu)
        )
        os.system(
            "sbatch run_yarara.s %s %s %.0f %s %.0f"
            % (star, ins, bin_length, drs_version, planet_simu)
    ...
    for star in stars:
        print(
            "sbatch run_transit.s %s %s %.0f %s %.0f"
            % (star, ins, bin_length, drs_version, planet_simu)
        )
        os.system(
            "sbatch run_transit.s %s %s %.0f %s %.0f"
    ...
    for star in stars:
        print(
            "sbatch run_yarara_step.s %s %s %.0f %s %s %.0f"
            % (star, ins, bin_length, type_reduction, drs_version, planet_simu)
        )
        os.system(
            "sbatch run_yarara_step.s %s %s %.0f %s %s %.0f"
            % (star, ins, bin_length, type_reduction, drs_version, planet_simu)
