...
# stage = 26 (final graphic summary) ---------------- BEGIN OF THE TIME DOMAIN
# stage = 27 (SHELL correction)
# stage = 28 (COLOR LBL PCA correction)
# stage = 29 (LBL PCA correction)
# stage = 30 (BIS correction)
# stage = 31 (General Analysis)
# stage = 32 (l1 periodogram)
# stage = 33 (hierarchical periodogram)
# stage = 34 (light the directory)
# stage = 35 (yarara qc file creation)
#
#
# =============================================================================
...
if stage==-2:
    ...
    myr.extract_table_dace(star, instrument=[instrument], update_table=False, auto_trend=True, m=m_clipping, prefit_planet=prefit_planet, drs_version=drs_version,force_reduction=force_reduction)
...
if stage==1:
    ...
    sts.yarara_correct_borders_pxl(pixels_to_reject=np.hstack([np.arange(1,6), np.arange(9107,9112)]))
...
if nb<=15:#15
    ...
    if stage<13:
        stage = 13
        print(' [INFO] Not enough observations for the pipeline reduction, directly jump to stage 13')
...
if stage==3:
    ...
    sts.yarara_correct_oxygen_bands(sub_dico='matching_cosmics', 
                                    berv_shift = 'berv', 
...
if stage==4:
    ...
    sts.yarara_telluric(sub_dico = None, 
                        mask='telluric',
                        continuum = 'linear',
                        reference=True,
                        ratio=True,
    ...
    sts.yarara_correct_telluric_proxy(sub_dico = None,
...
if stage==6:
    ...
    sts.yarara_correct_berv_line(sub_dico = 'matching_oxygen',
                             continuum = 'linear',
                             reference = 'median',
                             jdb_range=[[0,100000,2,4]], 
                             method='weighted',wave_min=5000,wave_max=5200)
...
if stage==7:
    ...
    if reference is None:
        sts.yarara_median_master(sub_dico = 'matching_berv', 
                                 continuum = 'linear',
                                 method = 'mean' #if smt else than max, the classical weighted average of v1.0
                                 )
...
if stage==9:
    ...
    sts.yarara_ccf(mask = sts.mask_harps, plot=True, sub_dico='matching_smooth', ccf_oversampling=1, rv_range=None) 
    
    proxy = ['Kernel_CaII','CaII'][reference is None] #sts.yarara_determine_optimal_Sindex()
    print('\n Optimal proxy of activity : %s'%(proxy))
    
    sts.yarara_correct_activity(sub_dico = 'matching_smooth', 
                                reference = ref,
...
if stage==10:
    ...
    #MEDIAN MASTER
    if reference is None:        
        sts.yarara_median_master(sub_dico = 'matching_activity', 
                                 continuum = 'linear',
                                 method = 'mean', #if smt else than max, the classical weighted average of v1.0
                                 bin_berv = 10, 
                                 bin_snr = 1000,
    ...
    for pattern_freq in [[2,2.5],[3,3.5],[6.3,6.6],[2,2.5],[3,3.5],[6.3,6.6]]:
        #CORRECT PATTERN IN FOURIER SPACE
        sts.yarara_correct_pattern(sub_dico = sub_dico, 
                                   continuum = 'linear', 
                                   reference = 'master', 
                                   correct_blue = True, 
                                   correct_red = True,
                                   width_range=pattern_freq,
                                   jdb_range=[0,100000]) #all the time a pattern on ESPRESSO
...
if stage==11:
    #CORRECT MAD
    sts.yarara_correct_mad(sub_dico='matching_fourier',
                           continuum='linear',
                           k_sigma = 2,
                           k_mad = 2,
                           n_iter = 1,
...
if stage==13:
    ...
    sts.yarara_median_master(sub_dico = sub_dico, 
                             continuum = 'linear',
...
if stage==14:
    ...
    command_line = sts.yarara_preprocess_mask(sub_dico_ref='matching_diff', sub_dico=sub_dico, method='stack', 
                                              plot=True, template_telluric=template_telluric, shift_spectrum=True, fixed_berv_span=fixed_berv_span)
...
if stage==15:
    ...
    sts.yarara_vsin(fwhm_inst=1.25,vmicro=None)
...
if stage==16:
    #WB ACTIVITY
    sts.yarara_activity_index(sub_dico='matching_smooth', continuum = 'linear', substract_map=['mad'])
    
    sts.yarara_wb_ca2(sub_dico = 'matching_smooth', substract_proxy='CaII', optimize='both', window_extraction = None, window_core = None,  substract_map=[])
...
if stage==25:
    ...
    sts.yarara_plot_all_rv('CCF_kitcat_mask_'+sts.starname, 'matching_morpho', ofac=10, photon_noise=0.4, deg_detrending=2, p_min=0, method='iterative')
...
if stage==32:
    ...
    for name,v in zip(['drs','yarara_v1_lbl','yarara_v2_lbl','yarara_shell','yarara_color'],[vec_ref, vec_lbl, vec_lbl2, vec_shell, vec_color]):
        sts.periodogram_l1(v, name_ext='_'+name, photon_noise=np.min([0.4,0.75*np.round(vec_lbl2.rms,1)]), sort_val='period', p_min=2, fap_min=-2, species=sts.table['ins'])
    ...
    for name,v in zip(['drs','yarara_v1_lbl','yarara_v2_lbl','yarara_shell','yarara_color'],[vec_ref, vec_lbl, vec_lbl2, vec_shell, vec_color]):
        sts.periodogram_kep(v, photon_noise=0.3, jitter=0.4, fap=1, name_ext='_'+name, deg=1, p_min=2, periods=list(v.l1_table['period']), name_pre='l1_', fit_ecc=True, species=sts.table['ins'], periodic=0.25)
...
if stage==33:
    ...
    for name,v in zip(['drs','yarara_v1_lbl','yarara_v2_lbl','yarara_shell','yarara_color'],[vec_ref, vec_lbl, vec_lbl2, vec_shell, vec_color]):
        sts.periodogram_kep(v, photon_noise=0.3, jitter=0.4, fap=1, name_ext='_'+name, deg=1, p_min=2, fit_ecc=False, species=sts.table['ins'], periodic=0.25)
    ...
    #     for dico, name, v in zip([sb_dico,'matching_empca','matching_shell3','matching_shell5','matching_shell7'],['yarara_v1_lbl','yarara_v2_lbl','yarara_shell3','yarara_shell5','yarara_shell7'],[vec_lbl, vec_lbl2, vec_shell, vec_shell2, vec_shell3]):
    #         sts.keplerian_phase_test(v, sub_dico=dico, kw_dico='lbl_iter', nb_planet=3, nb_cut=3, deg=0, deg_detrend=0, photon_noise=0.4, name_ext='_'+name, periods=signal_periods)  
    ...
    if sts.planet:
        ...
        for name,v in zip(['drs','yarara_v1_lbl','yarara_v2_lbl','yarara_shell','yarara_color'],[vec_ref, vec_lbl, vec_lbl2, vec_shell, vec_color]):
            sts.periodogram_kep(v, photon_noise=0.3, jitter=0.4, fap=1, name_ext='_'+name, deg=0, p_min=2, periods=list(true_p), name_pre='injected_', fit_ecc=False, species=sts.table['ins'], periodic=0.25)
...
if stage==53:
    ...
    sts.yarara_correct_continuum_absorption(model=None, T=None, g=None)
    sts.yarara_activity_index(sub_dico='matching_pca', continuum = 'linear', substract_map=['mad','smooth'])
...
if stage==69:
    ...
    sts.simu_detection_limit2(vec_lbl2, ext='_yarara_v2_lbl', photon_noise = 0.4, degree_detrending=2,
    ...
    sts.simu_detection_limit2(vec_ref, ext='_drs', photon_noise = 0.4, degree_detrending=2,
                            nb_phase = 36, fap=0.99,
                            k = np.arange(0.2, 5.1, 0.2), 
                            p = myf.my_ruler(2.9, 2000, 0.1, 50),
                            tresh_diff=0.1, tresh_phase=0.2,
...
if False:     
    ...
    sts.lbl_fit_sinus(36.5, 
                       sub_dico='matching_', season=3, good_morpho=False, kw_dico='dbd', col=0, deg=0,
    ...
    sts.lbl_fit_sinus_film(period = myf.my_ruler(6,400,0.05,0.1), frequency = np.linspace(1/400,1/10,2000), 
                       sub_dico=['matching_pca','matching_morpho'], seasons=[3], good_morpho=False, kw_dico='lbl', col=0,
                       color_axis='K', radial_axis='r_corr', cmax=None, 
