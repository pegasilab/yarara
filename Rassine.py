...
if spectrum_name.split(".")[-1] == "fits":  # to load a fits file
    ...
    spectrei = fits.getdata(spectrum_name).astype(
        "float64"
    )  # the flux of your spectrum
    grid = np.linspace(
        header["CRVAL1"],
        header["CRVAL1"] + (len(spectrei) - 1) * spectre_step,
        len(spectrei),
    ...
    if spectrum_name.split(".")[-1] == "csv":
        ...
        spectrei = np.array(data[column_flux])  # the flux of your spectrum
        try:
            spectrei_err = np.array(
                data[column_flux + "_err"]
        ...
        try:
            grid = np.array(
                data[column_wave]
            )  # the grid of wavelength of your spectrum
        except:
            grid = ras.create_grid(
                data["wave_min"], data["dwave"], len(data[column_flux])
            )
    ...
    elif (spectrum_name.split(".")[-1] == "txt") | (
        spectrum_name.split(".")[-1] == "rdb"
    ):
...
if type(par_stretching) != str:
    ...
    if (float(par_stretching.split("_")[1]) > 1) | (
        float(par_stretching.split("_")[1]) < 0
    ):
...
normalisation = float(len_y) / float(
    len_x
)  # stretch the y axis to scale the x and y axis
spectre = spectrei / normalisation

if synthetic_spectrum:
    spectre += (
        np.random.randn(len(spectre))
        * 1e-5
        * np.min(np.diff(spectre)[np.diff(spectre) != 0])
...
subset = np.sort(
    np.random.choice(
        np.arange(len(spectre)), size=int(len(spectre) / speedup), replace=False
    )
...
for iteration in range(5):  # k-sigma clipping 5 times
    ...
    if speedup > 1:
        ...
        sup_fast = interp1d(
            subset, sup_fast, bounds_error=False, fill_value="extrapolate"
        )(np.arange(len(spectre)))
        Q2_fast = interp1d(
            subset, Q2_fast, bounds_error=False, fill_value="extrapolate"
        )(np.arange(len(spectre)))
...
if par_fwhm == "auto":
    ...
    continuum_left[np.isnan(continuum_left)] = continuum_left[
        ~np.isnan(continuum_left)
    ][-1]
    ...
    if CCF_mask != "master":
        ...
        line_center = ras.doppler_r(
            0.5 * (mask_harps[:, 0] + mask_harps[:, 1]), RV_sys
        )[0]
    ...
    if not only_print_end:
        print(
            " [AUTO] FWHM computed from the CCF is about : %.2f [km/s]"
            % (popt[-1] * 2.35)
        )
...
if (feedback) & (par_smoothing_box != "auto"):
    ...
    if par_smoothing_box == "auto":
        ...
        if par_smoothing_kernel == "erf":
            alpha1 = np.exp(
                np.polyval(
                    np.array([0.00210819, -0.04581559, 0.49444111, -1.78135102]),
                    np.log(SNR_0),
                )
            ...
            alpha1 = np.exp(
                np.polyval(
                    np.array([0.01155214, -0.20085361, 1.34901688, -3.63863408]),
                    np.log(SNR_0),
                )
            )  # using the calibration curve calibration
        ...
        spectre_backup = spectre.copy()
        spectre = ras.smooth(
            spectre, int(par_smoothing_box), shape=par_smoothing_kernel
        )
        ...
        mask_out = np.unique(
            mask_out
            + np.arange(-smoothing_length, smoothing_length + 1, 1)[:, np.newaxis]
        )
        mask_out = mask_out[(mask_out >= 0) & (mask_out < len(grid))]
        spectre[mask_out.astype("int")] = spectre_backup[
            mask_out.astype("int")
...
big_windows = (
    100.0  # 100 typical line width scale (large window for the second continuum)
)
iteration = 5
reg = par_reg_nu
...
if out_of_calibration:
    ...
    big_windows = (
        20.0  # 20typical line width scale (large window for the second continuum)
    )
...
if (par_Rmax != par_R) | (par_Rmax == "auto"):
    ...
    if speedup > 1:
        sup_large = interp1d(
            subset, sup_large, bounds_error=False, fill_value="extrapolate"
        )(np.arange(len(spectre)))
    ...
    if speedup > 1:
        sup_small = interp1d(
            subset, sup_small, bounds_error=False, fill_value="extrapolate"
        )(np.arange(len(spectre)))
    ...
    if par_Rmax == "auto":
        while (loop) & (threshold > 0.2):
            ...
            cluster_length = np.hstack(
                [cluster_broad_line, penality_cluster[:, np.newaxis]]
            )
            ...
            for j in range(len(cluster_length)):
                cluster_length[j, 4] = np.nanpercentile(
                    abs(
                        np.diff(
                            spectre[
                                int(cluster_length[j, 0]) : int(cluster_length[j, 1])
                            ]
                        )
                    ),
                    10,
    ...
    if feedback:
        ...
        radius = (
            ...
            * (
                par_R
                + (par_Rmax - par_R) * penalite_step ** (float(reg.split("_")[-1]))
            )
        )
        ...
        radius2 = (
            ...
            * (
                par_R
                + (par_Rmax - par_R)
                * (1 + np.exp(-10 * float(reg.split("_")[-1]) * (penalite_step - ini)))
                ** -1
            )
        )
        law2 = (
            par_R
            + (par_Rmax - par_R)
            * (1 + np.exp(-10 * float(reg.split("_")[-1]) * (t - ini))) ** -1
        )
            ...
            def update(self, val):
                ...
                if self.model == "poly":
                    ...
                    radius = (
                        grid
                        / minx
                        * (par_R + (par_Rmax - par_R) * penalite_step ** (expo))
                    )
                    l1.set_ydata(radius)
        ...
        if callback.model == "poly":
            radius = law_chromatic * (
                par_R + (par_Rmax - par_R) * penalite_graph ** (expo)
            )
            ...
            radius = law_chromatic * (
                par_R
                + (par_Rmax - par_R)
                * (1 + np.exp(-10 * expo * (penalite_graph - expo2))) ** -1
            )
        ...
        if reg.split("_")[0] == "poly":
            ...
            radius = law_chromatic * (
                par_R
                + (par_Rmax - par_R)
                * (1 + np.exp(-width * (penalite_graph - center))) ** -1
            )
            par_model = reg
        else:
            if not only_print_end:
...
while loop == "y":
    ...
    while len(wave) - j > 3:
        ...
        cond1 = (cy - p1[1]) >= 0
        thetas = cond1 * (-1 * np.arccos((cx - p1[0]) / par_R) + np.pi) + (
            1 - 1 * cond1
    ...
    if feedback:
        ...
        distance = np.sign(waves) * np.sqrt(
            (waves) ** 2 + (flux - flux[:, np.newaxis]) ** 2
        )
        distance[distance < 0] = 0
        numero = np.arange(len(distance)).astype("int")
        count_iter += 1
...
if feedback:
    ...
    continuum = ras.troncated(continuum, spectre)
    (l9,) = plt.plot(
        grid, continuum, color="k", ls=":", alpha=1, label="updated continuum"
    )
    plt.legend(loc=1)
    plt.show(block=False)
    ...
    class Index(object):
        ...
        def update(self, event):
            ...
            y1, y2 = np.min(
                flux[sort_val[0] - win_grap1 : sort_val[0] + win_grap2]
            ), np.max(flux[sort_val[0] - win_grap1 : sort_val[0] + win_grap2])
        ...
        def backup(self, event):
            if len(self.wave_backup) > 1:
                ...
                win_grap2 = np.min([len(wave) - sort_val[0], win_grap]) - 1
                ax.set_xlim(
                    wave[sort_val[0] - win_grap1], wave[sort_val[0] + win_grap2]
                )
                y1, y2 = np.min(
                    flux[sort_val[0] - win_grap1 : sort_val[0] + win_grap2]
                ), np.max(flux[sort_val[0] - win_grap1 : sort_val[0] + win_grap2])
        ...
        def next_line(self, event):
            if self.ind < lines_showed - 1:
                ...
                lvline.set_xdata(x=grid[center])
                ax.set_xlim(
                    wave[sort_val[self.ind] - win_grap],
                    wave[sort_val[self.ind] + win_grap + 1],
                )
                y1, y2 = np.min(
                    flux[
                        sort_val[self.ind]
                        - win_grap : sort_val[self.ind]
                        + win_grap
                        + 1
                    ]
                ), np.max(
...
for j in range(5):
    ...
    mask_out_idx = list(
        mask_out_idx[(mask_out_idx > 3) & (mask_out_idx < (len(wave) - 3))]
    )
    mask_out_idx2 = []
...
if (feedback) | (plot_end) | (save_last_plot):
    ...
    plt.plot(
        grid[::jump_point], spectrei[::jump_point] / conti[::jump_point], color="k"
    )
    ...
    if feedback:
        plt.show(block=False)
        loop = ras.sphinx(
            "Press Enter to finish the execution and save the final product"
        )
        plt.close()
    else:
        plt.close()
