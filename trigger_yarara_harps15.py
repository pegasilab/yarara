...
if stage == 1:
    ...
    sts.import_material()
    cut = np.min(sts.material["wave"]) + 0.5 * (
        np.max(sts.material["wave"]) - np.min(sts.material["wave"])
    )
...
if stage == 2:
    ...
    if reference is None:
        ...
        sts.yarara_median_master(
            sub_dico="matching_cosmics", continuum="linear", method="mean"
        )
...
if stage == 3:
    ...
    if False:  # no fringing correction on HARPS15
        if reference is None:
            ...
            sts.yarara_telluric(
                sub_dico="matching_cosmics", continuum="linear", reference="norm"
            )
...
if stage == 12:
    ...
    sts.yarara_correct_smooth(
        sub_dico="matching_thar", continuum="linear", reference=ref, window_ang=5
    )
...
if stage == 34:
    ...
    for name, v in zip(
        ...
        sts.periodogram_l1(
            v,
            name_ext="_" + name,
            photon_noise=np.min([1.3, 0.75 * np.round(vec_lbl2.rms, 1)]),
            sort_val="period",
            p_min=2,
            fap_min=-2,
