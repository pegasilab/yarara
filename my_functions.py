...
all_alias = np.array([-2-1/365.25, 2+1/365.25,
                       -2, 2,
                       -1-1/365.25, 1+1/365.25,
                       -1, 1,
                       -1/365.25, 1/365.25,
                       -1/(2*365.25), 1/(2*365.25)])
...
def optimal_plot_axis(x,y,percent=10):
    ...
    return (np.min(x)-percent*int_x, np.max(x)+percent*int_x), (np.min(y)-percent*int_y, np.max(y)+percent*int_y)
...
def polar_err(phi,r,phi_err,r_err, color='k',marker='o'):
    plt.errorbar(phi*np.pi/180, r,yerr=r_err, capsize=0, color=color,marker=marker)
...
def detect_outliers_horn(array,Draw=False):
    ...
    IQ = Q3-Q1
    mask_outliers = (transformed_array>Q3+1.5*IQ)|(transformed_array<Q1-1.5*IQ)
...
def maximum2d(spectre,vicinity):
    ...
    for j in tqdm(range(np.shape(spectre)[0]-vicinity-1)):
        for i in range(np.shape(spectre)[1]-vicinity-1):
            ...
            maxi = np.product(spectre[j+vicinity//2,i+vicinity//2]>=spectre[j:j+vicinity+1,i:i+vicinity+1])
...
def smooth2d(y,box_pts,borders=True,mode='same'):
    if box_pts>1:
        ...
        if borders:
            ...
            y_smooth[:,0:int(box_pts/2+1)] = y_smooth[:,int(box_pts/2+2)][:,np.newaxis]
            y_smooth[:,-int(box_pts/2+1):] = y_smooth[:,-int(box_pts/2+2)][:,np.newaxis]
        else:
...
def plot_warmup():
    CCD_warmups = ['2014-06-18','2014-10-17','2015-02-03','2015-02-19','2015-02-23',
                '2015-05-19', '2015-10-13', '2016-03-31', '2016-10-26',
                 '2017-04-11', '2017-11-14', '2018-04-23', '2018-10-21',
                 '2019-03-12', '2019-07-19', '2019-12-22', '2020-05-28']
    
    focus_change = ['2014-03-22', '2020-02-19', '2020-06-01']

    CCD_warmups_jdb = np.array([Time.Time(i, format='iso').jd-2400000 for i in CCD_warmups])
    focus_jdb = np.array([Time.Time(i, format='iso').jd-2400000 for i in focus_change])
...
def plot_TH_changed(color='b',ls='-',alpha=1,time=None):
    ...
    focus_change = ['2003-10-30', '2007-07-31', '2012-08-29', '2015-10-07', '2018-11-28']
...
def import_dace_keplerian(filename, Mstar=1):
    ...
    parameters = np.vstack([parameters,(Mstar*(periods/365.25)**2)**(1/3),mass,0*mass])
    
    dico = {k:a for k,a in zip(['p','k','e','peri','long','a','mass','i'],parameters)}
    dico_err_inf = {k:a for k,a in zip(['p_std_inf','k_std_inf','e_std_inf','peri_std_inf','long_std_inf'],parameters_err)}
    dico_err_sup = {k:a for k,a in zip(['p_std_sup','k_std_sup','e_std_sup','peri_std_sup','long_std_sup'],parameters_err)}
...
def import_dace_mcmc_gael(filename,sigma=1):
    ...
    for begin,end,section in zip(loc[:-1],loc[1:],sections):
        if (section != 'Star') & (section != 'Offset') & (section != 'Drift') & (section != 'Noise') & (section != 'Activity cycle'):
            nb_planet+=1
    ...
    for i,section in enumerate(sections):
        paragraph,name_parameters,parameters,parameters_err,parameters_err_inf,parameters_err_sup = all_lines[i]
        ...
        if section == 'Star':
            ...
            dico_err_temp = {k:[a] for k,a in zip([n+'_std' for n in name],parameters_err)}
            dico_err_sup_temp = {k:[a] for k,a in zip([n+'_std_sup' for n in name],parameters_err_sup)}
            dico_err_inf_temp = {k:[a] for k,a in zip([n+'_std_inf' for n in name],parameters_err_inf)}
            cols += name
        elif section == 'Offset':
            name = ['offset_'+i.split('\gamma_{\mathrm{')[1].split('}')[0] for i in name_parameters]
            dico_temp = {k:[a] for k,a in zip(name,parameters)}
            dico_err_temp = {k:[a] for k,a in zip([n+'_std' for n in name],parameters_err)}
            dico_err_sup_temp = {k:[a] for k,a in zip([n+'_std_sup' for n in name],parameters_err_sup)}
            dico_err_inf_temp = {k:[a] for k,a in zip([n+'_std_inf' for n in name],parameters_err_inf)}
            ...
            name = ['noise_'+i.split('\sigma_{\mathrm{')[1].split('}')[0] for i in name_parameters]
            dico_temp = {k:[a] for k,a in zip(name,parameters)}
            dico_err_temp = {k:[a] for k,a in zip([n+'_std' for n in name],parameters_err)}
            dico_err_sup_temp = {k:[a] for k,a in zip([n+'_std_sup' for n in name],parameters_err_sup)}
            dico_err_inf_temp = {k:[a] for k,a in zip([n+'_std_inf' for n in name],parameters_err_inf)}
            ...
            if '\log P' in paragraph[0]:        
                ...
                dico_err_sup_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','logP_std_sup','logK_std_sup','sqrt(e)cosw_std_sup','sqrt(e)sinw_std_sup','L0_std_sup','a_s_std_sup','a_std_sup','e_std_sup','K_std_sup','w_std_sup','m_p_std_sup','P_std_sup','Tc_std_sup','Tp_std_sup'],np.append(np.array([pla_name[pla_itr]]),parameters_err_sup))})
                dico_err_inf_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','logP_std_inf','logK_std_inf','sqrt(e)cosw_std_inf','sqrt(e)sinw_std_inf','L0_std_inf','a_s_std_inf','a_std_inf','e_std_inf','K_std_inf','w_std_inf','m_p_std_inf','P_std_inf','Tc_std_inf','Tp_std_inf'],np.append(np.array([pla_name[pla_itr]]),parameters_err_inf))})
                cols_kep = ['planet','P','K','e','w','Tp','a','m_p','a_s','Tc','logP','logK','sqrt(e)cosw','sqrt(e)sinw','L0']
                ...
                dico_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','L0','a_s','a','e','K','w','m_p','P','Tc','Tp'],np.append(np.array([pla_name[pla_itr]]),parameters))})
                dico_err_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','L0_std','a_s_std','a_std','e_std','K_std','w_std','m_p_std','P_std','Tc_std','Tp_std'],np.append(np.array([pla_name[pla_itr]]),parameters_err))})
                dico_err_sup_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','L0_std_sup','a_s_std_sup','a_std_sup','e_std_sup','K_std_sup','w_std_sup','m_p_std_sup','P_std_sup','Tc_std_sup','Tp_std_sup'],np.append(np.array([pla_name[pla_itr]]),parameters_err_sup))})
                dico_err_inf_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','L0_std_inf','a_s_std_inf','a_std_inf','e_std_inf','K_std_inf','w_std_inf','m_p_std_inf','P_std_inf','Tc_std_inf','Tp_std_inf'],np.append(np.array([pla_name[pla_itr]]),parameters_err_inf))})
                cols_kep = ['planet','L0','P','K','e','w','Tp','a','m_p','a_s','Tc']
                ...
                dico_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','P','K','e','w','L0','a_s','a','m_p','Tc','Tp'],np.append(np.array([pla_name[pla_itr]]),parameters))})
                dico_err_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','P_std','K_std','e_std','w_std','L0_std','a_s_std','a_std','m_p_std','Tc_std','Tp_std'],np.append(np.array([pla_name[pla_itr]]),parameters_err))})
                dico_err_sup_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','P_std_sup','K_std_sup','e_std_sup','w_std_sup','L0_std_sup','a_s_std_sup','a_std_sup','m_p_std_sup','Tc_std_sup','Tp_std_sup'],np.append(np.array([pla_name[pla_itr]]),parameters_err_sup))})
                dico_err_inf_temp = pd.DataFrame({k:[a] for k,a in zip(['planet','P_std_inf','K_std_inf','e_std_inf','w_std_inf','L0_std_inf','a_s_std_inf','a_std_inf','m_p_std_inf','Tc_std_inf','Tp_std_inf'],np.append(np.array([pla_name[pla_itr]]),parameters_err_inf))})
                cols_kep = ['planet','P','K','e','w','L0','a','a_s','m_p','Tc','Tp']
    ...
    dico_kep = pd.concat([dico_kep, dico_kep_err, dico_kep_err_sup, dico_kep_err_inf],axis=1)
    idxtot = []
    for i in range(len(cols_kep)):
        idxtot.extend([i,i+len(cols_kep),i+2*len(cols_kep),i+3*len(cols_kep)])
...
def import_dace_mcmc(filename):
    ...
    dico = {k:a for k,a in zip(['p','k','e','peri','long','a','mass','tc','i'],parameters)}
    dico_err_sup = {k:a for k,a in zip(['p_std_sup','k_std_sup','e_std_sup','peri_std_sup','long_std_sup','a_std_sup','mass_std_sup','tc_std_sup'],parameters_err_sup)}
    dico_err_inf = {k:a for k,a in zip(['p_std_inf','k_std_inf','e_std_inf','peri_std_inf','long_std_inf','a_std_inf','mass_std_inf','tc_std_inf'],parameters_err_inf)}
...
def read_str_angle(array):
    ...
    for i in array:
        if type(i)==str:
            val = [np.float(j)/60**k for k,j in zip(np.arange(len(i.split(' '))),i.split(' '))]
...
def average_weight(array, weights, axis=0):
    if axis==1:
        array=array.T ; weights=weights.T
    new_array = [] ; new_weights =  []
    for j in range(np.shape(array)[0]):
        if np.sum(weights[:, j])==0:
            ...
            new_array.append(np.sum(array[:, j]*weights[:, j])/np.sum(weights[:, j]))
    new_array = np.array(new_array) ;  new_weights = np.array(new_weights) 
...
def plot_weight(y,yerr='none',normed=False,bin_method='scott',alpha=0.6,color='b'):
    ...
    if type(yerr)!=str:
        ...
        for j in range(len(val[1])-1):
            up = val[1][j+1]
            low = val[1][j]
            errors.append(np.sum((norm.cdf(up,y,yerr)-norm.cdf(low,y,yerr))*(1-(norm.cdf(up,y,yerr)-norm.cdf(low,y,yerr)))))
            mean.append(np.sum(norm.cdf(up,y,yerr)-norm.cdf(low,y,yerr)))
    else:
        errors = np.sqrt(val[0]) ; mean=0
    normalisation = 1
    if normed:
        val2 = astrohist(y, bins=bin_method, color=color, alpha=alpha, ec='black', density=True)
        normalisation = np.nanmedian(val[0]/val2[0])
        plt.ylim(0,np.max(val[0]/normalisation)+0.25*(np.max(val[0]/normalisation)))
    errors = np.array(errors)
    mean = np.array(mean)
    plt.errorbar(np.diff(val[1])/2+val[1][0:-1],val[0]/normalisation,yerr=errors/normalisation,color='k')
    return np.diff(val[1])/2+val[1][0:-1], val[0]/normalisation, errors/normalisation 
...
def supSigma(array, axis=None):
    return np.nanpercentile(array, 84, axis=axis)-np.nanpercentile(array, 50, axis=axis)
    
def infSigma(array, axis=None):
    return np.nanpercentile(array, 50, axis=axis)-np.nanpercentile(array, 16, axis=axis)
...
def grid_binning_slow(gridx,gridy,vecx,vecy,vecz,Draw=True,cmap='plasma'):
    ...
    all_val = [np.ravel(x),np.ravel(y),np.ravel(x),np.ravel(y),np.ravel(z),np.ravel(z),np.ravel(nb)]
    
    all_val = np.array(all_val).T
    limite = np.nanpercentile(all_val[all_val[:,-1]!=0,-1],5)
    table_flat = pd.DataFrame(all_val,columns=['x','y','binx','biny','binz','binzerr','nb'])
    ...
    if Draw:
        ...
        my_colormesh(np.unique(table_flat['x']),np.unique(table_flat['y']),z,vmin=vmin,vmax=vmax,cmap=cmap)
    return table_flat,table    

def grid_binning(gridx,gridy,vecx,vecy,vecz,veczerr=None,Draw=True,cmap='plasma'):
    ...
    all_val = [np.ravel(x),np.ravel(y),np.ravel(medx),np.ravel(medy),np.ravel(z),np.ravel(zerr),np.ravel(nb)]
    
    all_val = np.array(all_val).T
    limite = np.nanpercentile(all_val[all_val[:,-1]!=0,-1],5)
    #print('Minimum number of element required : ',limite)
    table_flat = pd.DataFrame(all_val,columns=['x','y','binx','biny','binz','binzerr','nb'])
    table_flat.loc[table_flat['nb']<=limite,'binz'] = median #remove the 5percent distribution with lowest count for the statistic
...
def poly2D_basis(x, y, deg=3, basis='legendre', Draw=False):
    ...
    if Draw:
        ...
        plt.subplots_adjust(left=0.08,right=0.97,top=0.96,bottom=0.08,hspace=0.3,wspace=0.3)
        plt.figure()
...
def poly2D_zernike_square(x2,y2,params):
    ...
    x -= np.nanmin(x) ; y -= np.nanmin(y)        
    x /= np.nanmax(x)/np.sqrt(np.pi) ; y /= np.nanmax(y)/np.sqrt(np.pi)
    x -= np.sqrt(np.pi)/2 ; y -= np.sqrt(np.pi)/2        
    ...
    z15 = (13 * x**4) + 4.39 * (-1.44 - 2.16 * y**2) * x**2 - (5.97 * y**2) + (12.6 * y ** 4) + 0.71
    z16 = (18.1 * x**4 * y) - 36.2 * (x**2 * y**3) + (3.62 * y**5) - (2.92 * y) + (4.81 * x**2 * y) + (6.23 * y**3)
    z17 = (11.1 * x**5) - 22.2 * (x**3 * y**2) - (3.84 * x**3) + (33 * y**2 * x) - (33.3 * x * y**4) - (2.9 * x)
    z18 = (7.21 * y) - 29.2 * (x**2 * y) - (24.6 * y**3) + (32.1 * x**4 * y) + (16.8 * x**2 * y**3) + (22.7 * y**5)
    #z19 = (7.9 * x) - (3.72 * x**3) - (6.77 * x * y**2) + (34.8 * x**5) + (16.4 * x**3 * y**2) - (8.4 * x * y**4) #function not ortthogonal
    z19 = (8.36 * (x*f)) - (42.6 * (x*f)**3) - (11 * (x*f) * (y/f)**2) + (43.2 * (x*f)**5) + (39.1 * (x*f)**3 * (y/f)**2) - (3.76 * (x*f) * (y/f)**4)
    #z19 = (7.55 * (x/f)) - (28.1 * (x/f)**3) - (11.2 * (x/f) * (y*f)**2) + (21.5 * (x/f)**5) + (20.1 * (x/f)**3 * (y*f)**2) - (1.58 * (x/f) * (y*f)**4)
    z20 = (-11.6 * x**2 * y) + 37 * (y**3) + (14.2 * x**4 * y) + (3.75 * x**2 * y**3) - (42.9 * y**5) - (5.22 * y)
    z21 = (30 * x**5) - (21.4 * x**3 * y**2) + (24.8 * x * y**4) + (3.07 * x) - (20.6 * x**3) - (6.59 * y**2 * x)
    
    z = [z1,z2,z3,z4,z5,z6,z7,z8,z9,z10,z11,z12,z13,z14,z15,z16,z17,z18,z19,z20,z21]
...
def make_poly2D(n,m,maxi=False):
    ...
    matrix_all = np.reshape(np.arange((n+1)*(m+1)),(m+1,n+1)).astype('float')
...
def my_colormesh(x,y,z,cmap='seismic',vmin=None,vmax=None,zoom=1,shading='auto', return_output=False, order=3, smooth_box=1):
...
def ccf2(wave,spec1,wave2,spec2,rv_max=30):
    ...
    for j in tqdm(all_shift):
        new_spec = interp1d(wave2,np.roll(spec2,j), kind='linear', bounds_error=False, fill_value='extrapolate')(wave)
...
def flux_norm_std(flux, flux_std, continuum, continuum_std):
    ...
    flux_norm_std = np.sqrt((flux_std/continuum)**2+(flux*continuum_std/continuum**2)**2)
    mask = flux_norm_std>flux_norm
    flux_norm_std[mask] = abs(flux_norm[mask]) #impossible to get larger error than the point value
...
def gauss_vincent(x, cen, cen2, amp, amp2, offset, wid, wid2):
    return amp * np.exp(-0.5*(x-cen)**2 / (2*wid**2)) + amp2 * np.exp(-0.5*(x-cen2)**2 / (2*wid2**2)) + offset
...
def gen_gaus(grid,number,xmin,xmax,sig_min,sig_max,amp_min,amp_max):
    ...
    for j in range(number):
        synthetic*= (1-rand_unif(amp_min,amp_max)*gaus(grid,rand_unif(xmin,xmax),rand_unif(sig_min,sig_max),norm=True))
    return synthetic
...
def ratio_line(l1, l2, grid, spectrei, continuum, window=3):
    ...
    subgrid1 = grid[l1-window:l1+window+1] ; subgrid2 = grid[l2-window:l2+window+1] 
    subspectre1 = spectrei[l1-window:l1+window+1] ; subspectre2 = spectrei[l2-window:l2+window+1] 
    subcont1 = continuum[l1-window:l1+window+1] ; subcont2 = continuum[l2-window:l2+window+1] 
    ...
    std_d1 = np.sqrt(subspectre1[mini1]*(1+(subspectre1[mini1]/subcont1[mini1])**2))/subcont1[mini1]
    std_d2 = np.sqrt(subspectre2[mini2]*(1+(subspectre2[mini2]/subcont2[mini2])**2))/subcont2[mini2]
    
    l3,l4 = np.min([l1,l2]),np.max([l1,l2])
    std_cont = 1/np.sqrt(np.percentile(spectrei[l3-8*window:l4+8*window],95)) 
    
    return d1, d2, std_d1, std_d2, d1/d2, std_cont*np.sqrt(2+(1./(1-d1))**2+(1./(1-d2))**2), np.sqrt((std_d1/d2)**2+(std_d2*d1/d2**2)**2)
...
def print_pourcent(value, total):
    ...
    return print(np.str(value)+'/'+np.str(total)+' ['+np.str(np.int(pourcentage))+' %]',  end ='\r')
#
def print_warning():
    return print('\n ------ LOOK AT THERE ! ------- \n')
...
def produce_dace_simbad_table():
    ...
    pickle_dump(pd.DataFrame(table,columns=['dace','simbad']),open(root+'/Python/Material/Dace_to_simbad.p','wb'))    
...
def AmpStar(ms, mp, periode, amplitude, i=90, e=0, code='Sun-Earth'):
    ...
    if type(periode)==int:    
        if periode==0:
            mp = mp ; amplitude = amplitude
            if code=='Sun-Earth':
                mp = Mass_earth*mp
            if code=='Sun-Jupiter':
                mp = Mass_jupiter*mp
            mass_proj = mp*np.sin(i*np.pi/180)
            coeff = (mass_proj**3*2*np.pi*6.67e-11)/(ms**2*amplitude**3*np.power(1-e**2, 1.5))
            time = coeff/(365.25*24*3600)
            return time
    if type(amplitude)==int:
        if amplitude==0:
            mp = mp ; periode = periode
            if code=='Sun-Earth':
                mp = Mass_earth*mp
            if code=='Sun-Jupiter':
                mp = Mass_jupiter*mp
            mass_proj = mp*np.sin(i*np.pi/180)
            time = periode*365.25*24*3600
            coeff = (mass_proj**3*2*np.pi*6.67e-11)/(ms**2*time*np.power(1-e**2, 1.5))
            return np.power(coeff, 1/3.)    
...
def compute_photon_noise_std(snr_continuum,kw='RV'):
    ...
    dico = {'RV': [-0.9855573935046936, 0.2468210415895288],
...
def merge_orders(wave, flux, flux_std, continuum=None, dwave=0.01, method='cubic'):
    ...
    for j in range(nb_orders):
        ...
        flux2 = interp1d(wave[j], flux_order, kind = method, bounds_error = False, fill_value = 'extrapolate')(new_wave)
        new_flux[j] =  flux2


        flux2_std = interp1d(wave[j], flux_order_std, kind = method, bounds_error = False, fill_value = 'extrapolate')(new_wave)
        ...
        if continuum is not None:
            ...
            continuum2 = interp1d(wave[j], continuum_order, kind = method, bounds_error = False, fill_value = 'extrapolate')(new_wave)
    ...
    merged_continuum = np.nansum(new_continuum*new_weight,axis=0)/np.nansum(new_weight,axis=0)
...
def star_year_observability(ra, dec, instrument='HARPS', Plot=False, airmass_min=2,new=True, nb_night=365, nb_pts=100, oversampling=5, return_curve=False, alt_sun_max=-12):
    if instrument=='HARPS':
        obs_loc = astrocoord.EarthLocation(lat=-29.260972*u.deg, lon=-70.731694*u.deg, height=2400) #HARPN
        utcoffset = 0*u.hour
    elif instrument=='HARPN':
        obs_loc = astrocoord.EarthLocation(lat=28.754000*u.deg, lon=-17.889055*u.deg, height=2387.2) #HARPN
        utcoffset = 0*u.hour
    elif instrument=='ESPRESSO':
        obs_loc = astrocoord.EarthLocation(lat=-24.627622*u.deg, lon=-70.405075*u.deg, height=2635) #ESPRESSO
        utcoffset = 0*u.hour
    elif instrument=='EXPRES':
        obs_loc = astrocoord.EarthLocation(lat=34.74444*u.deg, lon=-68.578056*u.deg, height=2360.0) #LDT
        utcoffset = 0*u.hour        
    elif instrument=='CARMENES':
        obs_loc = astrocoord.EarthLocation(lat=37.223611*u.deg, lon=2.546111*u.deg, height=2168.0) #Calar Alto
        utcoffset = 0*u.hour                 
    elif instrument=='Geneva':
        obs_loc = astrocoord.EarthLocation(lat=46.204391*u.deg, lon=6.143158*u.deg, height=300) #Geneva
        utcoffset = 0*u.hour
    ...
    dust, dust, sun_elev = my_colormesh(t,
                         nb,
    ...
    new_t, new_nb, star_air = my_colormesh(t,
                         nb,
                         star_airmass,
                         zoom=oversampling,
                         return_output=True,
    ...
    if Plot:
        if new:
            plt.figure()
        plt.title('Instrument : %s\nObs time (Airmass<%.1f) : %.1f %%'%(instrument,airmass_min,np.sum(min_airmass<airmass_min)*100/len(new_t)))
        ...
        if min_airmass[0]<airmass_min:
            plt.fill_betweenx(np.linspace(1,3,10),minx, x2=maxx, color='r', alpha=0.3)  
        else:
            plt.fill_betweenx(np.linspace(1,3,10),2020,x2=minx, color='r', alpha=0.3)  
            plt.fill_betweenx(np.linspace(1,3,10),maxx, x2=2021, color='r', alpha=0.3)  
...
def star_observability(ra, dec, utc_time='2020-9-16T00:00:00', instrument='HARPS', Plot=True, y_var='airmass',alt_sun_max=-12):
    if instrument=='HARPS':
        obs_loc = astrocoord.EarthLocation(lat=-29.260972*u.deg, lon=-70.731694*u.deg, height=2400) #HARPN
        utcoffset = 0*u.hour
    elif instrument=='HARPN':
        ...
        obs_loc = astrocoord.EarthLocation(lat=46.204391*u.deg, lon=6*u.deg, height=300) #Geneva
        utcoffset = 0*u.hour
    ...
    if Plot:
        ...
        plt.title('Instrument : %s\nObstime : %s\nElevation : %.2f°\nAirmass : %.2f'%(instrument,utc_time,m33.transform_to(AltAz(obstime=midnight, location=obs_loc)).alt.deg,m33.transform_to(AltAz(obstime=midnight, location=obs_loc)).secz))
        ...
        plt.fill_between(delta_midnight.to('hr').value, 0, 90, sunaltazs.alt < -0*u.deg, color='b', alpha=0.3)  
        plt.fill_between(delta_midnight.to('hr').value, 0, 90, sunaltazs.alt < alt_sun_max*u.deg, color='darkblue', alpha=0.5) 
        plt.plot(delta_midnight, val(m33altazs), color='k',lw=7)
        plt.scatter(delta_midnight, val(m33altazs), c=dist, label='Star', lw=0, s=8, cmap='YlOrRd_r', vmax=45, vmin=15, zorder=10)
        ax = plt.colorbar()
...
def sun_elevation(instrument = 'HARPS'):
    ...
    pickle_dump(dico,open('/Users/cretignier/Documents/Python/Material/Sun_elevation_'+instrument+'.p','wb'))
            

def star_instrument_sky_observability(instrument='HARPS', z_max=1.5, d_delta=5, d_alpha=1, alt_sun_max=-12, texp=18):
    ...
    if instrument=='HARPS':
        lat = -29.260972
        obs_loc = astrocoord.EarthLocation(lat=lat*u.deg, lon=-70.731694*u.deg, height=2400) #HARPN
        utcoffset = 0*u.hour
    elif instrument=='HARPN':
        lat = 28.754000
        obs_loc = astrocoord.EarthLocation(lat=lat*u.deg, lon=-17.889055*u.deg, height=2387.2) #HARPN
        utcoffset = 0*u.hour
    elif instrument=='ESPRESSO':
        lat = -24.627622
        obs_loc = astrocoord.EarthLocation(lat=lat*u.deg, lon=-70.405075*u.deg, height=2635) #ESPRESSO
        utcoffset = 0*u.hour
    elif instrument=='EXPRES':
        lat = 34.74444
        obs_loc = astrocoord.EarthLocation(lat=34.74444*u.deg, lon=-68.578056*u.deg, height=2360.0) #LDT
    ...
    sun = pd.read_pickle('/Users/cretignier/Documents/Python/Material/Sun_elevation_'+instrument+'.p')
    ...
    for i in tqdm(range(len(deltas))):
        ...
        for j in range(len(alphas)):
            ...
            airmass,cumu = transform_cumu(curve,limites=np.linspace(1,air_lim,int(1000*(air_lim-1)+1)))
    ...
    for j in range(len(alphas)):
        ...
        cs = plt.contour(air_matrix,delta_matrix,visi,colors='k',levels=[50,60,70,80])
    ...
    if len(alphas):
        ...
        for j in [1.3,1.5,1.7,1.9]:
            ...
            plt.subplot(2,2,c)
            my_colormesh(alphas,deltas,vis[:,:,loc],vmin=50,vmax=80,cmap='viridis',zoom=5)
            plt.colorbar()
            plt.title('Z_lim = %.1f'%(j))
        plt.subplots_adjust(hspace=0.3,wspace=0.3)
    
    dico = {'delta':deltas,'alpha':alphas,'airmass':airmass,'visibility':vis}
    pickle_dump(dico,open('/Users/cretignier/Documents/Python/Material/Visibility_declination_'+instrument+'.p','wb'))
...
def symmetric_cut_functions(break_loc=0.5,length=100):
    ...
    alpha[length-cut_alpha:] = (np.arange(length-cut_alpha,length)-(length-cut_alpha))/cut_alpha
    return alpha

def film_change_variable(x,y,x2,y2,z=None,z2=None,cmap='jet',figsize=(10,10),animation_time=3,break_label=0.5, xlabel='Old X var',xlabel2='New X var',ylabel='Old Y var',ylabel2='New Y var',zlabel='',zlabel2='',fontsize=15, lineplot=True):
    ...
    os.system('convert -delay 1 -quality 100 '+dir_output+'images*.png '+dir_output+'film_%s.mpeg'%(value))
...
def find_borders(matrix, cross_valid_size, frac_affected, r_min=0, Draw=False, loc_comp=None,alpha=0):
...
def find_borders_it(matrix, cross_valid_size, frac_affected, cv_rm, Draw=True, selection=20, tresh_complete=0.25, loc_comp=None):
    
    cluster_min_size = np.round(cross_valid_size*frac_affected,0)

    vecs = []
    for rcorr in np.arange(0.5,0.91,0.05):
        borders, cluster_loc, mask_cluster, med_cluster, inside = find_borders(matrix, cross_valid_size, frac_affected, r_min=rcorr, loc_comp=loc_comp)
        #vecs.append((np.sum(abs(cross_valid_size-np.diff(cluster_loc))==0),1-rcorr))
        criterion1 = abs(cross_valid_size-np.diff(cluster_loc))<=cross_valid_size*cv_rm/100*tresh_complete
    ...
    borders, cluster_loc, mask_cluster, med_cluster, inside = find_borders(matrix, cross_valid_size, frac_affected, r_min=np.round(vecs[-1][-1],2), loc_comp=loc_comp) 
        
    cluster_complete = abs(cross_valid_size-(np.diff(cluster_loc)-1))<cross_valid_size*cv_rm/100*tresh_complete
    ...
    for j,k in zip(cluster_loc[0:-1]-0.5,cluster_loc[1:]-0.5):
        if (k-j)>cluster_min_size:
            med_cluster.append(np.nanmedian(abs(matrix)[int(j+0.5):int(k+0.5),int(j+0.5):int(k+0.5)]))
      
    signi = [(i,j,k) for i,j,k in zip(1-np.isnan(med_cluster).astype(int),med_cluster,np.arange(len(med_cluster)))]
    signi.sort()
...
def block_matrix_iter(coeff, cross_valid_size, frac_affected, cv_rm, var_kept, selection, loc_comp):
    ...
    borders, cluster_loc, mask_cluster, med_cluster, cluster_signi, mask_cluster_complete = find_borders_it(coeff.matrix_corr.copy(),cross_valid_size, frac_affected,cv_rm,Draw=True,selection=selection,loc_comp=loc_comp)
    ...
    coeff.r_matrix(name=var_kept[mask_cluster][mask_cluster_signi],absolute=True,Plot=False,rm_diagonal=True)   
    
    x,y = np.meshgrid(np.arange(len(mapping2)),np.arange(len(mapping2))) ; x = np.ravel(x) ; y = np.ravel(y)
    ...
    final_vec_ordered = np.hstack([new_vec_ordered,var_kept[~np.in1d(var_kept,new_vec_ordered)]])
