...
import fastlinsquare_cholesky as l1_cholesky
...
class identification(object):
    ...
    def Identification(self):
        ...
        for j in self.table1[:, self.liste1]:
            Match = np.where(np.product(self.table2[:, self.liste2]==j, axis=1)==True)[0]
            if len(Match)==1:
    ...
    def reduction_overlapping(self, replace=False):
        ...
        inversion = identification(copy.table2, matrice1_reduced, copy.liste2, copy.liste1)
...
class projectedTable(object):
    ...
    def projection(self):
        Mean2 = [] ; Min2 = [] ; Std2 = [] ; Max2 = []
        for j in range(len(self.liste1)):
            Mean = [] ; Min = [] ; Std = [] ; Max = []
            self.unprojected_par = self.liste1[j]
            self.recorded_par = self.liste2[j]
            exclusion = float('nan')
            values = [np.unique(self.table[:, j]).tolist() for j in self.unprojected_par]
            combination = np.array(list(itertools.product(*values)))
            for item in combination:
                position=np.where(np.product((self.table[:,self.unprojected_par]==item),axis=1)==1)[0]
                Mean.append(item.tolist()+[np.mean(self.table[position,k]) for k in self.recorded_par])
                Std.append(item.tolist()+[np.std(self.table[position,k]) for k in self.recorded_par])
                try : 
                    Min.append(item.tolist()+[np.min(self.table[position,k]) for k in self.recorded_par])
                except ValueError:
                    Min.append(item.tolist()+[exclusion for k in self.recorded_par])
                try : 
                    Max.append(item.tolist()+[np.max(self.table[position,k]) for k in self.recorded_par])
                except ValueError:
                    Max.append(item.tolist()+[exclusion for k in self.recorded_par])
            Max2.append(np.array(Max)) ; Mean2.append(np.array(Mean)) ; Min2.append(np.array(Min)) ; Std2.append(np.array(Std)) 
        self.Mean = Mean2 ; self.Min = Min2 ; self.Std = Std2 ; self.Max = Max2  
...
class tableXYZ(object):
    ...
    def ternary(self,cmap='seismic'):
        ...
        plt.scatter(0.5*(2*self.y+self.z)/(self.x+self.y+self.z),np.sqrt(3)*self.z/2/(self.x+self.y+self.z))
    
    def plot3D(self, Show=False, fraction=1,xlabel='', ylabel='', zlabel='', color='k', cmap='Greys', marker='o', ax=None, s=20, alpha_p=1, alpha_c=0.5,proj=False, nbins=50, n_levels=['2d', [1, 2]]):
        if ax==None:
            fig = plt.figure()
            ax = Axes3D(fig)
            self.ax = ax
            ...
            ax = ax
        mask = np.random.choice(np.arange(len(self.x)),int(fraction*len(self.x)),replace=False)
        ax.scatter(self.x[mask], self.y[mask], self.z[mask], color=color, marker=marker, s=s, alpha=alpha_p)
        xlim = ax.get_xlim() ; ylim = ax.get_ylim() ; zlim = ax.get_zlim()
        plt.xlabel(xlabel) ; plt.ylabel(ylabel) ; ax.set_zlabel(zlabel)
        if proj:
            for permu in [[self.x,self.y,'z',zlim[0]],[self.x,self.z,'y',ylim[1]],[self.z,self.y,'x',xlim[0]]]:
                x, y, direction,lim = permu
                DX = x.max()-x.min()
                DY= y.max()-y.min()
                nbins=nbins
                k = kde.gaussian_kde(np.vstack([x, y]))
                xi1, yi1 = np.mgrid[x.min()-0.5*DX:x.max()+0.5*DX:nbins*1j, y.min()-0.5*DY:y.max()+0.5*DY:nbins*1j]
                z = k(np.vstack([xi1.flatten(), yi1.flatten()]))
                
                if type(n_levels)!=int:
                    if n_levels[0]=='1d':
                        niveaux = np.sort([np.hstack(z)[np.hstack(z).argsort()[::-1][myf.find_nearest(np.cumsum(np.sort(np.hstack(z))[::-1]),(1-2*(1-norm_gauss.cdf(j)))*np.sum(z))]] for j in n_levels[1]])
                    elif n_levels[0]=='2d':
                        niveaux = np.sort([np.hstack(z)[np.hstack(z).argsort()[::-1][myf.find_nearest(np.cumsum(np.sort(np.hstack(z))[::-1]),(1-np.exp(-0.5*j**2))*np.sum(z))[0]]] for j in n_levels[1]])
                ...
                if direction=='x':
                    zi1,xi1 = xi1,zi1
                ax.contour(xi1, yi1, zi1, zdir=direction, offset = lim, levels=niveaux, colors=color)
                ax.contourf(xi1, yi1, zi1, zdir=direction, offset = lim, levels=niveaux, cmap=cmap, alpha=alpha_c)

            ax.set_xlim(xlim) ; ax.set_ylim(ylim) ; ax.set_zlim(zlim)
        if Show==True:
            plt.show()
    
    def plot4Dto3Dscatter(self, Show=False, vmax=None, vmin=None, cmap='coolwarm', marker='s', s=30, xlabel='', ylabel='', zlabel=''):
        """WARNING : no colorbar possible with 3D axis."""
        fig = plt.figure()
        ax = Axes3D(fig)
        ax.scatter(self.x, self.y, self.z, c=self.h, cmap=cmap, vmax=vmax, vmin=vmin, marker=marker, s=s)
        plt.xlabel(xlabel) ; plt.ylabel(ylabel) ; ax.set_zlabel(zlabel)
        if Show==True:
            plt.show()
    
    def plot3Dto2Dscatter(self, Show=False, Showbar=True, Use=2, vmax=None, vmin=None, fontsizetext=12, cmap='coolwarm', marker='s', s=30, xlabel='', ylabel='', colorlabel=''):
        if Use==2:
            col_vec = self.z
        else:
            col_vec = self.h
        plt.scatter(self.x, self.y, c=col_vec, cmap=cmap, vmax=vmax, vmin=vmin, marker=marker, s=s)
        plt.xlabel(xlabel, fontsize=fontsizetext) ; plt.ylabel(ylabel, fontsize=fontsizetext)
        if Showbar==True:
            ax1 = plt.colorbar()
    ...
    def plot4Dto2D(self, Show=False, Showbar=True,  Nb=10, vmax=None, vmin=None, alpha=0.8, cmap='coolwarm', xlabel='', ylabel='', colorlabel='', fontsizetext=12, inline=1, fontsize=10, linewidth=0.5, fmt='%1.1f'):
        ...
        if Showbar==True:
            ...
            ax1 = mpl.colorbar.ColorbarBase(ax, norm=mpl.colors.Normalize(vmin=vmin, vmax=vmax))
            ax1.ax.set_ylabel(colorlabel, fontsize=fontsizetext)
    ...
    def plot4Dto2Dscatter(self, Show=False, Showbar=True,  Nb=10, vmax=None, vmin=None, marker='s', s=30,cmap='coolwarm', xlabel='', ylabel='', colorlabel='', fontsizetext=12, inline=1, fontsize=10, linewidth=0.5, fmt='%1.1f'):
        dim1=len(np.unique(self.x))
        dim2=int(len(self.x)/dim1)
        X=np.reshape(self.x,(dim1,dim2))
        Y=np.reshape(self.y,(dim1,dim2))
        Z=np.reshape(self.z,(dim1,dim2))
        plt.scatter(self.x, self.y, c=self.h, cmap=cmap,vmin=vmin, vmax=vmax, marker=marker, s=s)
        plt.xlabel(xlabel, fontsize=fontsizetext) ; plt.ylabel(ylabel, fontsize=fontsizetext)
        if Showbar==True:
            ax1 = plt.colorbar()
            ax1.ax.set_ylabel(colorlabel, fontsize=fontsizetext)
...
class histo(object):
    ...
    def plot1(self, col, Show=False, color='r', alpha=0.7, normed=False, fontsize=14):
        ...
        astrohist(self.x[col], bins=self.bin_method, color=color, alpha=alpha, ec='black', label=self.label[col], density=normed)
        plt.xlabel(self.nom, fontsize=fontsize)
        if Show==True:
            plt.show()
        return astrohist(self.x[col], bins=self.bin_method, color=color, alpha=alpha, ec='black', label=self.label[col], density=normed)
    def plotmany(self, cols, Show=True, colors=None, alpha=0.5, normed=False, fontsize=14, loc='best',vline=False):
        bins = astrohist(np.hstack([self.x[j] for j in cols]), bins=self.bin_method, color='white', density=normed)[1] 
        if colors==None:
        ...
        for j in colors[cols].argsort()[::-1]:
            if (colors[j]=='w')|(colors[j]=='white'):
                plt.hist(self.x[j], bins=bins, color=colors[j], alpha=alpha, ec=ec[j], density=normed)
            else:
                plt.hist(self.x[j], bins=bins, color=colors[j], alpha=alpha, ec=ec[j], label=self.label[j], density=normed)
            plt.hist(self.x[j], bins=bins, color=ec[j], density=normed, histtype='step', linewidth=0.5)
            if (self.label!=[None]*len(self.x))&(self.label!=['']*len(self.x)):
...
class histoXY(object):
    ...
    def plot1(self, Show=False, color='r', alpha=0.7, normed=False):
        astrohist(self.x, bins=self.bin_method, color=color, alpha=alpha, ec='black', label=self.xlabel,density=normed)
        plt.xlabel(self.nom)
        if Show==True:
            plt.show()
    def plot2(self, Show=False, color='g', alpha=0.7):
        astrohist(self.y, bins=self.bin_method, color=color, alpha=alpha, ec='black', label=self.ylabel)
        plt.xlabel(self.nom)
        if Show==True:
            plt.show()
    def plotboth(self, Show=True, color1='r', color2='g', alpha=0.5, normed=False, fontsize=14):
        bins = astrohist(np.hstack((self.x,self.y)), bins=self.bin_method, color='white', density=normed)[1] 
        plt.hist(self.x, bins=bins, color=color1, alpha=alpha, ec='black', label=self.xlabel, density=normed)
        plt.hist(self.y, bins=bins, color=color2, alpha=alpha, ec='black', label=self.ylabel, density=normed)
        plt.hist(self.y, bins=bins, color='k', density=normed, histtype='step', linewidth=0.5)
        plt.hist(self.x, bins=bins, color='k', density=normed, histtype='step', linewidth=0.5)
        plt.legend()
        plt.xlabel(self.nom, fontsize=fontsize) 
        if Show==True:
            plt.show()
...
class table(object):
        ...
        if Plot:
            ...
            for i in range(len(r_matrix)):
                for j in range(len(r_matrix)):
                    if not light_plot:
                        if unit==1:
                            plt.annotate('%.2f'%(r_matrix[i,j]),(i,j),fontsize=[13,16][int(paper_plot)],va='center',ha='center')
                        else:
                            plt.annotate('%.0f'%(r_matrix[i,j]*100),(i,j),fontsize=[13,16][int(paper_plot)],va='center',ha='center')                            
                    else:
                        if (abs(r_matrix[i,j])>0.4)&(r_matrix[i,j]!=1):
                            plt.annotate('%.0f'%(abs(r_matrix[i,j])*100),(i-0.25,j),fontsize=8,va='center')                        
                
    def cross_validation(self, cross_valid_size, nb_comp, r_min = 0.6, frac_affected = 0.01, cv_rm = 20, debug = False, offset=0, fig_num=1, algo_block=2, algo_borders=3):
        ...
        def highlight_comp(var,legend=False,Plot=False):
            ...
            if Plot:
                for n,p in enumerate(l):
                    ...
                    plt.scatter(p,p,color=None,label='%.0f'%(n+1),zorder=100,edgecolor='k')
                if legend:
                    plt.legend(loc=1)
        ...
        coeff.r_matrix(name=list(coeff.table.keys()),absolute=False,light_plot=True,angle=90,Plot=debug)
        ...
        var_kept = np.array(list(coeff.table.keys()))[np.where(np.sum(abs(coeff.matrix_corr)>r_min,axis=0)!=0)[0]]
        
        if debug:
            plt.figure()
        coeff.r_matrix(name=var_kept,absolute=False,light_plot=True,angle=90,vmin=-1.2,vmax=1.2,Plot=debug)   
        ...
        if debug:
            plt.figure()
        coeff.r_matrix(name=var_kept[block_order],absolute=False,light_plot=True,angle=90,vmin=-1.2,vmax=1.2,Plot=debug)   
        ...
        if debug:
            plt.figure()
        coeff.r_matrix(name=var_kept[block_order][mask_kept],absolute=False,light_plot=True,angle=90,vmin=-1.2,vmax=1.2,Plot=debug)   
        #coeff.matrix_corr = myf.rm_sym_diagonal(coeff.matrix_corr,k=offset)
        
        vec_ordered = list(var_kept[block_order][mask_kept])+list(np.array(all_vec_name)[~np.in1d(all_vec_name,var_kept[block_order][mask_kept])])
        
        if debug:
            plt.subplots_adjust(left=0.05,right=0.95,top=0.95,bottom=0.05)

        plt.figure(fig_num,figsize=(10,12))
        plt.title('R_min=%.3f, Nb_comp=%.0f, Nb_draw=%.0f, Min_size_cluster=%.0f%%, CV_rm=%.0f%%, Algo=%.0f'%(r_min,nb_comp,cross_valid_size,frac_affected*100,cv_rm,algo_borders))
        coeff.r_matrix(name=vec_ordered,absolute=False,light_plot=True,angle=90,vmin=-1.2,vmax=1.2,Plot=debug)   
        plt.imshow(abs(coeff.matrix_corr),vmin=0.5,vmax=1) ; dust = highlight_comp(vec_ordered,legend=True,Plot=True)      
        #coeff.matrix_corr = myf.rm_sym_diagonal(coeff.matrix_corr,k=offset)
        ...
        if algo_borders==1:#based on 1 diagonal-off algorithm
            ...
            border_left = np.sum(grad>0,axis=1)/cross_valid_size ; border_left = np.hstack([1,border_left])
            border_right = np.sum(grad<0,axis=1)/cross_valid_size ; border_right = np.hstack([0,border_right])
            loc_comp = dust[0]
            loc_comp = np.hstack([np.sort(loc_comp),[len(border_left)]])
            left = [0] ; right = []
            for i in np.arange(0,len(loc_comp)-1):
                if i:
                    left.append(np.argmax(border_left[loc_comp[i-1]:loc_comp[i]+1])+loc_comp[i-1])
                right.append(np.argmax(border_right[loc_comp[i]:loc_comp[i+1]])+loc_comp[i])
            ...
            if (k-j)>cluster_min_size:
                ...
                if len(components):
                    ...
                    for n,c in enumerate(components):
                        vec_ordered[int(j+0.5)+n], vec_ordered[l[c]] =  vec_ordered[l[c]], vec_ordered[int(j+0.5)+n]
                        coeff.matrix_corr[int(j+0.5)+n], coeff.matrix_corr[l[c]] =  coeff.matrix_corr[l[c]], coeff.matrix_corr[int(j+0.5)+n]
                        coeff.matrix_corr[:,int(j+0.5)+n], coeff.matrix_corr[:,l[c]] =  coeff.matrix_corr[:,l[c]], coeff.matrix_corr[:,int(j+0.5)+n]
        ...
        if algo_borders==2:
            ...
            plt.plot(np.arange(len(final_coeff))-0.5,border_left,color='k',zorder=100)
            plt.plot(np.arange(len(final_coeff))-0.5,border_right-1,color='gray',zorder=100)
        ...
        mini = np.argsort([np.min(i) for i in comp_inside_cluster]) #TBD to order cluster by the index of the component (semble prise de tete)
        ...
        for c in clusters:
            ...
            percent = (m-n-self.cv_nb_comp_inside[counter-1])*100/cross_valid_size
            percentage.append(percent)
            components_stacked.append(np.median(indep_comp,axis=1))
            med_r = np.nanmedian(abs(final_coeff[n:m,n:m]))
            r_med.append(med_r)
            plt.subplot(nb_comp//2+nb_comp%2,2,comp_inside_cluster[counter-1][0]+1)            
            plt.title('Components : '+'+'.join([str(i+1) for i in comp_inside_cluster[counter-1]])+'  |  frequency = %.0f %%  |  median(R)=%.3f'%(percent,med_r))
            plt.plot(indep_comp,color='k',alpha=0.1)
            plt.plot(np.median(indep_comp,axis=1),color='r',lw=1.5)#,label='frequency = %.0f %%\nmedian(R)=%.3f'%(percent,med_r))
            plt.plot(np.sign(temp[loc_sign,n])*temp[:,n]/myf.mad(temp[:,n]),color='b')
            #plt.legend()
        ...
        self.cv_percentage_norm  = np.array([self.cv_percentage[i]/len(self.cv_comp_inside[i]) for i in np.arange(len(self.cv_comp_inside))])
        
                         
    def cross_validation_it(self, cross_valid_size, nb_comp, frac_affected = 0.33, iteration=5, fig_num=1, cv_rm=10, algo_borders=[3,3]):
        ...
        for condition in range(iteration):
            ...
            plt.close('all')
            print('Iteration %.0f, threshold fixed at R=%.3f'%(condition+1,r_min_it))
            try:
                self.cross_validation(cross_valid_size, nb_comp, r_min=r_min_it, frac_affected=frac_affected, debug=False, cv_rm=cv_rm, fig_num=fig_num, algo_borders=algo_borders[0])
                ...
                if np.max(self.cv_percentage_norm)<=100-cv_rm:
                    ...
                    tresh = 100-cv_rm
                crit = np.sum((np.array(self.cv_percentage_norm)>=tresh)&(np.array(self.cv_percentage_norm)<=(105)))
        ...
        liste = [(i,j,k) for i,j,k in zip(sizes_binary, highest_percentage, good_r_min)]
        #print(liste)
        liste.sort()    
        r_min_it = liste[-1][-1]
            
        print('Best R found to be : %.3f'%(r_min_it))
        self.cross_validation(cross_valid_size, nb_comp, r_min=r_min_it, frac_affected=frac_affected, debug=False, cv_rm=cv_rm, fig_num=fig_num, algo_borders=algo_borders[1])
        plt.pause(0.1)
    ...
    def cross_validation2(self, nb_comp, r_min=0.6, frac_affected=0.01, cv_rm=20, fig_num=1, overselection=2):
        """For a NxT matrix with T the number of time observations and N the number of vectors (N=CxS with C the number of component and S the number of independent simulations)"""
        ...
        coeff.r_matrix(name=list(coeff.table.keys()),absolute=True,Plot=False,rm_diagonal=True)    
        #only kept relevant rows
        var_kept = np.array(list(coeff.table.keys()))[np.where(np.sum(coeff.matrix_corr>r_min,axis=0)!=0)[0]]
        coeff.r_matrix(name=var_kept,absolute=True,Plot=False,rm_diagonal=True)   
        
        plt.figure(fig_num+2,figsize=(9,8))
        
        plt.subplot(3,3,1) ; plt.title('Iteration 0')
        plt.imshow(coeff.matrix_corr.copy())
        #block algo
        block_order,dust = myf.block_matrix2(coeff.matrix_corr.copy())
        coeff.r_matrix(name=var_kept[block_order],absolute=True,Plot=False,rm_diagonal=True)   
        var_kept1 = var_kept[block_order]
        coeff1 = coeff.copy()
        
        for itera in np.arange(1,8):
            print('Iteration %.0f : '%(itera))
            loc_comp,dust = myf.highlight_comp(var_kept1,nb_comp,legend=True,Plot=False)  
            plt.subplot(3,3,1+itera) ; plt.title('Iteration %.0f'%(itera))
            coeff1,var_kept1 = myf.block_matrix_iter(coeff, cross_valid_size, frac_affected, cv_rm, var_kept1, selection=selection,loc_comp=loc_comp)
        
        plt.subplot(3,3,9) ; plt.title('Iteration 8')
        borders,cluster_loc,mask_cluster,med_cluster,cluster_signi,mask_cluster_complete = myf.find_borders_it(coeff1.matrix_corr.copy(), cross_valid_size, frac_affected, cv_rm, Draw=True,selection=selection)
        plt.subplots_adjust(left=0.05,right=0.95,top=0.95,bottom=0.05)
        ...
        plt.title('Nb_comp=%.0f, Nb_draw=%.0f, Min_size_cluster=%.0f%%, CV_rm=%.0f%%'%(nb_comp,cross_valid_size,frac_affected*100,cv_rm),fontsize=15)
        ...
        borders,cluster_loc,mask_cluster,med_cluster,cluster_signi,mask_cluster_complete = myf.find_borders_it(coeff.matrix_corr.copy(), cross_valid_size, frac_affected, cv_rm, Draw=True, selection=selection)
        plt.imshow(coeff.matrix_corr.copy()) ; ax = plt.colorbar(pad=0) ; ax.ax.set_ylabel(r'$|\mathcal{R}_{Pearson}|$',fontsize=15)
        #coeff.matrix_corr = myf.rm_sym_diagonal(coeff.matrix_corr,k=offset)
        final_coeff = coeff.matrix_corr.copy()
        ...
        for j,k in zip(cluster_loc[0:-1]-0.5,cluster_loc[1:]-0.5):
            if (k-j)>cluster_min_size:
                ...
                if len(components):
                    ...
                    for n,c in enumerate(components):
                        final_vec_ordered[int(j+0.5)+n], final_vec_ordered[l[c]] =  final_vec_ordered[l[c]], final_vec_ordered[int(j+0.5)+n]
                        coeff.matrix_corr[int(j+0.5)+n], coeff.matrix_corr[l[c]] =  coeff.matrix_corr[l[c]], coeff.matrix_corr[int(j+0.5)+n]
                        coeff.matrix_corr[:,int(j+0.5)+n], coeff.matrix_corr[:,l[c]] =  coeff.matrix_corr[:,l[c]], coeff.matrix_corr[:,int(j+0.5)+n]
        ...
        for c in clusters:
            ...
            percent = (m-n-nb_comp_inside[counter-1]-sum(mask_out))*100/cross_valid_size
            percentage.append(percent)
            ...
            plt.plot(np.median(indep_comp[:,~mask_out],axis=1),color='r',lw=1.5)#,label='frequency = %.0f %%\nmedian(R)=%.3f'%(percent,med_r))
            plt.plot(np.sign(temp[loc_sign,n])*temp[:,n]/myf.mad(temp[:,n]),color='b')
            #plt.legend()
    ...
    def count_outliers(self,m=1.5,transpose=False):
        ...
        mask_sup = matrix>(np.nanpercentile(matrix,75,axis=1)+(m*IQ))[:,np.newaxis]
        mask_inf = matrix<(np.nanpercentile(matrix,25,axis=1)-(m*IQ))[:,np.newaxis]
    ...
    def gram_schimdt_modified(self,vector,base_std=None,Plot=True):
        ...
        if len(vector)==len(tab.T):
            ...
            print('Shape of the vector (%.0f) is different from the basis shape (%.0f)'%(len(vector),len(self.table.T)))
    ...
    def export_to_dace(self,file_name,convert_to_kms=1,convert_to_jdb=0):
        ...
        all_kw = ['fwhm','sig_fwhm','contrast','sig_contrast','bis_span','sig_bis_span',
                  's_mw','sig_s','ha','sig_ha','na','sig_na','ca','sig_ca','rhk','sig_rhk',
                  'berv','sn_caii','model','ins_name']
    ...
    def rv_subselection(self, rv_std=None, selection=None):
        ...
        mean_rv = np.sum(matrix_rv[selection]/matrix_rv_std[selection]**2,axis=0)/np.sum(1/matrix_rv_std[selection]**2,axis=0)
        mean_rv_std = 1/np.sqrt(np.sum(1/matrix_rv_std[selection]**2,axis=0))

        return tableXY(np.arange(len(mean_rv)), mean_rv, mean_rv_std)        
    
    def slilder_plot(self, x, color='k', cut_index_min = 0, cut_index_max = -1, logx=False, logy=False):
        ...
        if len(np.shape(self.table))!=2:
            ...
            ssmoothing = Slider(axsmoothing, 'Index', index1, index2, valinit = index1, valstep=dindex)
    ...
    def UAPCA(self, weight=None, m=2, kind='inter', scaling=1, comp_max=None):
        ...
        self.zscore_uapca= np.mean(self.uapca_coeff,axis=1)/np.std(self.uapca_coeff,axis=1)   
    ...
    def SVD(self,comp_max=None,weight=None):   
        ...
        self.svd_vec,self.svd_eig_val,dust = np.linalg.svd(np.dot(self.table.T,self.table))
        ...
        self.zscore_svd = np.mean(self.svd_coeff,axis=1)/np.std(self.svd_coeff,axis=1)                 
    
    def PCA(self, weight = None, m=2, kind='inter', num_sim=3000, comp_max=None, abs_coeff = False):
        ...
        if weight is None:
            ...
            if False:#old version which was wrong (producing good result but the model cannot be applied on other test sample, I keep it until other algo as ICA and UMPCA are corrected in a same way)        
                pca.fit(R.T)
                ...
                self.components = pca.transform(self.table).T               #coefficient of the pca fit
            self.vec_fitted = np.dot(self.vec,self.components).T #sample projected on the basis
            
            self.var = pca.explained_variance_                   #variance explained of the pca fit
            self.var_ratio = pca.explained_variance_ratio_       #variance_ratio explained of the pca fit
            ...
            self.zscore_components = np.mean(components,axis=1)/np.std(components,axis=1)
            self.pca_model= pca
        else:  
            #PCA with error bars following the Paper Tamuz, Mazeh 2013
            a_save = [] ; c_save=[] ; chi2 = [np.sum(w*R**2)]
            ...
            for i in tqdm(range(max_c)):
                for j in range(num_sim):
                    ...
                    if np.nanmean(c)<0: #leave degeneracy +/- for the vector direction
                        c *=-1
                        a *=-1
                    #c = c * (2*(np.sum(np.sign(a))>=0)-1)
                    #a =  a * (2*(np.sum(np.sign(a))>=0)-1)
                    a2 = a.copy()
                    a = a+np.random.randn(len(a))*0.1#fix the liberty degree of c*a by posing the std of a at 1
    ...
    def eval_model(self, model_fitted, table):
        ...
        rslope = np.median((table-np.median(table,axis=0))/((vec_fitted-np.median(vec_fitted,axis=0))+1e-6),axis=1)
    ...
    def fit_base(self, base_vec, weight=None, num_sim = 1):
        ...
        if np.shape(base_vec)[1] != np.shape(self.table)[0]:
            base_vec = np.array([base_vec[i]*np.ones(np.shape(self.table)[0])[:,np.newaxis] for i in range(len(base_vec))])
        ...
        coeff = np.array([np.linalg.lstsq(base_vec[:,i,:].T*np.sqrt(weight[i])[:,np.newaxis], (self.table[i])*np.sqrt(weight[i]),rcond=None)[0] for i in range(len(self.table))])
        
        vec_bootstrap = np.array([(self.table[i]+np.random.randn(num_sim,len(weight[i]))/np.sqrt(weight[i])) for i in range(len(self.table))])
        
        coeff_test = np.array([np.linalg.lstsq(base_vec[:,i,:].T*np.sqrt(weight[i])[:,np.newaxis], (vec_bootstrap[i]*np.sqrt(weight[i])).T,rcond=None)[0] for i in range(len(self.table))])
        ...
        vec_fitted = np.array([np.sum(coeff[j]*base_vec[:,j,:].T,axis=1) for j in range(len(self.table))])
    ...
    def fit_unique_base(self, base_vec, weight=None):
        ...
        coeff = np.linalg.lstsq(base_vec.T*np.sqrt(weight), ((self.table)*np.sqrt(weight)).T,rcond=None)[0].T
    ...
    def fit_corr2(self,x,perm=1000,rm=False):
        y = self.table[0,:,:]
        w = self.table[1,:,:] ; yerr = 1/np.sqrt(w) 
    ...
    def fit_corr(self, x, weight=True):
        ...
        if len(Signal[0])!=len(x):
            ...
            for j in tqdm(range(len(Signal))):
                ...
                num+=1
            self.rpearson, self.rslope, self.rms, self.med_err = rpearson, rslope, rms, med_err 
    
            
    def fit_pca(self, base_vec, weight=None, ini_par = None, par_over=True,method='Powell'):
        ...
        for j in tqdm(range(np.shape(R)[0])):
            result = minimize(Chicarre, ini_par, args=(R[j,:], Rerr[j,:]), method=method)
            result2 = minimize(Chicarre, ini_par, args=(R[j,:], Rerr[j,:]), method='SLSQP')
        ...
        self.coeff_n = coefficient[:,1:]*np.std(base_vec,axis=0) #normalise the coefficient by the rms of the base vector
        self.coeff_nn = self.coeff_n/np.std(Signal,axis=1)[:,np.newaxis] #normalise the coefficient by the rms of the vectors decomposed
        self.coeff_nna = abs(self.coeff_nn)/np.sum(abs(self.coeff_nn),axis=1)[:,np.newaxis]
    ...
    def cross_counter(self,col1,col2,order_index=None,order_col=None):
        ...
        count = np.array(count) ; count = np.reshape(count,(len(flavour1),len(flavour2)))
    ...
    def KDE(self, cycle=0, cols=None, nbins=70, fraction=1., edgecolor=None, s=10, n_levels=['2d', [1, 2]], col_species=None, liste=["Blues", "Reds", "Greens","Greys","Purples","Oranges"], contour_color='k', alpha_c=0.7, alpha_s=0.8, coeff_alpha=[0.2,0]):
        ...
        for num,idx in enumerate(indices):
            ...
            plt.contourf(xi1, yi1, z.reshape(xi1.shape), nlevel, cmap=liste[idx], levels=niveaux, alpha=alpha_c+coeff_alpha[num])
            if contour_color!=None:
                plt.contour(xi1, yi1, z.reshape(xi1.shape), n_levels, colors=contour_color, levels=niveaux, linewidths=1)
            
            sample = np.random.choice(np.arange(len(x)), np.int(fraction*len(x)), replace=False)
            plt.scatter(x.iloc[sample], y.iloc[sample], color=np.array(['b', 'r', 'g', 'k', 'purple', 'orange'])[idx], s=s, edgecolor=edgecolor, alpha=alpha_s+coeff_alpha[num])
            ...
            for name in keys_table:
                ...
                if cols[0]=='r':
                    if name not in cols[1]:
                        key=np.append(key, name)
            keys_table = key
        X = self.table[keys_table].copy() ; X = X.reset_index(drop=True)
        y = self.tabkle[col_species].copy() ;  y = y.reset_index(drop=True)
        
        X = np.append(arr = np.ones((30, 1)).astype(int), values = X, axis = 1)   # <---- change this 30 
        numVars = len(X[0])
    ...
    def histo_cumu(self,density=True,cumulative=True,cols=None,nbins=50,xmin=None,xmax=None):
        keys_table = self.table.keys()
        if cols is not None:
            key= np.array([])
            for name in keys_table:
    ...
    def plot3D(self,cols=None, col_species=None, Show=False, fraction=1, xlabel='', ylabel='', zlabel='', marker='o', ax=None, s=20, alpha_p=1, alpha_c=0.5,proj=False, nbins=50, n_levels=['2d', [1, 2]]):
        ...
        if len(cols)!=3:
            ...
            for enum, species in enumerate(np.unique(self.table[col_species])):
                if enum ==0:
                    test_save = tableXYZ(np.array(self.table.loc[self.table[col_species]==species,cols[0]]),
                                np.array(self.table.loc[self.table[col_species]==species,cols[1]]),
                                np.array(self.table.loc[self.table[col_species]==species,cols[2]]))
                    test_save.plot3D(Show=False, fraction=fraction, xlabel=xlabel, ylabel=ylabel, zlabel=zlabel, 
                            color=color[enum], cmap=cmap[enum], marker=marker, ax=ax, s=s, alpha_p=alpha_p, alpha_c=alpha_c, proj=proj, nbins=nbins, n_levels=n_levels)
                else:
                    test = tableXYZ(np.array(self.table.loc[self.table[col_species]==species,cols[0]]),
                                np.array(self.table.loc[self.table[col_species]==species,cols[1]]),
                                np.array(self.table.loc[self.table[col_species]==species,cols[2]]))
                    test.plot3D(Show=Show, fraction=fraction, xlabel=xlabel, ylabel=ylabel, zlabel=zlabel, 
                            color=color[enum], cmap=cmap[enum], marker=marker, ax=test_save.ax, s=s, alpha_p=alpha_p, alpha_c=alpha_c, proj=proj, nbins=nbins, n_levels=n_levels)
    ...
    def match_nearest_vector(self,base2,m_out=0,Plot=False):
        ...
        for i,j in zip(perms2[best],perms[best]):
            ...
            if m_out:
                test = tableXY(np.array(matrix.table[0:5][i]),np.array(matrix.table[N:][j]))
    ...
    def pairplot(self, cycle=0, cols=None, nbins=70, bins_method='scott',
                 fraction=1., xline=None, yline=None, edgecolor=None, s=10, 
                 n_levels=['2d', [1, 2]], col_species=None, kde_plot=True,
                 liste=["Blues", "Greens", "Reds","Greys","Purples","Oranges"], 
                 contour_color='k', alpha_c=0.8, alpha_h=0.6, alpha_s=0.9, color_param = None):
        ...
        for i, j in enumerate(keys_table):
            table1=[self.table[j].loc[species[items]] for items in range(len(species))]
            ...
            Histo.plotmany(np.arange(len(species)).tolist(), alpha=alpha_h, colors=list(np.array(['b', 'g', 'r', 'k', 'purple', 'orange'])[np.arange(len(species)).tolist()]), normed=True, Show=False)
        ...
        for i,  j in enumerate(matrix_index[np.triu_indices(len(keys_table), 1)]):
            ...
            for it in range(cycle):
                indices = indices[1:] + [indices[0]]
            for idx in indices:
                x = self.table[keys_table[np.triu_indices(len(keys_table), 1)[1][i]]].loc[species[idx]]
                DX = x.max()-x.min()
                y = self.table[keys_table[np.triu_indices(len(keys_table), 1)[0][i]]].loc[species[idx]]
                DY= y.max()-y.min()
                if kde_plot:
                    nbins=nbins
                    k = kde.gaussian_kde(np.array([x, y]))
                    ...
                    if type(n_levels)!=int:
                        if n_levels[0]=='1d':
                            niveaux = np.sort([np.hstack(z)[np.hstack(z).argsort()[::-1][myf.find_nearest(np.cumsum(np.sort(np.hstack(z))[::-1]),(1-2*(1-norm_gauss.cdf(j)))*np.sum(z))[0]]] for j in n_levels[1]])
                        elif n_levels[0]=='2d':
                            niveaux = np.sort([np.hstack(z)[np.hstack(z).argsort()[::-1][myf.find_nearest(np.cumsum(np.sort(np.hstack(z))[::-1]),(1-np.exp(-0.5*j**2))*np.sum(z))[0]]] for j in n_levels[1]])
                        niveaux = np.hstack(niveaux)
                        niveaux = np.sort(niveaux)
                        ...
                        nlevel=n_levels
                        niveaux=None
                    plt.contourf(xi1, yi1, z.reshape(xi1.shape), nlevel, cmap=liste[idx], levels=niveaux, alpha=alpha_c)
                    plt.contour(xi1, yi1, z.reshape(xi1.shape), n_levels, colors=contour_color, levels=niveaux, linewidths=1)
                ...
                if color_param is not None:
                    ...
                    iq = myf.IQ(color)
                    color[color>np.percentile(color,75)+1.5*iq] = np.percentile(color,75)+1.5*iq
                    color[color<np.percentile(color,25)-1.5*iq] = np.percentile(color,25)-1.5*iq


                plt.scatter(x.iloc[sample], y.iloc[sample], c=color, s=s, edgecolor=edgecolor, alpha=alpha_s)
            ...
            plt.xlim(x_limites[(save-1)%len(keys_table)]); plt.ylim(x_limites[(save-1)//len(keys_table)])
        plt.subplots_adjust(hspace=0, top=0.99, left=0.01, right=0.99, bottom=0.09)
        self.table = self.table_backup

    
    def splitting_data(self, col_species, cols=None, categorical=False, seed=0, test_size=0.2, outliers = ['inter',2]):
            ...
            for name in keys_table:
                ...
                if cols[0]=='r':
                    if name not in cols[1]:
                        key=np.append(key, name)
            keys_table = key
        X = self.table.dropna(axis=0)[keys_table].copy() ; X = X.reset_index(drop=True)
        y = self.table.dropna(axis=0)[col_species].copy() ;  y = y.reset_index(drop=True)
        ...
        X = X.loc[out] ; y = y.loc[out] ; X = X.reset_index(drop=True) ; y = y.reset_index(drop=True)
    ...
    def Machine_learning(self, col_species, xgb = True, logi = True, pred_full = False, objective='binary:logistic', plot_imp = False, cols = None, seed=0, outliers = ['inter',2], num_splits = 3, test = [1,'xgb'], print_infos=True, regular=1, n_est = 300, alpha=0.1, ax1 = None, ax2=None,ax3=None):
        ...
        for j in X.keys():
            out = out & rm_out(X[j], m = outliers[1], kind = outliers[0])[0]
        X = X.loc[out] ; y = y.loc[out] ; X = X.reset_index(drop=True) ; y = y.reset_index(drop=True)
        keys_table = np.setdiff1d(keys_table,np.array(list((X.nunique().index)[X.nunique()==1])))
        ...
        self.save_prog = []
        if test[0]!=0:
            accuracy_xgb = [] ; accuracy_logi = [] ; precision_xgb = [] ; precision_logi = [] ; recall_xgb = [] ; recall_logi = [] ; coeff_power_xgb=[] ; coeff_power_logi = []
            for j in np.arange(test[0]):
                skf = StratifiedKFold(n_splits=num_splits, shuffle = True, random_state = j+seed)
                for train_index, test_index in skf.split(X, y):
                    X_train, X_test = X.iloc[train_index], X.iloc[test_index]
                    y_train, y_test = y.iloc[train_index], y.iloc[test_index]
                    #XGB
                    if test[1] != 'logi':
                        model = XGBClassifier(objective = objective, n_estimators = n_est, learning_rate = alpha)
                        model.fit(X_train, y_train)
                        mat = metrics.confusion_matrix(y_test, model.predict(X_test))
                        precision_xgb.append(np.diag(mat)/(np.sum(mat, axis=0)+0.001))
                    ...
                    if (test[0]==1)&(print_infos==True)&(test[1]!='logi'):
                        ...
                        eval_metric = ["error"]
                        model1 = XGBClassifier(objective = objective, n_estimators = n_est, learning_rate = alpha)
                        model1.fit(X_train, y_train, eval_metric = eval_metric, eval_set = eval_set, verbose = False)
                        #graphic to show how iteration increase precision + diagnostiv of overfitting
                        ax3 = plt.subplot(1,2,1)
                        y1 = 100*(1-np.array(model1.evals_result_['validation_0']['error']))
                        y2 = 100*(1-np.array(model1.evals_result_['validation_1']['error']))
                        ax3.plot(myf.smooth(y1,5),color='k')
                        ax3.plot(myf.smooth(y2,5))
                        self.save_prog.append([y1,y2])
                        ax3.set_ylabel('Accuracy [%]') ; ax3.set_xlabel('Iteration')  
                        ...
                        for k in np.arange(0,0.45,0.01):
                            ...
                            output.append([CAA,CBB,len(proba2),100*(CAA+CBB)/len(proba2)])
                        output = np.array(output)
                        ax2 = plt.subplot(2,2,2)
                        self.save_sim.append(output) 
                        ax2.plot(0.5+np.arange(0,0.45,0.01),myf.smooth(output[:,3],3)) ; ax = plt.gca() ; ax2.set_ylabel('accuracy [%]')
                        ax3 = plt.subplot(2,2,4,sharex=ax)
                        ax3.plot(0.5+np.arange(0,0.45,0.01),myf.smooth(output[:,2],3)) ; ax3.set_ylabel('Size sample') ; ax3.set_xlabel('Confidence level threshold')
                        ...
                        print('\n Final report : \n '+np.str(metrics.classification_report(y_test, model.predict(X_test)))+'\n\n')

                    #Logistique
                    if test[1] != 'xgb':
                        regularization = regular
                        regressor = linear_model.LogisticRegression(penalty = 'l2', class_weight='balanced', C = regularization)
                        regressor.fit(X_train, y_train)
                        mat = metrics.confusion_matrix(y_test, regressor.predict(X_test))
                        precision_logi.append(np.diag(mat)/(np.sum(mat, axis=0)+0.001))
                        recall_logi.append(np.diag(mat)/np.sum(mat, axis=1))
                        accuracy_logi.append(np.trace(mat)/np.sum(mat))
                        coeff_power_logi.append(regressor.coef_[0]*np.std(X_train, axis=0))
                    ...
                    if (test[0]==1)&(print_infos==True)&(test[1]!='xgb'):
                        ...
                        print('\n Final report : \n '+np.str(metrics.classification_report(y_test, regressor.predict(X_test))))
            
            
            self.acc_xgb = np.array([0]) ; self.prec_xgb = np.array([0]) ; self.recall_xgb = np.array([0])
            self.acc_logi = np.array([0]) ; self.prec_logi = np.array([0]) ; self.recall_logi = np.array([0])
            
            if test[1] != 'logi':
                self.acc_xgb = np.array(accuracy_xgb)
                self.prec_xgb = np.array(precision_xgb)
                self.recall_xgb = np.array(recall_xgb)
                self.coeff_power_xgb = pd.DataFrame(np.array(coeff_power_xgb),columns=keys_table).describe(percentiles=[0.025, 0.16, 0.50, 0.84, 0.975])
            
            if test[1] != 'xgb':
                self.acc_logi = np.array(accuracy_logi)
                self.prec_logi = np.array(precision_logi)
                self.recall_logi = np.array(recall_logi)
                self.coeff_power_logi = pd.DataFrame(np.array(coeff_power_logi), columns=keys_table).describe(percentiles=[0.025, 0.16, 0.50, 0.84, 0.975])
...
#            self.infos = self.infos.T
        ...
        if logi == True:
            ...
            if pred_full:
                ...
                self.table.loc[self.table[col_species].isna(),'predictions_logi'] = y_pred
            
            print('\n\n Regressor score on test sample : %s' %(regressor.score(X_train, y_train)))
        ...
        if xgb==True:
            ...
            if plot_imp == True :
                ...
                columns_name = []
                if ax1 is not None:
                    plot_importance(model,ax=ax1,importance_type='gain',title='Feature imp. (gain)')
                    m1 = np.array([[ax1.patches[i].get_width() for i in ax1.get_yticks()],[ax1.get_ymajorticklabels()[i].get_text() for i in ax1.get_yticks()]])
                    m1 = m1[:,np.argsort(m1[1])]
                    m.append(m1)
                    columns_name.append('gain')
                if ax2 is not None:
                    plot_importance(model,ax=ax2,importance_type='weight',title='Feature imp. (weight)')  
                    m2 = np.array([[ax2.patches[i].get_width() for i in ax2.get_yticks()],[ax2.get_ymajorticklabels()[i].get_text() for i in ax2.get_yticks()]])
                    m2 = m2[:,np.argsort(m2[1])]
                    m.append(m2)
                    columns_name.append('weight')
                if ax3 is not None:
                    plot_importance(model,ax=ax3,importance_type='cover',title='Feature imp. (cover)')  
                    m3 = np.array([[ax3.patches[i].get_width() for i in ax3.get_yticks()],[ax3.get_ymajorticklabels()[i].get_text() for i in ax3.get_yticks()]])
                    m3 = m3[:,np.argsort(m3[1])]
                    m.append(m3)
                    columns_name.append('cover')
                
                self.feature_importance = pd.DataFrame((np.array(m)[:,0,:].astype('float')).T,index=m[0][1],columns=columns_name)
            ...
            if pred_full:
                ...
                X_topred = self.table[keys_table][self.table[col_species].isna()]
            y_pred = model.predict_proba(X_topred)[:,1] #changed to predict proba 16.11.20
            ...
            if pred_full:
                ...
                self.table.loc[self.table[col_species].isna(),'predictions_xgb'] = y_pred            
    ...
    def barplot(self,who=['xgb','logi']):
        if ('xgb' in who)&('logi' not in who):
            ...
            plt.bar(self.coeff_power_xgb.columns[sort],self.coeff_power_xgb.loc['50%'][sort],yerr=[self.coeff_power_xgb.loc['50%'][sort]-self.coeff_power_xgb.loc['16%'][sort],self.coeff_power_xgb.loc['84%'][sort]-self.coeff_power_xgb.loc['50%'][sort]])
        elif ('xgb' not in who)&('logi' in who):
            sort = self.coeff_power_logi.loc['50%'].argsort()[::-1]
            plt.bar(self.coeff_power_logi.columns[sort],self.coeff_power_logi.loc['50%'][sort],yerr=[self.coeff_power_logi.loc['50%'][sort]-self.coeff_power_logi.loc['16%'][sort],self.coeff_power_logi.loc['84%'][sort]-self.coeff_power_logi.loc['50%'][sort]])
        else :
            sort_logi = self.coeff_power_logi.loc['50%'].argsort()[::-1]
            sort_xgb = self.coeff_power_xgb.loc['50%'].argsort()[::-1]
            plt.subplot(1,2,2)
            plt.bar(self.coeff_power_logi.columns[sort_logi],self.coeff_power_logi.loc['50%'][sort_logi],yerr=[self.coeff_power_logi.loc['50%'][sort_logi]-self.coeff_power_logi.loc['16%'][sort_logi],self.coeff_power_logi.loc['84%'][sort_logi]-self.coeff_power_logi.loc['50%'][sort_logi]])
            plt.subplot(1,2,1)
            plt.bar(self.coeff_power_xgb.columns[sort_xgb],self.coeff_power_xgb.loc['50%'][sort_xgb],yerr=[self.coeff_power_xgb.loc['50%'][sort_xgb]-self.coeff_power_xgb.loc['16%'][sort_xgb],self.coeff_power_xgb.loc['84%'][sort_xgb]-self.coeff_power_xgb.loc['50%'][sort_xgb]])
...
class tableXY(object):
    ...
    def rm_from_plot(self,invers=False):
        ...
        self.x = new.x ; self.y = new.y
        self.xerr = new.xerr ; self.yerr = new.yerr
    
    def export(self, name):
        """export in pickle format """
        myf.pickle_dump(pd.DataFrame(np.array([self.x, self.y, self.yerr]).T, columns=['x', 'y', 'yerr']), open(name, 'wb'))
    ...
    def cross_corr(self, array, fill_hole=False, dx=1, Draw=True, save=True, periodic=True,num_sim=100):
        self.y -= np.median(self.y)
        array -= np.median(array)
        
        indices = [np.arange(len(self.y))] # put the real correlation in the bootstrap vector
        for j in range(num_sim):
            indices.append(np.random.choice(np.arange(len(self.y)),len(self.y),replace=False))
        ...
        new_y_random[:,match[:,1].astype('int')] = random1[:,match[:,0].astype('int')]
        new_y2_random[:,match[:,1].astype('int')] = random2[:,match[:,0].astype('int')]        
        ...
        for i,k in zip([0,len(new_grid)],[0,1]):
            ...
            for j in range(len(new_grid)):
                ...
                if not periodic:
                    slider = np.insert(slider,i,np.nan,axis=1)[:,k:np.shape(slider)[1]+k]
                else:
                    slider = np.roll(slider,1,axis=0)     
        autocorr = np.array(autocorr_test)[:,0]
        noise = np.array(autocorr_test)[:,1:]
        
        noise_level1 = np.nanmedian(noise,axis=1) + 1.5*(np.nanpercentile(noise,75,axis=1) - np.nanpercentile(noise,25,axis=1))
        ...
        if Draw:
            plt.scatter(np.arange(len(autocorr))*dx-(len(autocorr)//2)*dx,autocorr,color='k')
            plt.plot(np.arange(len(autocorr))*dx-(len(autocorr)//2)*dx,noise_level1,color='r')
        
        if save:
            self.crosscorrelation = tableXY(np.arange(len(autocorr))*dx-(len(autocorr)//2)*dx,autocorr)
        else:
            return tableXY(np.arange(len(autocorr))*dx-(len(autocorr)//2)*dx,autocorr,0/np.sqrt(np.ones(len(autocorr)))), noise_level1
    
    def autocorr(self, fill_hole=False, dx=1, Draw=True, save=True,
                 periodic=False, norm=False, peak_vicinity = 10, num_sim=100):
        if save:
            
            self.autocorrelation, noise_level = self.cross_corr(self.y, fill_hole=fill_hole, dx=dx, Draw=False, periodic=periodic,save=False,num_sim=num_sim)
            ...
            noise_level.y[center] = 0.5*(noise_level.y[center-1]+noise_level.y[center+1])
            ...
            if norm:
                ...
                highest = self.autocorrelation.clipped.x_max[np.argmax(self.autocorrelation.clipped.y_max)]
                print('highest_peak : %.1f'%(highest))
                reference = tableXY(self.x,np.sin(2*np.pi/highest*self.x))
                normalisation, dust = reference.cross_corr(reference.y,fill_hole=fill_hole, dx=dx, Draw=False, periodic=periodic, save=False)
                ...
                maximum.interpolate(new_grid=self.autocorrelation.x,replace=True,method='linear')
            ...
            if Draw:
                ...
                plt.subplot(2,1,1)
                plt.plot(self.autocorrelation.x,self.autocorrelation.y*100,color='k')
                plt.plot(self.autocorrelation.x,noise_level*100,color='r',label='1.5IQ level')
                ...
                plt.plot(self.autocorrelation.x,abs(self.autocorrelation.y*100),color='k')
                plt.plot(self.autocorrelation.x,noise_level*100,color='r')
                plt.ylim(1e-2,100)
                plt.xlim(0,None)
                plt.yscale('log')                
            ...
            return self.cross_corr(self.y, fill_hole = fill_hole, dx = dx, Draw = Draw, periodic = periodic, save = False, num_sim = num_sim)
    ...
    def switch_for(self,vec='old_values'):
        if type(vec)==str:
            self.y, self.yerr, self.x, self.xerr = self.oldy, self.oldyerr, self.oldx, self.oldxerr
        else:
            self.oldy, self.oldyerr, self.oldx, self.oldxerr = self.y, self.yerr, self.x, self.xerr
    ...
    def frac(self,frac,replace=False):
        ...
        if replace:
            ...
            self.sub_frac = tableXY(self.x[sel],self.y[sel],self.xerr[sel],self.yerr[sel])
    ...
    def add_noise(self,noise,noise_x=0):
        ...
        self.noisy = tableXY(self.x + np.random.randn(len(self.x))*noise_x, 
                             self.y + np.random.randn(len(self.y))*noise,
                             np.sqrt(self.xerr**2+noise_x**2),
                             np.sqrt(self.yerr**2+noise**2))
    def rescale_yerr(self):
        self.yerr =  np.nanstd(self.y)*np.ones(len(self.yerr))+(self.yerr-self.yerr)
    ...
    def scatter(self,c = 'none',alpha=np.ones(1),cmap='viridis',zorder=100,vmin=None,vmax=None,edgecolor=None):
        if type(c)!=str:
        ...
        if len(alpha)!=1:
            c1[:,-1] = alpha
            plt.scatter(self.x[self.x.argsort()], self.y[self.x.argsort()], c = c1, zorder=zorder)
        else:
            if vmin is None:
                vmin = np.nanpercentile(c,16)
            if vmax is None:
                vmax = np.nanpercentile(c,84)
            plt.scatter(self.x[self.x.argsort()], self.y[self.x.argsort()], c = c, cmap=cmap, zorder=zorder, vmin=vmin, vmax=vmax, edgecolor=edgecolor)
    ...
    def substract_calib(self,calib):
        ...
        if calib=='HARPS_CCF_fwhm':
            self.calib.y -= np.polyval(np.array([3.73878777e-06, 2.50768767e+00]),self.calib.x) #calibrated on HD10700 
            
        elif calib=='HARPS_CCF_contrast':
            self.calib.y -= np.polyval(np.array([6.57656028e-07, 3.85548927e-01]),self.calib.x) #calibrated on HD10700
    ...
    def two_point_smoothing(self, dx_max = 1,replace=True):
        ...
        for j in range(len(self.x)-1):
            if points_to_average[j]:
                ...
                newyerr.append((1/self.yerr[j]**2+1/self.yerr[j+1]**2)**(-0.5))
                newy.append((self.y[j]/self.yerr[j]**2+self.y[j+1]/self.yerr[j+1]**2)/(1/self.yerr[j]**2+1/self.yerr[j+1]**2))
            else:
        ...
        smoothed = interp1d(self.sx, self.sy, kind='linear', bounds_error=False, fill_value='extrapolate')(self.x)
        smoothed[0] = first
        smoothed[-1] = last
        
        self.smoothed = tableXY(self.x,
                                smoothed,
                                self.xerr,
                                self.yerr)
    
        if replace:
            self.y = interp1d(self.sx, self.sy, kind='linear', bounds_error=False, fill_value='extrapolate')(self.x)
            self.y[0] = first
            self.y[-1] = last

        else:
            self.y_smoothed = interp1d(self.sx, self.sy, kind='linear', bounds_error=False, fill_value='extrapolate')(self.x)
            self.y_smoothed[0] = first
            self.y_smoothed[-1] = last
    
    def myscatter(self,num=True, liste=None, factor=30,color='b', alpha=1, x_offset=0, y_offset=0, color_text='k',modulo=None):
        ...
        if num:
            for i, txt in enumerate(n):
                plt.annotate(txt, (np.array(newx)[i]+x_offset, np.array(self.y)[i]+y_offset),color=color_text)
        if liste is not None:
            for i, txt in enumerate(liste):
                plt.annotate(txt, (np.array(newx)[i]+dx+x_offset, np.array(self.y)[i]+dy+y_offset),color=color_text)           
            
    def myannotate(self, liste, colors=None, colors_line=None, rotation=None, direction='x', frac_shift=0.2, draw_line=False, fontsize=14, reference='data', color_cycle=['k','b','r','g'], n_first=None):
        ax = plt.gca()
        ...
        for n,x in enumerate(values):
            if str(liste[n])!='':
                ...
                if reference=='data':
                    ...
                    val = [axes[0],axes[1]][cond]+frac_shift*span*np.sign(cond-0.5)
                    val_line = [axes[0],axes[1]][cond]+frac_shift*span*np.sign(cond-0.5)*0.9             
                if direction=='x':
                    plt.annotate('%s'%(str(liste[n])),(val,self.y[n]),ha=['right','left'][cond],va='center',rotation=rotation,fontsize=fontsize,color=colors_text[n])
                    if draw_line:
                        plt.plot([val_line,self.x[n]],[self.y[n],self.y[n]],alpha=0.2,color=colors_line[n])
                else:
                    plt.annotate('%s'%(str(liste[n])),(self.x[n],val),ha='center',va=['top','bottom'][cond],rotation=rotation,fontsize=fontsize,color=colors_text[n])
                    if draw_line:
                        plt.plot([self.x[n],self.x[n]],[val_line,self.y[n]],alpha=0.2,color=colors_line[n])                
    ...
    def center_symmetrise(self,center,replace=False,Plot=False):
        ...
        sym_kernel = np.ravel(np.mean(np.array([kernel.y[0:nb_elem-1],kernel.y[nb_elem:][::-1]]),axis=0))
    ...
    def plot_outliers_cut(self, m=1.5, kind='inter', color=0, label=None):
        ...
        plt.scatter(self.x[~mask_out],self.y[~mask_out],marker='o',color=cycle_colors[color])
        plt.scatter(self.x[mask_out],self.y[mask_out],marker='x',color=cycle_colors[color])
        
    def plot_lowest_perio(self,table_xy2,color1='b',color2='g',zorder_min=10,alpha=1):
        ...
        c1 = v1.copy() ; c1[mask_inf] = np.nan
        c2 = v1.copy() ; c2[mask_sup] = np.nan
        c3 = v2.copy() ; c3[mask_inf] = np.nan
        c4 = v2.copy() ; c4[mask_sup] = np.nan
    ...
    def fill_between(self, color='k', alpha=0.2, borders=True, label=None, oversampling=1, scale='lin'):
        ...
        x = self.x ; y = self.y ; inf = self.xerr ; sup = self.yerr
        if oversampling>1:
    ...
    def plot(self, Show=False, color='k', label='', ls='',offset=0, mask=None, capsize=3, fmt='o', markersize=6, zorder=1, species=None, alpha=1, modulo=None, modulo_norm=False, cmap='viridis', new=False, phase_mod=0, periodic=False, frac=1, yerr=True):
        ...
        for num, selection in enumerate(np.unique(species)):
            ...
            if ls!='':
                plt.plot(self.x[mask2][loc][sel],self.y[mask2][loc][sel]+offset,ls=ls,zorder=zorder,label=label,color=color,alpha=alpha)
            else:
                if modulo is not None:
                    ...
                    plt.scatter(new_x[sel], self.y[mask2][loc][sel]+offset, marker='o', c=self.x[mask2][loc][sel], cmap=cmap, s=markersize,zorder=zorder*100)
                    if periodic:
                    ...
                    plt.errorbar(self.x[mask2][loc][sel], self.y[mask2][loc][sel]+offset, xerr=self.xerr[mask2][loc][sel], yerr=self.yerr[mask2][loc][sel]*int(yerr), fmt=fmt, color=color, alpha=alpha, capsize=capsize, label=label, markersize=markersize,zorder=zorder)
        if Show==True:
            plt.legend()
            plt.show()
    ...
    def binning(self, bin_width, db=0, replace=False, debug = False):
        ...
        mini = self.x.min() ; maxi =self.x.max() 
        binning = np.arange(mini+db,maxi,bin_width)
        if debug:
            print(binning,bin_width)
        self.binx = binning+bin_width/2
        t = (self.x[:,np.newaxis]<self.binx+bin_width/2.)*(self.x[:,np.newaxis]>self.binx-bin_width/2.)
        Y = self.y[:,np.newaxis]*t ; Yerr = self.yerr[:,np.newaxis]*t
        Yerr[Yerr==0] = np.inf ; Wy = 1/Yerr**2
        ...
        self.binxerr = np.ones(len(self.binx))*bin_width/2. 
        self.biny = new_y
        self.binyerr = new_yerr
        
        mask = ~np.isnan(self.binx)&~np.isnan(self.biny)&~np.isnan(self.binyerr)&~np.isnan(self.binxerr)
    ...
    def joint_plot_multi(self, classes=None, columns=None, kind='kde', space=0, stat_func=None, shade=True, 
               shade_lowest=False, n_levels=['2d', [0.5,1, 2]], colors_s = ['r','b','g'], nbins=50, s=5, alphas=0.5,alphac=0.8):
        ...
        if type(classes)==np.ndarray:
            self.table = pd.DataFrame(np.array([self.x, self.y, classes]).T, columns=columns+['classes'])
        else:
            self.table = pd.DataFrame(np.array([self.x, self.y, np.array(['name']*len(self.x))]).T, columns=columns+['classes'])
    
        g = sns.JointGrid(columns[0], columns[1], self.table,space=space)
        i=0
        for day, day_tips in self.table.groupby('classes'):
            x = np.array(day_tips[columns[0]]).astype('float') ; DX = x.max()-x.min()
            y = np.array(day_tips[columns[1]]).astype('float') ; DY= y.max()-y.min()
            ...
            if kind=='kde':
                sns.kdeplot(day_tips[columns[0]], ax=g.ax_marg_x, legend=False, color=colors[i], shade=True, alpha=0.5)
                sns.kdeplot(day_tips[columns[1]], ax=g.ax_marg_y, legend=False, vertical=True,color=colors[i],shade=True, alpha=0.5)
                sns.kdeplot(day_tips[columns[0]], ax=g.ax_marg_x, legend=False, color='k', shade=False)
                sns.kdeplot(day_tips[columns[1]], ax=g.ax_marg_y, legend=False, vertical=True, color='k',shade=False)
            if kind=='hist':
                g.ax_marg_x.hist(np.array(day_tips[columns[0]]).astype('float'), bins = np.linspace(self.x.min(),self.x.max(),10), color=colors[i], alpha=0.5)
                g.ax_marg_y.hist(np.array(day_tips[columns[1]]).astype('float'), bins = np.linspace(self.y.min(),self.y.max(),10), color=colors[i], alpha=0.5,orientation='horizontal')
            sns.kdeplot(day_tips[columns[0]],day_tips[columns[1]], ax=g.ax_joint, legend=False,cmap=cmaps[i], alpha=alphac, shade=shade, shade_lowest=shade_lowest, n_levels=niveaux[1:])  
            sns.kdeplot(day_tips[columns[0]],day_tips[columns[1]], ax=g.ax_joint, legend=False,color='k', alpha=1, shade=False, n_levels=niveaux[1:])  
        ...
        sns.scatterplot(columns[0], columns[1],hue='classes', ax=g.ax_joint,data=self.table,zorder=0,palette=colors_s)
        g.ax_joint.set_xlabel(columns[0], fontsize=15)
        g.ax_joint.set_ylabel(columns[1], fontsize=15)
        self.graphique = g
    
    def joint_plot(self, classes=None, columns=None, kind='kde', space=0, stat_func=None, marginal_kws={"color":"black", "lw":0.5}, 
                   joint_kws={"colors":None,"cmap":'Greys'}, shade=True, 
                   shade_lowest=False, n_levels=['2d', [0.5, 1, 2, 3]],nbins=50,s=5,alphas=0.5):
        import seaborn as sns ; sns.set(style="ticks", color_codes=True)
        ...
        x = self.x ; DX = x.max()-x.min()
        y = self.y ; DY= y.max()-y.min()
        ...
        if kind=='scatter':
            self.graphique = sns.jointplot(x=str(self.table.keys()[0]), y=str(self.table.keys()[1]), data=self.table, kind='scatter', space=space)
        else:
            if type(n_levels)!=int:
                if n_levels[0]=='1d':
                    niveaux = np.sort([np.hstack(z)[np.hstack(z).argsort()[::-1][myf.find_nearest(np.cumsum(np.sort(np.hstack(z))[::-1]),(1-2*(1-norm_gauss.cdf(j)))*np.sum(z))]] for j in n_levels[1]])
                ...
                n_levels = niveaux
            self.graphique = sns.jointplot(x=str(self.table.keys()[0]), y=str(self.table.keys()[1]), data=self.table, 
                                           kind=kind, space=space, stat_func=stat_func, marginal_kws=marginal_kws, 
                                           joint_kws=joint_kws, shade=shade, 
                                           thresh=shade_lowest, n_levels=n_levels)
            self.graphique.plot_joint(plt.scatter, c="k", s=s,linewidth=1, marker="o",alpha=alphas)
            self.graphique.ax_joint.collections[0].set_alpha(0) #remove the first contour
 
    
    def binned_scatter(self, bin, extend=True, Plot=True, Show=False, color='k', size=5, xlabel='', ylabel='', alpha_p=0.5, cap=True, ls='', lw=None,replace=False):
        ...
        if (type(bin)==np.ndarray)&(extend==True):
            ...
            num1 = int(abs(xmin-bin[0])/dbin)+2 ; num2 = int(abs(xmax-bin[-1])/dbin)+2
        ...
        for j in np.arange(len(bins)-1):
            if sum((self.x>bins[j])&(self.x<=bins[j+1]))>1:            
                binning[j] = np.median(self.y[(self.x>bins[j])&(self.x<=bins[j+1])])
            if cap==True:
                if sum((self.x>bins[j])&(self.x<=bins[j+1]))>1:
                    binning_sup = np.percentile(self.y[(self.x>bins[j])&(self.x<=bins[j+1])], 84)
                    binning_inf = np.percentile(self.y[(self.x>bins[j])&(self.x<=bins[j+1])], 16)
                    bin2sig_sup = np.percentile(self.y[(self.x>bins[j])&(self.x<=bins[j+1])], 97.5)
                    bin2sig_inf = np.percentile(self.y[(self.x>bins[j])&(self.x<=bins[j+1])], 2.5)
        ...
        self.binx = np.array([np.mean([bins[j], bins[j+1]]) for j in np.arange(len(bins)-1)])
        self.biny = binning
        self.binsup = [err_binning_sup, err_binning2_sup]
        self.bininf = [err_binning_inf, err_binning2_inf]
        
        matrice = np.array([bins[0:-1],bins[1:]])
        bins_idx = (self.x > matrice[0][:,np.newaxis])&(self.x <= matrice[1][:,np.newaxis])
        ...
        if replace:
            ...
            self.binned_data = tableXY(self.binx,self.biny,0.5*(self.binsup[0]+self.bininf[0]))
        
        if Plot:
            if alpha_p:
                plt.scatter(self.x, self.y, marker='o', color=color, edgecolor='k', alpha=alpha_p, s=size)
            if cap==True:
                xerr = (bins[1]-bins[0])/2
            else:
                xerr = np.zeros(len(binning))
            plt.errorbar(self.binx, binning, 
            xerr=xerr, yerr=[err_binning_inf, err_binning_sup], c=color, marker='o', capsize=4, markeredgecolor='k',ls=ls,lw=lw)
            plt.xlabel(xlabel, fontsize=14) ; plt.ylabel(ylabel, fontsize=14)  
            if Show==True:
                plt.show()

    def binned_area(self, bin, extend=True, Show=False, color='k', size=5, xlabel='', ylabel='', alpha=0.7, alpha_p=0.5):
        """allow to binned a scatter plot giving either the number of bins or the list of binned. If extend =True extend, the list of bins to match with the border of the data"""
        ...
        for j in np.arange(len(bins)-1):
            ...
            if sum((self.x>bins[j])&(self.x<=bins[j+1]))>5:
                ...
                err_binning_sup[j] = binning_sup - binning[j]
                err_binning_inf[j] = binning[j] - binning_inf
        plt.plot(np.array([np.mean([bins[j], bins[j+1]]) for j in np.arange(len(bins)-1)]), binning, color=color)
        plt.fill_between(np.array([np.mean([bins[j], bins[j+1]]) for j in np.arange(len(bins)-1)]),binning - err_binning_inf,err_binning_sup+binning,  color=color, alpha=alpha)
        plt.xlabel(xlabel, fontsize=14) ; plt.ylabel(ylabel, fontsize=14)  
        if Show==True:
            plt.show()
    ...
    def split_species(self,species):
        self.species_splited = [self.masked(species==s,replace=False) for s in np.unique(species)]
        self.species_name = np.unique(species)
        self.species_mederr = {i:np.nanmedian(j.y) for i,j in zip(np.unique(species),self.species_splited)}
    ...
    def split_seasons(self,min_gap=50,Plot=False):
        ...
        for i in range(len(self.seasons_splited)):
            ...
            self.seasons_meany_std.append(1/np.sqrt(np.nansum(1/self.seasons_splited[i].yerr**2)))
    ...
    def seasons_periodogram(self, min_gap=40, split_graph=False, infos=False, deg=0, legend=None, axis_y_var='p', alpha=1):
        self.split_seasons(min_gap=min_gap)
        maxi = []
        c=1
        ...
        for j in range(len(seasons_cuted)):
            try:
                ...
                mean_time = Time.Time(np.mean(seasons_cuted[j].x),format='mjd').decimalyear
                
                seasons_cuted[j].substract_polyfit(deg,replace=False)
                seasons_cuted[j].detrend_poly.periodogram(Norm=True,norm_val=True,color=None,legend=legend,infos=infos,level=0.90,axis_y_var=axis_y_var,alpha=alpha) #10%FAP
                maxi.append(np.max(seasons_cuted[j].detrend_poly.power/seasons_cuted[j].detrend_poly.fap))
                plt.ylabel('power')
    ...
    def species_recontinuity(self, dist_max=100, alpha=1):
        ...
        if len(self.species_name)>1:
            match = myf.match_unique_closest(self.species_splited[0].x,self.species_splited[1].x)
            match[:,-1] = np.abs(match[:,-1])
            match = match[match[:,-1]!=0]
            
            if sum(match[:,-1]<dist_max)<2:
                offset=0
                print(' [ERROR] No points closer than %s, closest pair found at : %s'%(str(dist_max),str(np.nanmin(match[:,-1]))))
            else:
                ...
                diff = self.species_splited[1].y[match[:,1].astype('int')] - self.species_splited[0].y[match[:,0].astype('int')]
                diff_std = (1/(match[:,-1]+0.001)**2) * np.sqrt(self.species_splited[1].yerr[match[:,1].astype('int')]**2 + self.species_splited[0].yerr[match[:,0].astype('int')]**2)
    ...
    def merge_species(self,replace=False):
        ...
        xerr_merged = np.hstack([self.species_splited[j].xerr for j in range(nb_species)])
        y_merged = np.hstack([self.species_splited[j].y for j in range(nb_species)])
        yerr_merged = np.hstack([self.species_splited[j].yerr for j in range(nb_species)])
        species_merged = np.hstack([[self.species_name[j]]*len(self.species_splited[j].x) for j in range(nb_species)])
        ...
        if replace:
            ...
            self.species_merged = tableXY(x_merged[order],y_merged[order],xerr_merged[order],yerr_merged[order])
    ...
    def extend_clustering(self,extend):
        ...
        val = val[val!=0]
        self.y = myf.flat_clustering(len(self.x),borders,elevation=val,extended=extend)
    ...
    def chunck(self,idx1,idx2,inv=False):
        ...
        if not inv:
            ...
            chunk = tableXY(np.hstack([self.x[0:idx1],self.x[idx2:]]),
    ...
    def my_periodogram(self, p_min=1, p_max=None, p_nb=None, nb_bins=20, dbin=4, min_element_in_bins=10, bootstrap=100, num_sim=10, Draw=True, freq=False):
        """
        ...
        random = np.array([np.random.choice(np.arange(len(self.x)),len(self.x),replace=False) for num in range(bootstrap)])
        ...
        std_random = []
        for j in tqdm(range(bootstrap)):
            std_days_random = np.nansum(conds*(self.y - mean_random[j][:,:,:,np.newaxis])**2,axis=3)
            normalisation = nb_days_in_bins
        ...
        self.myp_ts = np.array([(self.myp_bins*np.ones(p_nb)[:,np.newaxis]).T,mean,std])

 
        if not freq:
            bdiff = tableXY(liste_p[1:-1],c1[1:-1] / c3[1:-1])
            bdiff.autocorr(Draw=False,num_sim=num_sim,dx = np.unique(np.diff(liste_p))[0])
            bdiff2 = tableXY(liste_p[1:-1],c2[1:-1] / c4[1:-1])
            bdiff2.autocorr(Draw=False,num_sim=num_sim,dx = np.unique(np.diff(liste_p))[0])           
        c_ratio1 = c1/c3
        c_ratio2 = c2/c4
        ...
        if Draw:
            if not freq:
                ...
                plt.plot(bdiff.autocorrelation.x,bdiff.autocorrelation_noise_level,color='r')
                ...
                plt.plot(bdiff2.autocorrelation.x,bdiff2.autocorrelation_noise_level,color='r')
                plt.axhline(y=0,color='k',alpha=0.3)
                
                plt.subplot(3,3,9,sharex=ax1)
        
                test = tableXY(liste_p,k1)
                test.x = test.x[1:-1] ; test.y = test.y[1:-1]
                test.xerr = test.xerr[1:-1] ; test.yerr = test.yerr[1:-1]
                test.autocorr(Draw=False,num_sim=num_sim,dx = np.unique(np.diff(liste_p))[0])
        
                test2 = tableXY(liste_p,k2)
                test2.x = test2.x[1:-1] ; test2.y = test2.y[1:-1]
                test2.xerr = test2.xerr[1:-1] ; test2.yerr = test2.yerr[1:-1]
                test2.autocorr(Draw=False,num_sim=num_sim,dx = np.unique(np.diff(liste_p))[0])
                ...
                plt.plot(test.autocorrelation.x,test.autocorrelation_noise_level,color='r')
                plt.axhline(y=0,color='k',alpha=0.3)  
            else:
                plt.figure(figsize=(24,18))
                k1 = (c_ratio1+c_ratio2)/(c_ratio1*c_ratio2)
...
#        plt.ylabel('Normalised power',fontsize=14)
    ...
    def substract_bin(self, windows, method = 'interp', degree = 3, periodic=False, replace=False, 
                      Draw=False, color='r', color_p='k', lw=1.5, alpha_p=0.5, alpha_bin=1,zorder=10):
        if periodic:
            length = len(self.x)
            vec_before = tableXY(self.x-(np.max(self.x)-np.min(self.x)),self.y,self.xerr,self.yerr)
            vec_after = tableXY(self.x+(np.max(self.x)-np.min(self.x)),self.y,self.xerr,self.yerr)
            vec_before.add(self)
        ...
        if Draw:
            ...
            plt.errorbar(self.binx, self.biny, yerr=self.binyerr, xerr=self.binxerr,fmt='ro',alpha=alpha_bin)
            self.plot(color=color_p,alpha=alpha_p)
            plt.plot(new_plot.x[new_plot.x.argsort()],new_plot.y[new_plot.x.argsort()],color=color,lw=lw,zorder=zorder)
        if replace:
            self.x_backup = self.x.copy()
            self.y_backup = self.y.copy()
            self.xerr_backup = self.xerr.copy()
    ...
    def fit_dependency(self):
        ...
        sample_dist_XY = np.sum(aij*bij) /n**2-np.sum(ai*bi)*2/n**3+a*b/n**4
        sample_dist_XX = np.sum(aij*aij) /n**2-np.sum(ai*ai)*2/n**3+a*a/n**4
        sample_dist_YY = np.sum(bij*bij) /n**2-np.sum(bi*bi)*2/n**3+b*b/n**4
    ...
    def fit_base(self,base_vec,Plot=False,num_sim=1):
        ...
        new.fit_base(base_vec, weight=(1/self.yerr[:,np.newaxis].T)**2,num_sim=num_sim)
    ...
    def fit_xgb(self, base_vec, column_name=None, frac_cv=0.20, n_est=None, depth_tree=None, num_sim=10):
        ...
        def split_train_test():
            mask_cv = np.in1d(index,np.random.choice(index,size=int(len(index)*frac_cv),replace=False))
        ...
        if simulation:
            ...
            for depth in depth_tree:
                for n in n_est:
                    ...
                    for j in tqdm(range(num_sim)):
                        ...
                        model = XGBRegressor(objective='reg:squarederror', n_estimators=n, max_depth=depth)
                        model.fit(pd.DataFrame(X_train,columns=column_name), y_train)
            
                        y_cv_test = model.predict(pd.DataFrame(X_test,columns=column_name))
                        y_cv_train = model.predict(pd.DataFrame(X_train,columns=column_name))
                    ...
                    plt.errorbar(np.median(M1),np.median(M2)-np.median(M1),color='k',xerr=np.std(M1),yerr=np.std(M2),marker='o',mec='k',capsize=6,lw=3)
                    plt.errorbar(np.median(M1),np.median(M2)-np.median(M1),color=None,xerr=np.std(M1),yerr=np.std(M2),marker='o',mec='k',capsize=0,label='D=%.0f | N=%.0f'%(depth,n))
                    parameters.append([depth, n, np.median(M1), np.median(M2)-np.median(M1),np.sqrt(np.median(M1)**2+(np.median(M2)-np.median(M1))**2)])
            ...
            plt.xlim(0,1)
            plt.ylim(0,1)
            plt.xlabel(r'Underfitting = $R_{train}$ = std($Y_{train}$ - model)/std($Y_{train}$)',fontsize=16)
            ...
            print(' [INFO] Best parameters found : D=%.0f | N=%.0f'%(depth_tree,n_est))
        ...
        model_binary = XGBClassifier(objective='binary:logistic', n_estimators=n_est, max_depth=depth_tree)
        model_binary.fit(X_train, (y_train>np.median(y)).astype('int'))
        plot_importance(model_binary,ax=ax1,importance_type='gain',title='Binary feature imp. (gain)')
        ...
        plt.figure(4)
        table4 = table(pd.DataFrame(np.vstack([y_train,X_train.T]).T,columns=['species']+column_name))
        table4.pairplot(color_param='species',kde_plot=False)
                
        plt.figure(2)
        ax1 = plt.subplot(1,2,2)
        model = XGBRegressor(objective='reg:squarederror', n_estimators=n_est, max_depth=depth_tree)
        model.fit(pd.DataFrame(X_train,columns=['a','b','c','d']), y_train)
        plot_importance(model,ax=ax1,importance_type='gain',title='Regressor feature imp. (gain)')
        ...
        plt.figure(6)
        table5 = table(pd.DataFrame(np.vstack([y_pred,X_to_pred.T]).T,columns=['species']+column_name))
    ...
    def product(self,new_tableXY,const=1):
        self.y = self.y * new_tableXY.y*const
        self.yerr = const*np.sqrt((self.yerr*new_tableXY.y)**2 + (self.y*new_tableXY.yerr)**2)      
    ...
    def rolling(self,window=1,quantile=None,median=True,iq=True):
        ...
        if iq:
            self.roll_Q1 = np.ravel(pd.DataFrame(self.y).rolling(window,min_periods=1,center=True).quantile(0.25))
            self.roll_Q3 = np.ravel(pd.DataFrame(self.y).rolling(window,min_periods=1,center=True).quantile(0.75))
            self.roll_IQ = self.roll_Q3-self.roll_Q1
        if quantile is not None:
            self.roll = np.ravel(pd.DataFrame(self.y).rolling(window,min_periods=1,center=True).quantile(quantile))

    def rolling_quantile(self,window=1, quantile=0.50):        
        return np.ravel(pd.DataFrame(self.y).rolling(window,min_periods=1,center=True).quantile(quantile))
    ...
    def draw_alpha_shape(self, alpha, only_outer = True):
        ...
        edges_random = myf.alpha_shape(points_random, alpha = alpha, only_outer = only_outer)
    ...
    def kde(self, levels= ['2d',[1,2]], alpha=0.8, cmap='Greys', vmin=0.1, vmax=0.9, zorder=1, Plot=True, contour_color=None, lw=2):
        x = self.x
        y = self.y
        ...
        xx, yy = np.meshgrid(np.linspace(x.min()-dx*0.2,x.max()+dx*0.2,100),np.linspace(y.min()-dy*0.2,y.max()+0.2*dy,100))
        ...
        if levels[0]=='1d':
            zz_curve = [zz_sort[myf.find_nearest(zz_cumsum,(1-2*(1-norm_gauss.cdf(j))))[0]][0] for j in levels[1]]
        if levels[0]=='2d':
            zz_curve = [zz_sort[myf.find_nearest(zz_cumsum,(1-np.exp(-0.5*j**2)))[0]][0] for j in levels[1]]
        ...
        if alpha!=0:
            self.cs = plt.contourf(xx,yy,zz,levels = np.hstack([zz_curve[::-1],zz_sort[0]]),colors=colorlvl,alpha=alpha,zorder=zorder)
        if contour_color!=None:
            plt.contour(xx,yy,zz,levels = np.hstack([zz_curve[::-1],zz_sort[0]]), colors=contour_color, linewidths=lw)
        ...
        self.vertices_curve = [self.cs.collections[i].get_paths()[0].vertices.copy() for i in range(len(self.cs.collections))]
        polygon = Polygon([(k, l) for k,l in zip(vertices[:,0],vertices[:,1])])
        
        x = np.random.uniform(low=vertices[:,0].min(),high=vertices[:,0].max(),size=3000)
        y = np.random.uniform(low=vertices[:,1].min(),high=vertices[:,1].max(),size=3000)
...
#        #self.r2_kde = 1 - self.area_intersection/self.area_union
    ...
    def fit_alpha_shape(self, alpha = 0.2, Plot = True, kde = ['1d',[1]]):
        ...
        if self.nb_cluster_random!=1:
            print('[WARNING] : the independent joint distribution has more than 1 cluster, increase alpha')
        
        nb_pts_in_cluster = [len(self.liste_cluster[0][j]) for j in range(len(self.liste_cluster[0]))]
        nb_pts_in_cluster_random = [len(self.liste_cluster[1][j]) for j in range(len(self.liste_cluster[1]))]
        ...
        polygon_random = Polygon([(k, l) for k,l in zip(env_random[:,0],env_random[:,1])])
        ...
        for num,j in enumerate(sub_cluster):
            if sign_cluster[num]==1:
                polygon_positif = polygon_positif.union(Polygon([(k, l) for k,l in zip(np.array(save[0][j])[:,0],np.array(save[0][j])[:,1])]))
            if sign_cluster[num]==-1:
                polygon_negatif = polygon_negatif.union(Polygon([(k, l) for k,l in zip(np.array(save[0][j])[:,0],np.array(save[0][j])[:,1])]))
            area += sign_cluster[num]*myf.PolyArea(np.array(save[0][j])[:,0],np.array(save[0][j])[:,1]) 
        ...
        self.area_intersection = polygon_intersection_pos.area - polygon_intersection_neg.area 
        self.area_union = polygon_union_pos.area + polygon_intersection_neg.area - polygon_negatif.area
        ...
        if Plot:
            plt.figure()
            plt.subplot(1,3,1).plot(points[:, 0], points[:, 1], '.',label='area %.1f'%(self.area))
            plt.axis('equal')
            for i, j in edges:
                plt.plot(points[[i, j], 0], points[[i, j], 1])
            plt.legend()
            plt.subplot(1,3,2).plot(points_random[:, 0], points_random[:, 1], '.',label='area %.1f'%(self.area_random))
            plt.axis('equal')
            for i, j in edges_random:
                plt.plot(points_random[[i, j], 0], points_random[[i, j], 1])
            plt.legend()
            ...
            plt.plot(points[:, 0], points[:, 1], '.',color='b',alpha=0.3,label='area %.1f'%(self.area))
            plt.plot(points_random[:, 0], points_random[:, 1], '.',color='g',alpha=0.3,label='area %.1f'%(self.area_random))
    
            patch = PolygonPatch(polygon_union_pos, facecolor='k', edgecolor='k', alpha=0.20,label='union : area %.1f'%(self.area_union))
            patch2 = PolygonPatch(polygon_intersection_pos, facecolor='r', edgecolor='r', alpha=0.20,label='intersection : area %.1f'%(self.area_intersection))
            ax.add_patch(patch)
            ax.add_patch(patch2)
    ...
    def fit_def(self, Draw = False, Mcmc = False, Plot = False, Corner=True, True_param = '', color='r', kind = 'fill1', name='', infos=False, color_truth='b', mean=0):
        x = self.x
        y = self.y
        yerr = self.yerr
        ...
        if Mcmc==True:
            self.samples = mym.MCMC(x, y, yerr, mym.my_func, True_param=True_param, name=name, infos=infos, savegraph=Corner, color=color_truth)
        ...
        if Plot==True:
            mym.FinalPlot(x, y, yerr, mym.my_func, kind = kind,  color = color, name=name, mean=mean)
    
    def fit_line(self, perm=1000, Draw=False, color='k', info=False, fontsize=13, label=True):
        k = perm
        ...
        temp = tableXY(self.x, self.y-((self.x-np.mean(self.x))*self.lin_slope_w+self.lin_intercept_w), self.yerr)
        temp.rms_w()
        self.fit_line_rms = temp.rms
        
        if Draw:
            if label:
                plt.plot(self.x,(self.x-np.mean(self.x))*self.lin_slope_w+self.lin_intercept_w,color=color,ls='-.',
                         label=' $\\mathcal{R}$ = %.2f $\\pm$ %.2f \n S = %.2f $\\pm$ %.2f \n rms : %.2f'%(self.r_pearson_w, self.r_errpearson_w, self.lin_slope_w, self.lin_errslope_w, temp.rms))
            else:
                plt.plot(self.x,(self.x-np.mean(self.x))*self.lin_slope_w+self.lin_intercept_w,color=color,ls='-.')
    ...
    def fit_line_weighted(self, Draw=False, Infos=False,color='k'):
        ...
        self.lin_slope_w = results.params[1] ; self.lin_intercept_w = results.params[0]
        self.r_pearson_w = np.sqrt(results.rsquared)*np.sign(results.params[1])
        self.p_val_w = results.pvalues[1]
        self.lin_errslope_w = results.bse[1] ;  self.lin_errintercept_w = results.bse[0]
        self.rms_w() ; rms1 = self.rms ; rms2 = np.std(self.x)
        self.r_err_w = self.lin_errslope_w*rms2/rms1
        if Draw==True:
            plt.plot([np.min(self.x), np.max(self.x)], [self.lin_intercept_w+self.lin_slope_w*np.min(self.x), self.lin_intercept_w+self.lin_slope_w*np.max(self.x)], 
                     linestyle='-.', color=color, linewidth=1,label=r'S = %.2f $\pm$ %.2f'%(self.lin_slope_w,self.lin_errslope_w)+ '\n'+ r'$\mathcal{R}$ = %.2f '%(self.r_pearson_w))
        if Infos==True:
    ...
    def fit_line_odr(self, Draw=False, Infos=False,color='r'):
        ...
        model = odr.ODR(data, quad_model, beta0=[0., 1.])
        out = model.run()   
    ...
    def fit_gauss_proba(self, Draw=False, guess=[0,1,1], report=False, c='r'):
        ...
        fit_params.add('offset',value=0) ; fit_params['offset'].vary = False
    ...
    def fit_par_morceau(self, cut, degree=1, Draw=False):
        ...
        for j in range(len(cut)-1):
            ...
            y_fine[int(left):int(right)+1] = np.polyval(self.clipped.poly_coefficient,x_fine[int(left):int(right)+1])
    ...
    def fit_discontinuity(self, cut, degree=0, guess=np.zeros(8), Draw=False,report=False,c='r'):  
    ...
    def fit_sinus(self, Draw=False, d=0, guess=[0,1,0,0,0,0], p_max=500, report=False, c='r'):
        ...
        fit_params.add('a',value=guess[4]) ; fit_params['a'].vary = False
        fit_params.add('b',value=guess[5]) ; fit_params['b'].vary = False
        if d==1:
    ...
    def fit_poly2d(self,z,zerr=None,expo1=3,expo2=3,maximum=True,Draw=False,cmap='seismic',ax_label = '',vmin=None,vmax=None,alpha_p=1):
        ...
        self.z_fitted = np.ravel(S_test(self.x[:,np.newaxis],self.y[:,np.newaxis],par))
        ...
        if Draw:
            ...
            plt.scatter(self.x,self.y,c=z,edgecolor='k',vmin=vmin,vmax=vmax,cmap=cmap,alpha=alpha_p)
    
    def fit_gauss(self, Draw = False):
        self.best_par = mym.BestChi(self.x, self.y, self.yerr, mym.gauss, Draw)
        self.best_chi = mym.Chicarre(self.best_par, self.x, self.y, self.yerr, mym.gauss)
    
    def fit_line2(self, Draw = False, Mcmc = False, Plot = False, True_param = '', kind = 'fill1', color = 'r', name = '', infos = False, output_dir = None):
        ...
        if Mcmc==True:
            self.samples = mym.MCMC(self.x, self.y, self.yerr, mym.line, True_param = True_param, name = name, infos = infos, output_dir = output_dir)
            self.med_samples=np.median(self.samples, axis=0)
            self.sup_samples=np.percentile(self.samples, 84,  axis=0)
            self.inf_samples=np.percentile(self.samples, 16, axis=0)
        if Plot==True:
            mym.FinalPlot(self.x, self.y, self.yerr, mym.line, kind = kind,  color = color, name=name)
    ...
    def pattern(self,width_range=[0.1,20],new=True,color='k',label=None):
        ...
        width = 1e-7*abs(np.fft.fftfreq(len(diff_fourrier)))[0:int(len(diff_fourrier)/2)+1]/dl    #transformation of the frequency in mm of the material
        ...
        print('\n[INFO] The interference pattern is produced by material with a width of %.3f mm'%(freq_maximum_ref))
        plt.xscale('log')
        
    
    def circular_corr(self, grid, border=5, dx=1, fill_hole=0, complete_analysis=False, Draw=False,min_pts=10):
        ...
        if len(locations)>0:
            for j in locations:
               new_grid = np.hstack([new_grid,np.arange(grid[j]+dgrid,grid[j]+dgrid*holes[j],dgrid)]) 
        ...
        self.x_backup,self.y_backup,self.xerr_backup,self.yerr_backup = self.x,self.y,self.xerr,self.yerr
        ...
        if ((2*(len(self.x)-border))>min_pts)&(complete_analysis):
                print('number of subplots too high, border = fixed at %.0f'%(len(self.x)-5))
                border = len(self.x)-5
        ...
        for i in range(len(self.x)-border):
            ...
            test = tableXY(self.x[index[new_vec]],self.y[index1[new_vec]],self.xerr[index[new_vec]],self.yerr[index1[new_vec]])
            ...
            if complete_analysis:
                plt.subplot(1,2*(len(self.x)-border)-1,(len(self.x)-border)-(i))
        ...
        for i in range(len(self.x)-border-1):
            ...
            if (complete_analysis)&(i!=0):
                plt.subplot(1,2*(len(self.x)-border)-1,i+(len(self.x)-border))
                test.plot(label='num_points : %.0f'%(len(test.x)))
                test.fit_line(Draw=True,info=True)

        rho  = np.hstack([rho[len(self.x)-border+1:][::-1] ,rho[0:len(self.x)-border]])        
        correlation  = np.hstack([correlation[len(self.x)-border+1:][::-1] ,correlation[0:len(self.x)-border]])
        errcorrelation  = np.hstack([errcorrelation[len(self.x)-border+1:][::-1] ,errcorrelation[0:len(self.x)-border]])
        slopes  = np.hstack([slopes[len(self.x)-border+1:][::-1] ,slopes[0:len(self.x)-border]])
        errslopes  = np.hstack([errslopes[len(self.x)-border+1:][::-1] , errslopes[0:len(self.x)-border]])
        points  = np.hstack([points[len(self.x)-border+1:][::-1] ,points[0:len(self.x)-border]])
        ...
        if Draw:
            ...
            plt.errorbar(np.arange(len(rho))*dx-(len(rho)//2)*dx,correlation,yerr=errcorrelation,fmt='ko') 
            plt.subplot(4,1,3,sharex=ax)
            plt.axvline(x=0,color='k',ls=':')
            plt.errorbar(np.arange(len(rho))*dx-(len(rho)//2)*dx,slopes,yerr=errslopes,fmt='ko') 
            plt.ylabel('slope',fontsize=13)
            plt.subplot(4,1,4,sharex=ax)
            plt.axvline(x=0,color='k',ls=':')
            plt.plot((np.arange(len(rho))*dx-(len(rho)//2)*dx)[points>min_pts],points[points>min_pts])                
    ...
    def rolling_regression(self, box=5, min_freq=None, weighted=True, center=False, outliers=[True, 1.5, 'inter']):
        if weighted:
            if center:
                if box%2==0:
                    box=box+1 
            ...
            for j in range(num):
                if outliers[0]==True:
                    mask = rm_out(self.y[j:box+j], m=outliers[1], kind=outliers[2])[0]
                    X = self.x[j:box+j][mask] ; Y = self.y[j:box+j][mask] ; YERR = (self.yerr[j:box+j][mask])**2
                else : 
                    X = self.x[j:box+j] ; Y = self.y[j:box+j] ; YERR = self.yerr[j:box+j]**2
                X = sm.add_constant(X)
                wls_model = sm.WLS(Y, X, weights=1/YERR)
                results = wls_model.fit()
                list.append([results.params[1] , results.bse[1], results.params[0], results.bse[0], np.sqrt(results.rsquared)*np.sign(results.params[1]), results.pvalues[1]])
            if center:
                list = list+[[np.nan]*6]*int(box/2)
            self.rolling_values = pd.DataFrame(np.array(list), columns=['slope', 'err_slope','intercept', 'err_intercept', 'Rcorr', 'pval'])
        else:
            if center:
                if box%2==0:
                    box=box+1 
            ...
            for j in range(num):
                if outliers[0]==True:
                    mask = rm_out(self.y[j:box+j], m=outliers[1], kind=outliers[2])[0]
                    X = self.x[j:box+j][mask] ; Y = self.y[j:box+j][mask]
                else : 
                    X = self.x[j:box+j] ; Y = self.y[j:box+j]
    ...
    def periodogram_l1(self, starname='', dataset_name='', photon_noise=0, max_n_significant=9, p_min=2, text_output=1, fap_min = -3, method_signi=['fap','evidence_laplace'],sort_val='log10faps',species=None):
        ...
        V = covariance_matrices.covar_mat(self.x, self.yerr, sigmaW, sigmaR, sigma_calib, tau)
        ...
        c.set_model(omegamax = 2*np.pi/p_min, #We will search frequencies up to 1.5 cycles/day = 3*np.pi rad/day
                    V = V,
        ...
        if len(c.omega_peaks)>0:
            ...
            c.significance['log10_bayesf_laplace'] = c.significance['log10_bayesf_laplace'][sort]
    ...
    def diff_phase_periodogram(self, vec2, detrend_deg=2, Plot=True):
        ...
        if Plot:
            ...
            plt.subplot(5,1,1)
            ax = plt.gca()
            self.detrend_poly.periodogram(nb_perm=1,all_outputs=True,color='k',Norm=True)
            vec2.detrend_poly.periodogram(nb_perm=1,all_outputs=True,color='r',Norm=True)
            
            plt.subplot(5,1,2,sharex=ax)
            plt.plot(1/self.detrend_poly.freq, self.detrend_poly.phase*180/np.pi+180,color='k')
            plt.plot(1/vec2.detrend_poly.freq, vec2.detrend_poly.phase*180/np.pi+180,color='r')
            ...
            plt.scatter(1/self.detrend_poly.freq, self.detrend_poly.power,c=dist,cmap='jet')
            plt.subplot(5,1,5,sharex=ax)
            grad_dist = abs(np.gradient(dist))
            plt.scatter(1/self.detrend_poly.freq, self.detrend_poly.power,c=grad_dist,vmin=np.percentile(grad_dist,16),vmax=np.percentile(grad_dist,84),cmap='jet')
            ...
            plt.plot(1/self.detrend_poly.freq, prim[1]*self.detrend_poly.power,color='k')   

            self.phase_penality = prim[1]                     
        
        else:
            self.detrend_poly.periodogram(nb_perm=1,all_outputs=True,color='k',Norm=True,Plot=False)
            vec2.detrend_poly.periodogram(nb_perm=1,all_outputs=True,color='r',Norm=True,Plot=False)
    ...
    def polar_periodogram(self,dtime=1):
        ...
        fap10 = self.fap10/norm ; fap1 = self.fap1/norm ; fap01 = self.fap01/norm ; fap = self.fap/norm
        ...
        plt.plot(np.linspace(0,2*np.pi,100),fap1*np.ones(100),color='r',ls='-',lw=2.5)
    ...
    def corr(self,tablexy,new=False,detrend=0,color='k',cmap=None,capsize=3,fontsize=13, Draw=True):
    ...
    def corr_elliptical(self,tablexy,window=5):
        ...
        baseline = np.max(table.x) - np.min(table.x) ; period_ref = (baseline/10)
        ref = table.copy() ; ref.y*=0 ; ref.y+=np.sin(2*np.pi/period_ref*ref.x)
        ...
        width_peak_freq = abs(myf.find_nearest(power_spectrum_ref.min_extremum.y,1/period_ref)[2][0])
    ...
    def periodogram(self, nb_perm=1, ofac=10, p_min=0, detrending=0, p_max=None, Plot=True, Norm=False, norm_val=False, supp=None, infos=False, level=None, color='k', ls='-', P_th=None, axis_y_var='p', axis_x_var='p', xlim=[None,None], all_outputs=False, compute_fap_levels=True, legend='', zorder=1, lw=1.5, alpha=1, warning=True):
        ...
        if np.product(self.yerr!=0)==0:
            if warning:
                print('At least one weight is null ! Reperform the analysis taking care of it.')
            self.yerr[self.yerr==0]=np.min(self.yerr[self.yerr!=0])/10
            cond = 0

        if np.sum(self.yerr!=0)==0:
            if warning:
                print('All the weights are null ! Warning : Kernal could died. Reperform the analysis taking care of it.')
            self.yerr[self.yerr==0]=1 
            cond = 0
            
        if np.sum(abs(self.y))==0:
            if warning:
                print('All the y are null ! Warning : Kernal could died. Reperform the analysis taking care of it.')
            cond = 0
        ...
        if cond>5:
            ...
            fap = np.max(pXav[1])
            self.fap=fap ; self.fap01 = None ; self.fap1 = None ; self.fap10 = None
            ...
            if all_outputs != 'no':
                ...
                self.phase = pXav[2]#-2*np.pi*pXav[0]*np.min(self.x))%(2*np.pi) #np.sin(t+phi) sign positif convention
            ...
            self.fap_max = 100*myf.fap(self.power_max, self2.x, self2.yerr, np.max(self.freq), p0=1, Teff=None)
            
            if compute_fap_levels:
                if nb_perm>1:
                    if level!=None:
                        fap = f3.calculate_fap(self2.x, self2.y, sig_y = self2.yerr, level = level,  nb_perm = nb_perm, ofac = ofac)
                        self.fap = fap 
                    else:
                        fap01 = f3.calculate_fap(self2.x, self2.y, sig_y = self2.yerr, level = 0.999,  nb_perm = nb_perm, ofac = ofac)                
                        fap = f3.calculate_fap(self2.x, self2.y, sig_y = self2.yerr, level = 0.99,  nb_perm = nb_perm, ofac = ofac)    
                        fap10 = f3.calculate_fap(self2.x, self2.y, sig_y = self2.yerr, level = 0.90,  nb_perm = nb_perm, ofac = ofac)    
                        self.fap01 = fap01 ; self.fap1 = fap ; self.fap10 = fap10 
                else:
                    dichotomic = np.array([100*myf.fap(self.power_max*0.01*j, self2.x, self2.yerr, np.max(self.freq), p0=1, Teff=None) for j in range(1,201,10)])
                    dicho_power = np.array([self.power_max*0.01*j for j in range(1,201,10)])
                    ...
                    if level==None:
                        ...
                        self.fap01 = dic.y[0] ; self.fap1 = dic.y[1] ; self.fap10 = dic.y[2] ; fap = dic.y[1] ; self.fap = fap
                    else:
                        fap_liste = np.array([np.log10(100*(1-level))])
                        dic.interpolate(new_grid=fap_liste,method='linear')   
                        fap = dic.y[0] ; self.fap = fap
                            
            high = np.where(pXav[1]>fap)[0][np.in1d(np.where(pXav[1]>fap)[0], argrelextrema(pXav[1], np.greater)[0])]
            self.period_significant = 1/pXav[0][high[pXav[1][high].argsort()[::-1]]]
            self.power_significant = pXav[1][high[pXav[1][high].argsort()[::-1]]]
            if all_outputs != 'no':
                self.amplitude_significant = self.amplitude[high[pXav[1][high].argsort()[::-1]]]
                self.damplitude_significant = 0.5*abs(self.amplitude[(high[pXav[1][high].argsort()[::-1]])+1]-self.amplitude[(high[pXav[1][high].argsort()[::-1]])-1])
                self.phase_significant = self.phase[high[pXav[1][high].argsort()[::-1]]]
                self.dphase_significant = 0.5*abs(self.phase[(high[pXav[1][high].argsort()[::-1]])+1]-self.phase[(high[pXav[1][high].argsort()[::-1]])-1])
            ...
            if Plot==True:
                if axis_x_var=='f':
                    ...
                    if Norm==True:
                        ...
                        ax1.set_ylabel(['Power','K Amplitude'][int(axis_y_var!='p')],fontsize=14)
                    if P_th!=None:
                    ...
                    if Norm==True:
                        if norm_val:
                            plt.plot(1/pXav[0], pXav[1]/fap, color=color, ls=ls, label=legend, zorder=zorder, lw=lw, alpha=alpha)
                            plt.axhline(y=1, linestyle='-.', color='k',zorder=zorder+1)                  
                            plt.ylabel('Power normalised',fontsize=14)
                        else:
                            plt.plot(1/pXav[0], pXav[1], color=color, ls=ls, label=legend, zorder=zorder, lw=lw, alpha=alpha)
                            plt.axhline(y=fap, linestyle='-.', color='k',zorder=zorder+1)                  
                            plt.ylabel(['Power','K Amplitude'][int(axis_y_var!='p')],fontsize=14)
                        ...
                        if self.fap1 is not None:
                            plt.axhline(y=self.fap1, linestyle='-', color='k',label='e')
            ...
            if infos:
                for k in range(int(infos)):
                    plt.annotate(' P = %.2f \n K = %.2f +/- %.2f \n Phi = %.2f +/- %.2f'%(self.period_significant[k],self.amplitude_significant[k],self.damplitude_significant[k], self.phase_significant[k],self.dphase_significant[k]), 
                                 xy=(self.period_significant[k], self.power_significant[k]/fap), 
                                 xytext=(self.period_significant[k], 1.5*(self.power_significant[k]/fap)), ha='center', va='bottom',
    ...
    def periodogram_keplerian_hierarchical(self, periods=[None], fap=0.1, photon_noise=0, nb_planet=8, Plot=False, deg=0, ms = 1, ofac=20, p_min=0, model=False, auto_long_trend=True, fit_ecc=True, mcmc=False, jitter=0.8, periodic=True, species=None, min_nb_cycle=1.5, nb_bins=9, capsize=0, eval_on_t=None, transits=None, sort_planets=True,known_planet=[]):
        ...
        if deg>-1:
            ...
            self.detrend_poly.periodogram(nb_perm=1, Norm=True, level=1-fap/100, Plot=False, p_min=p_min)
            long_power = int((self.detrend_poly.power[0]/self.detrend_poly.fap)>1)
            if (long_power)&(auto_long_trend):
                deg += 1
                print('Power at long period found, power detrending increased to : %.0f'%(deg))
        ...
        if spleaf_version=='old':
            rv_model = rvmodel(x_backup-epoch_rjd, y_backup, yerr**2, inst_id=species_id, var_jitter_inst=np.array([float(jitter)]*len(np.unique(species))))
        else:
            yerr_rv = term.Error(yerr)
            instjit = {}
            instruments = np.unique(species)    
            for inst in instruments:
                instjit[f'inst_jit_{inst}'] = term.InstrumentJitter(species==inst, jitter)
        ...
        if len(np.unique(species))-1:
            for num,kinst in enumerate(np.unique(species)):
                if spleaf_version=='old':
                    rv_model.addlin(1.0*(species==kinst), 'offset_inst_{}'.format(kinst))
                else:
                    rv_model.add_lin(1.0*(species==kinst), 'offset_inst_{}'.format(kinst))
        
        P_kep = [] ; K_kep = [] ; e_kep = [] ; la0_kep = [] ; w_kep = [] ; a_kep = [] ; mass_kep = [] ; phi_kep = []
        
        if Plot:
            plt.figure(figsize=(14,3+3*(nb_planet)))
            plt.subplots_adjust(left=0.06,right=0.97,top=0.95,bottom=0.10,hspace=0,wspace=0.4)
        ...
        for i in range(1,nb_planet+1):
            ...
            ts.periodogram(nb_perm=1, Plot=Plot, level=1-fap/100, Norm=True, ofac=ofac, p_min=p_min, p_max=p_max)
            ...
            if Plot:
                ...
                plt.ylabel('Power',fontsize=14)
                plt.xlabel(None)
                plt.tick_params(direction='in',top=True,labeltop=(i==0),labelbottom=False)
                if periods[i-1] is not None:
                    ...
                    fap_max = [10,1,0.1][int(curve.x_max[close]>ts.fap01)+int(curve.x_max[close]>ts.fap1)]
                else:
            ...
            if ((ts.power_max>=ts.fap)&(not button))|((i!=len(periods)+1)&(button)):
                if spleaf_version=='old':
                    rv_model.smartaddpla(perio_max)
                    rv_model.changeparpla(rv_model.planame[i-1], params=['P', 'la0', 'K', 'ecosw', 'esinw'])
                    if not fit_ecc:
        ...
        if i:
            for j in range(1,i+1):
                if spleaf_version=='old':
                    rv_model.changeparpla(rv_model.planame[j-1], params=['P', 'la0', 'K', 'w', 'e'])
                    ...
                    rv_model.changeparpla(rv_model.planame[j-1], params=['P', 'la0', 'K', 'ecosw', 'esinw'])
                else:
                    rv_model.set_keplerian_param(str(j-1), param=['P', 'la0', 'K', 'w', 'e'])                
                    ...
                    rv_model.set_keplerian_param(str(j-1), param=['P', 'la0', 'K', 'ecosw', 'esinw'])                 
                mass_planet = myf.AmpStar(ms, 0, P_planet, abs(K_planet), i=90, e=e_planet, code='Sun-Earth')
                a_planet = (ms*7.496e-6*P_planet**2)**(1/3)
                phi_planet = (la0_planet*180/np.pi-360*epoch_rjd/P_planet -270)%360
                ...
                if Plot:
                    if spleaf_version=='old':
                        res = tableXY(x_backup, rv_model.residuals() + rv_model.kep[j-1].rv(x_backup - epoch_rjd), yerr)
                    else:
                        res = tableXY(x_backup, rv_model.residuals() + rv_model.keplerian[str(j-1)].rv(x_backup - epoch_rjd), yerr)
                    ...
                    plt.tick_params(direction='in',top=True,labeltop=((j-1)==0),labelbottom=False)
                    if not j-1:
                        ax.xaxis.set_label_position('top')
                        ax.yaxis.set_major_formatter(FormatStrFormatter('%.0f'))
                        plt.xlabel('Phase',fontsize=14)
                    
                    ax.annotate('P = %s days \nK = %.2f m/s \ne = %.2f \nMsin(i) = %.1f'%(myf.format_number(P_planet,digit=2), K_planet, e_planet, mass_planet),xy=(1.02,0.33),xycoords='axes fraction',horizontalalignment='left',fontsize=13)
                    
                    res.plot(modulo=P_planet, periodic=True, modulo_norm=True,alpha=0.5,capsize=capsize)
                    ...
                    plt.errorbar(res.mod.binned.x,res.mod.binned.y,yerr=res.mod.binned.yerr,capsize=0,fmt='ro',markersize=8,markeredgecolor='k',markeredgewidth=2,zorder=1001)
                    ...
                    if transits is not None:
                        if type(transits)==dict:
                            ...
                            p_detect = all_p[myf.find_nearest(all_p.astype('float'),P_planet)[0][0]]
                            if transits[p_detect][0] is not None:
                                jdb_transit = transits[p_detect]
                                for trans in jdb_transit:
                                    plt.axvline(x=(trans%P_planet)/P_planet,color='k',alpha=0.8,lw=2,zorder=1001)
                        else:
                            for color_trans,trans in zip(['r','g','b',
                                                          'purple','olive','cyan',
                                                          'pink','lime','navy',
                                                          'brown','gold','teal','k'][0:len(transits)],transits):
                                plt.axvline(x=(trans%P_planet)/P_planet,color=color_trans,alpha=0.8,lw=2,zorder=1001)                            
        ...
        self.planet_fitted = pd.DataFrame({'p':P_kep, 'p_std':0,
                                           'k':K_kep, 'k_std':0,
                                           'e':e_kep, 'e_std':0,
                                           'phi':phi_kep, 'phi_std':0,
                                           'peri':w_kep, 'peri_std':0, #before update node
                                           'long':la0_kep, 'long_std':0, #before update peri
                                           'a':a_kep, 'a_std':0,
        ...
        self.planet_fitted.index = ['planet %.0f'%(i) for i in range(1,1+nb_planet)]
        ...
        for kpow in range(deg+1):
            if spleaf_version=='old':
                self.model_drift += rv_model.get_params('lin.drift_pow%.0f'%(kpow)) * (x_backup-epoch_rjd)**(kpow)
            else:
                self.model_drift += rv_model.get_param('lin.drift_pow%.0f'%(kpow)) * (x_backup-epoch_rjd)**(kpow)                
        for kpla in range(nb_planet):
            if spleaf_version=='old':
                self.model_keplerian += rv_model.kep[kpla].rv(x_backup - epoch_rjd)
                self.model_keplerian_i.append(tableXY(x_backup,rv_model.kep[kpla].rv(x_backup - epoch_rjd)))
            else:
                self.model_keplerian += rv_model.keplerian[str(kpla)].rv(x_backup - epoch_rjd)
                self.model_keplerian_i.append(tableXY(x_backup,rv_model.keplerian[str(kpla)].rv(x_backup - epoch_rjd)))
        ...
        if Plot:
            if i:
                ...
                plt.subplot(1,2,2)
                plt.subplots_adjust(left=0.06,right=0.97,top=0.90,bottom=0.20,hspace=0,wspace=0.4)
    
            res = tableXY(x_backup, rv_model.residuals(), yerr)
            
            
            res.periodogram(nb_perm=1, Plot=Plot, level=1-fap/100, Norm=True, ofac=ofac, p_min=p_min, p_max=p_max)
            for periods_planets in known_planet:
                plt.axvline(x=periods_planets,color='b',alpha=0.25)
            
            axe = plt.gca()
            plt.plot(axe.get_xlim(),[res.fap,res.fap],ls='-.',label='FAP = %.1f%%'%(fap),color='k')
            plt.legend(loc=1)
            plt.ylabel('Power',fontsize=14)
            plt.ylim(0.001,None)
            ...
            self.y+=self.model_drift
            res.plot(label='Res.',color='k',species=species,capsize=capsize,markersize=4)
            ...
            if model:
                ...
                for kpow in range(deg+1):
                    if spleaf_version=='old':
                        drift += rv_model.get_params('lin.drift_pow%.0f'%(kpow)) * (new_t-epoch_rjd)**(kpow)
                    else:
                        drift += rv_model.get_param('lin.drift_pow%.0f'%(kpow)) * (new_t-epoch_rjd)**(kpow)                        
                for kpla in range(nb_planet):
                    if spleaf_version=='old':                    
            ...
            self.bic = bic
            nax.annotate(r'$\chi^2_r$ = %.3f'%(chi2),xy=(1.02,0.57),xycoords='axes fraction',horizontalalignment='left',fontsize=13)
            nax.annotate(r'BIC = %.2f'%(bic),xy=(1.02,0.33),xycoords='axes fraction',horizontalalignment='left',fontsize=13)
            nax.annotate(r'LTD = %.0f'%(deg),xy=(1.02,0.09),xycoords='axes fraction',horizontalalignment='left',fontsize=13)

            plt.title('rms_input = %.2f m/s         rms_residuals = %.2f m/s'%(vec_init.rms, res.rms),fontsize=13)
            plt.legend()
            plt.ylabel('RV [m/s]',fontsize=14)
            self.rv_residues = res
            self.rv_residues_rms = res.rms
            self.model_info = {'rms_res':res.rms,'rms_input':vec_init.rms,'chi2':self.chi2,'bic':self.bic,'nb_obs':len(self.x),'timespan':np.max(self.x)-np.min(self.x)}
    ...
    def planetary_phase_test(self, periods=None, vecs=None, ref_vector=None, ref_time=0, split_vector=None, fit_ecc=False, deg_detrend=0, deg=0, photon_noise=0, nb_planet=1, nb_cut=2, bbox=(-0.25, -0.35), markers = ['o','x','^','s','v','<','>'], colors = ['blue','red','green','orange','purple'],Plot=True, rmax=None, ymax=None, xmin=None, xmax=None):
        ...
        if periods is None:
            vec_lbl.periodogram_keplerian_hierarchical(Plot=False,nb_planet=nb_planet,fit_ecc=fit_ecc,photon_noise=photon_noise,fap=1)
        ...
        if len(periods):
            ...
            if vecs is None:
                if split_vector is None:
                    limits = np.array([np.percentile(vec_lbl.x,100/nb_cut*i) for i in range(0,nb_cut)])
                    if np.min(np.diff(limits))<1.5*np.max(periods):
                        nb_cut-=1
                        print('Cut number reduced of 1 otherwise baseline to short for the planetary fit')
                    vecs = [vec_lbl.masked((vec_lbl.x>=np.percentile(vec_lbl.x,100/nb_cut*i))&(vec_lbl.x<=np.percentile(vec_lbl.x,100/nb_cut*(i+1))),replace=False) for i in range(0,nb_cut)]                
                    rhks = [ref_vector.masked((vec_lbl.x>=np.percentile(vec_lbl.x,100/nb_cut*i))&(vec_lbl.x<=np.percentile(vec_lbl.x,100/nb_cut*(i+1))),replace=False) for i in range(0,nb_cut)]                
                else:
                    if len(split_vector)==len(vec_lbl.x):
                        vecs = [vec_lbl.masked((split_vector>=np.percentile(split_vector,100/nb_cut*i))&(split_vector<=np.percentile(split_vector,100/nb_cut*(i+1))),replace=False) for i in range(0,nb_cut)]                
                        rhks = [ref_vector.masked((split_vector>=np.percentile(split_vector,100/nb_cut*i))&(split_vector<=np.percentile(split_vector,100/nb_cut*(i+1))),replace=False) for i in range(0,nb_cut)]                
                    else:
            ...
            for b in range(len(vecs)):
                ...
                for p in periods:
                    base.append(np.cos(2*np.pi*v.x/p)) ; base.append(np.sin(2*np.pi*v.x/p))
                base = np.array(base)
        
                v.fit_base(base) 
                v.y = v.y - np.sum(v.coeff_fitted[0:1+deg]*base[0:1+deg].T,axis=1)
                
                k.append(np.sqrt(v.coeff_fitted[1+deg::2]**2+v.coeff_fitted[2+deg::2]**2))         
                f.append((np.arctan2(v.coeff_fitted[1+deg::2],v.coeff_fitted[2+deg::2])*180/np.pi)%360)          
                dk.append(np.sqrt((v.coeff_fitted[1+deg::2]**2*v.coeff_fitted_std[1+deg::2]**2+v.coeff_fitted[2+deg::2]**2*v.coeff_fitted_std[2+deg::2]**2)/k[-1]**2))            
                ...
                df.append(180/np.pi*np.arctan(np.sqrt((v.coeff_fitted_std[2+deg::2]/v.coeff_fitted[1+deg::2])**2+(v.coeff_fitted_std[1+deg::2]*v.coeff_fitted[2+deg::2]/v.coeff_fitted[1+deg::2]**2)**2)))
            ...
            if Plot:
                ...
                for j in range(len(periods)):
                    m = markers[j]
                    for b in range(1,len(vecs)):
                        t1 = myf.polar_err(f[b][j],k[b][j],df[b][j],dk[b][j], color=colors[b-1], marker=m)
                    t = myf.polar_err(f[0][j],k[0][j],df[0][j],dk[0][j],color='k',marker=m)
                    plt.polar(f[0][j]*np.pi/180,k[0][j],'k'+m,label='P=%.2f'%(list(periods)[j]))
                    plt.legend(loc=3 ,bbox_to_anchor=bbox, ncol=3)
                
                dist = [sum(myf.dist_modulo(d*np.pi/4,np.pi/180*np.ravel(f))) for d in [1,2,3,5,6,7]]        
            ...
            return {'phi':f,'k':k,'phi_std':df,'k_std':dk,'axe_x':save_x,'axe_x2':save_x2,'axe_y':save_y,'p':periods}

        
        
    def periodogram_auto_detrend(self, kmin=7, emax=0.85, photon_noise=0.3, fap=0.1, p_min=1.1, jitter=0.7, fit_ecc=True, species=None, power_max=3, degree_max=2, x_export=None):
        ...
        for power in np.arange(power_max+1):
            ...
            for deg in np.arange(degree_max+1):
                ...
                vec.detrend_poly.periodogram_keplerian_hierarchical(photon_noise=photon_noise, jitter=jitter, fap=fap, deg=deg, p_min=p_min, fit_ecc=fit_ecc, species=species, periodic=0.25, Plot=False, auto_long_trend=False)
                vec.detrend_poly.periodogram_keplerian_hierarchical(photon_noise=photon_noise, jitter=jitter, fap=fap, deg=deg, p_min=p_min, fit_ecc=fit_ecc, species=species, periodic=0.25, Plot=True, nb_planet=vec.detrend_poly.nb_planet_fitted, auto_long_trend=False)
                
                bic = (vec.detrend_poly.nb_par+power)*np.log(len(vec.detrend_poly.x)) + vec.detrend_poly.rv_model.chi2() #np.sum(vec.detrend_poly.rv_residues.y**2)/np.sum(vec.detrend_poly.yerr**2)
                
                save.append([power,deg,vec.detrend_poly.keplerian_converged,vec.detrend_poly.bic,bic,vec.detrend_poly.rv_residues_rms,vec.detrend_poly.nb_planet_fitted])
                plt.close('all')
        tab = pd.DataFrame(save,columns=['deg_detrend','deg_fit_kep','conv','bic','bic2','rms','nb_planet'])
        tab = tab.dropna()
        ...
        if len(tab):
            ...
            vec = self.copy()
            vec.substract_polyfit(power,replace=False)
            vec.detrend_poly.periodogram_keplerian_hierarchical(sort_planets=False, photon_noise=photon_noise, jitter=jitter, fap=fap, deg=deg, p_min=p_min, fit_ecc=fit_ecc, species=species, periodic=0.25, Plot=True, nb_planet=nb_planet, auto_long_trend=False)
            print('\n----------------------------\n[INFO] Keplerian model :\n----------------------------\n')
            print(vec.detrend_poly.planet_fitted[['p','k','e','mass','peri','long']])
            ...
            def produce_model(x):
                ...
                for kpow in range(deg+1):
                    if spleaf_version=='old':
                        model_drift += vec.detrend_poly.rv_model.get_params('lin.drift_pow%.0f'%(kpow)) * (x-np.mean(vec.x))**(kpow)
                    else:
                        model_drift += vec.detrend_poly.rv_model.get_param('lin.drift_pow%.0f'%(kpow)) * (x-np.mean(vec.x))**(kpow)
                
                t = vec.detrend_poly.planet_fitted.loc[(vec.detrend_poly.planet_fitted['k']>kmin)&(vec.detrend_poly.planet_fitted['e']<emax)]
                pp = np.where((vec.detrend_poly.planet_fitted['k']>kmin)&(vec.detrend_poly.planet_fitted['e']<emax))[0]
                self.auto_nb_planet = len(pp)
                for kpla in pp:
                    if spleaf_version=='old':     
                        model_keplerian += vec.detrend_poly.rv_model.kep[kpla].rv(x - np.mean(vec.x))
                    else:
                        model_keplerian += vec.detrend_poly.rv_model.keplerian[str(kpla)].rv(x - np.mean(vec.x))                        
            ...
            model_smooth = tableXY(myf.vec_oversamp(vec.x),produce_model(myf.vec_oversamp(vec.x))[0])
            if x_export is None:
                x_export = vec.x
    ...
    def evaluate_keplerian(self, planet_fitted, epoch_rjd=None):
        ...
        table_param['ecosw'] = table_param['e']*np.cos(table_param['peri']*np.pi/180)
        table_param['esinw'] = table_param['e']*np.sin(table_param['peri']*np.pi/180)
        ...
        if spleaf_version=='old':
            rv_model = rvmodel(self.x, self.y, self.yerr**2, inst_id=species_id, var_jitter_inst=np.array([float(jitter)]*len(np.unique(species))))
        else:
        ...
        for kpla in table_keplerian.index:
            ...
            if spleaf_version=='old':
                ...
                rv_model.set_params(np.array(table_keplerian.loc[kpla][params2[:-1]]), rv_model.fitparams)
    ...
    def periodogram_rolling(self, prot_estimated=25, windows=100, min_nb_points=25, ofac=10, inv=False):
            ...
            if (len(proxy1.x)>min_nb_points):
                ...
                if (jdb_old_min!=proxy1.x[0])|(jdb_old_max!=proxy1.x[-1]):
                    ...
                    if ((np.max(proxy1.x)-np.min(proxy1.x))>1.5*prot_estimated):
                        ...
                        power = tableXY(proxy1.freq, proxy1.power/proxy1.fap01,0*proxy1.freq)
                        power.interpolate(1/np.linspace(pmin,pmax,1000),method='linear',interpolate_x=False)
        ...
        if len(all_power)>5:
            if not inv:
                myf.my_colormesh(np.linspace(pmin,pmax,1000),time_idx, all_power, vmin=0, vmax=2)
                plt.title('Rotational period : %.1f'%(prot_estimated),fontsize=14)
                plt.xlabel('Period [days]',fontsize=14)
                plt.ylabel('Window (%.0f days) center idx'%(windows),fontsize=14)
            else:
                myf.my_colormesh(time_idx, np.linspace(pmin,pmax,1000), all_power.T, vmin=0, vmax=2)
                plt.title('Rotational period : %.1f'%(prot_estimated),fontsize=14)
    ...
    def periodogram_cumu(self, min_nbpoints = 20, nb_perm=1, ofac=5, grid_period = np.arange(0.5,400,0.1), Plot=True, zoom=5, time_direction=1,  all_plot=False):
        all_power = []
        all_fap = []
        all_amplitude = []
        all_phase = []
        ...
        for j in tqdm(np.arange(min_nbpoints,len(self.x)-1)[::-1]):
            ...
            for obs,save in zip([new.power,new.amplitude,new.phase],[all_power,all_amplitude,all_phase]):
                test = tableXY(1/new.freq,obs)
                test.order()
                test.interpolate(new_grid=grid_period,replace=False,method='linear')
                test.y_interp[(grid_period<np.min(test.x))|(grid_period>np.max(test.x))] = 0
    ...
    def periodogram_cumu_plot(self,zoom=5,minp=2.5,maxp=50,vmin=0.5,vmax=1.5,logz=False,all_plot=False):
        ...
        if all_plot:
            plt.figure(figsize=(18,6))
            plt.subplot(1,3,1)
            myf.my_colormesh(self.all_period[mask_period],self.sequence,z,vmin=vmin,vmax=vmax,zoom=zoom)            
            ax = plt.gca()
            plt.colorbar()
            plt.subplot(1,3,2,sharex=ax,sharey=ax)
            myf.my_colormesh(self.all_period[mask_period],self.sequence,self.all_amplitude[:,mask_period],
                             zoom=zoom,vmin=np.nanpercentile(self.all_amplitude,16),vmax=np.nanpercentile(self.all_amplitude,84),cmap='jet')            
            plt.colorbar()
            plt.subplot(1,3,3,sharex=ax,sharey=ax)
            myf.my_colormesh(self.all_period[mask_period],self.sequence,self.all_phase[:,mask_period],
                             zoom=zoom,vmin=np.nanpercentile(self.all_phase,16),vmax=np.nanpercentile(self.all_phase,84),cmap='twilight')            
            plt.colorbar()
        else:
            myf.my_colormesh(self.all_period[mask_period],self.sequence,z,vmin=vmin,vmax=vmax,zoom=zoom)
        plt.xlim(minp,maxp)
    ...
    def xav_bis(self):
        model, offset, contrast, rv, fwhm, sig_offset, sig_contrast, sig_v0, sig_fwhm, iterations, span, ff, ee, len_depth = fit3.gauss_bis(self.x,self.y,self.yerr, np.ones(10000,'d'))
    ...
    def match_x(self,table_xy,replace=False):
        ...
        if replace:
            self.x, self.y, self.xerr, self.yerr = self.x[match[:,0]], self.y[match[:,0]], self.xerr[match[:,0]], self.yerr[match[:,0]]
            table_xy.x, table_xy.y, table_xy.xerr, table_xy.yerr = table_xy.x[match[:,1]], table_xy.y[match[:,1]], table_xy.xerr[match[:,1]], table_xy.yerr[match[:,1]]
        else:
            v1 = tableXY(self.x[match[:,0]], self.y[match[:,0]], self.xerr[match[:,0]], self.yerr[match[:,0]])
            v2 = tableXY(table_xy.x[match[:,1]], table_xy.y[match[:,1]], table_xy.xerr[match[:,1]], table_xy.yerr[match[:,1]])
            return v1,v2
    ...
    def fit_kernel0(self, guess=None, Plot=True, color='r',free_offset=True,free_center=True):
    ...
    def fit_multigaussian(self, guess=[], model='gauss',vrot=1.5,lim='pouet'):
        ...
        bounds = (list(np.hstack([np.array(guess[0:lengh])-dop_max,
                                           -1*np.ones(lengh),
                                           0*np.ones(lengh),
                                           vrot_min, 0.8])),
                                 list(np.hstack([np.array(guess[0:lengh])+dop_max,
                                                          0*np.ones(lengh), 
                                                          1*np.ones(lengh),
                                                          vrot_max,1]))) #100 m/s doppler max 
        
        if lim == 'none':
            popt, pcov = opt.curve_fit(lambda selfx, *p0: wrapper(selfx, *p0), selfx, selfy, p0=p0) #call with lambda function
        else:
            popt, pcov = opt.curve_fit(lambda selfx, *p0: wrapper(selfx, *p0), selfx, selfy, p0=p0, method='trf',bounds=bounds) #call with lambda function
        yfit = gen_data(selfx, popt[0:lengh], popt[lengh:2*lengh], popt[2*lengh:3*lengh], popt[-2], popt[-1], noise=0)
        plt.plot(selfx,yfit)
        for j in range(lengh):
            if lengh!=1:
                plt.plot(selfx,gen_data(selfx, [popt[0:lengh][j]], [popt[lengh:2*lengh][j]], [popt[2*lengh:3*lengh][j]], [popt[-2]], [popt[-1]], noise=0))
        self.gauss_par = popt
        self.gauss_cov = pcov
    ...
    def interpolate2d(self, z, new_grid = 'auto', method = 'linear', replace = True, fill_value = 'extrapolate'):
        if type(new_grid)==str:
            X, Y = np.meshgrid(np.linspace(self.x.min(),self.x.max(),50),np.linspace(self.y.min(),self.y.max(),50))
            
        if type(new_grid)==int:
            X, Y = np.meshgrid(np.linspace(self.x.min(),self.x.max(),new_grid),np.linspace(self.y.min(),self.y.max(),new_grid))
        ...
        if replace:
            ...
            self.interpolated = tableXY(self.x_interp,self.y_interp,self.z_interp,self.z_interp)
    ...
    def export_to_dace(self,file_name,convert_to_kms=1,convert_to_jdb=0):
        ...
        matrice = np.array([self.x+convert_to_jdb,convert_to_kms*self.y,convert_to_kms*self.yerr]).T
        try:
            np.savetxt(file_name,matrice,delimiter='\t',header='jdb\tvrad\tsvrad\n---\t----\t-----')
